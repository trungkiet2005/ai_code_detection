"""
[EXP24] MambaCode: Selective State Space Model + MoE for Code Detection

Core novelty: Replace cross-attention fusion with Mamba-style SSM + MoE routing.
  - Selective SSM: input-dependent dynamics (like attention but O(n) complexity)
  - K=4 SSM experts, top-2 selected per view
  - Each expert specializes in a different code representation
  - Load balancing loss prevents expert collapse
  - Linear-time complexity vs quadratic attention

Architecture: Multi-view SSM scanner + MoE routing
  loss = L_focal + lambda_balance*L_load_balance + 0.2*L_ssm_aux

Inspired by: Mixture-of-Mamba (ICLR 2025), Mamba (Gu & Dao 2024)
...
class Config:
    ...
    ssm_d_state: int = 16              # MambaCode: SSM state dimension
    ssm_d_conv: int = 4                # MambaCode: conv kernel size
    ssm_expand: int = 2                # MambaCode: expansion factor
    moe_num_experts: int = 4           # MambaCode: number of SSM experts
    moe_top_k: int = 2                 # MambaCode: top-k routing
    lambda_balance: float = 0.01       # MambaCode: load balancing loss weight
    lambda_ssm_aux: float = 0.2        # MambaCode: SSM auxiliary loss weight
    ...
    save_dir: str = "./mambacode_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
# ============================================================================
# MambaCode: Selective SSM + MoE Components (Exp24 novelty)
# ============================================================================
...
class SelectiveSSMLayer(nn.Module):
    ...
    h_t = A_bar * h_{t-1} + B_bar * x_t  (state update, input-dependent)
    ...
    def __init__(self, d_model: int, d_state: int = 16, d_conv: int = 4, expand: int = 2):
        ...
        # Fixed A matrix (log-space for stability)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Conv
        x_conv = self.conv1d(x_path.transpose(1, 2))[:, :, :L].transpose(1, 2)
        x_conv = F.silu(x_conv)

        # Selective params from input
        x_dbl = self.x_proj(x_conv)  # (B, L, d_state*2 + 1)
        B_sel = x_dbl[:, :, :self.d_state]
        C_sel = x_dbl[:, :, self.d_state:2*self.d_state]
        delta = F.softplus(self.dt_proj(x_dbl[:, :, -1:]))  # (B, L, d_inner)
        ...
        for t in range(L):
            ...
            B_bar = dt.unsqueeze(-1) * B_sel[:, t, :].unsqueeze(1)  # (B, d_inner, d_state)
            h = A_bar * h + B_bar * x_conv[:, t, :].unsqueeze(-1)
            y[:, t, :] = (h * C_sel[:, t, :].unsqueeze(1)).sum(dim=-1)
...
class MambaExpert(nn.Module):
    """Single Mamba expert with SSM layer."""
    def __init__(self, d_model: int, d_state: int = 16, d_conv: int = 4, expand: int = 2):
        super().__init__()
        self.ssm = SelectiveSSMLayer(d_model, d_state, d_conv, expand)
...
class MoEMambaFusion(nn.Module):
    """Mixture-of-Mamba-Experts for multi-view code fusion."""
    ...
    def __init__(self, d_model: int, num_experts: int = 4, top_k: int = 2,
        ...
        self.experts = nn.ModuleList([
            MambaExpert(d_model, d_state, d_conv, expand) for _ in range(num_experts)
        ])
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        x: (B, 3, D) from [token, ast, struct] views stacked.
        Returns: (fused (B, D), load_balance_loss)
        """
        B, S, D = x.shape
        x_flat = x.reshape(B * S, D)  # (B*3, D)

        # Router logits
        router_logits = self.router(x_flat)  # (B*3, num_experts)
        ...
        for k in range(self.top_k):
            expert_indices = topk_indices[:, k]  # (B*3,)
            expert_weights = topk_probs[:, k]    # (B*3,)
            for e_idx in range(self.num_experts):
                mask = expert_indices == e_idx
                if mask.sum() == 0:
                    continue
                expert_input = x_flat[mask].unsqueeze(1)  # (n, 1, D)
                expert_out = self.experts[e_idx](expert_input).squeeze(1)
                output[mask] += expert_weights[mask].unsqueeze(-1) * expert_out

        # Reshape and pool
        output = output.reshape(B, S, D)
        fused = output.mean(dim=1)  # (B, D) global pool
        ...
        for k in range(self.top_k):
            for e_idx in range(self.num_experts):
                expert_counts[e_idx] += (topk_indices[:, k] == e_idx).float().sum()
        expert_frac = expert_counts / (expert_counts.sum() + 1e-8)
        router_mean = router_probs.mean(dim=0)
        load_balance_loss = self.num_experts * (expert_frac * router_mean).sum()

        return fused, load_balance_loss
...
def compute_all_losses(
    ...
    """Compute losses for MambaCode: focal + load_balance + ssm_aux."""
    ...
    balance_loss = outputs.get("balance_loss", torch.tensor(0.0, device=labels.device))
    total_loss = task_loss + config.lambda_balance * balance_loss + config.lambda_ssm_aux * neural_loss + 0.3 * spectral_loss
    return {"total": total_loss, "task": task_loss, "neural": neural_loss, "spectral": spectral_loss, "balance": balance_loss}
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                avg_balance = total_losses.get("balance", 0.0) / num_batches
                lr = self.scheduler.get_last_lr()[0]
                logger.info(
                    f"[Exp24] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"mambacode_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP24] Starting MambaCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP24] MambaCode - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building MambaCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP24] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP24_MambaCode | runs={len(run_plan)}")
