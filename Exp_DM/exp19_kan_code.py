"""
[EXP19] KANCode: Kolmogorov-Arnold Network Classification for AI Code Detection

Core novelty: Replace MLP classification heads with KAN (learnable B-spline activations).
  - KAN layers use learnable activation functions on EDGES (not fixed activations on nodes)
  - Each edge has a B-spline function: y_j = Σ_i φ_{i,j}(x_i)
  - B-splines with configurable grid size and order capture nonlinear decision boundaries
  - Base linear residual prevents gradient vanishing
  - Interpretable: spline shapes reveal which features matter most
  - Applied to neural_head, spectral_head, and gating network

Architecture: SpectralCode backbone + KAN classification heads
  loss = L_focal + 0.3*L_neural_kan + 0.3*L_spectral_kan

Inspired by: KAT (Kolmogorov-Arnold Transformer, ICLR 2025 Oral)
...
class Config:
    ...
    kan_grid_size: int = 5             # KANCode: B-spline grid resolution
    kan_spline_order: int = 3         # KANCode: B-spline order (cubic)
    kan_hidden_dim: int = 64          # KANCode: KAN intermediate dimension
    ...
    save_dir: str = "./kan_code_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
class SpectralCode(nn.Module):
    ...
    def __init__(self, config: Config, num_classes: int):
        ...
        # === KAN Classification Heads (Exp19 novelty) ===
        # Neural KAN classifier
        self.neural_head = KANClassificationHead(
        ...
        # Spectral KAN classifier
        self.spectral_head = KANClassificationHead(
            spectral_dim, num_classes,
            hidden_dim=config.kan_hidden_dim,
            grid_size=config.kan_grid_size,
            spline_order=config.kan_spline_order,
        )

        # === KAN Gate: learns when to trust neural vs spectral ===
        self.gate = KANGate(
    ...
    def forward(
        ...
        gate_weights = self.gate(gate_input)  # (B, 2) — softmax inside KANGate
...
# ============================================================================
# KANCode: Kolmogorov-Arnold Network Components (Exp19 novelty)
# ============================================================================


class KANLayer(nn.Module):
    """
    Kolmogorov-Arnold Network layer with B-spline activation functions.

    Each edge (i→j) has a learnable B-spline function φ_{i,j}(x_i).
    Output: y_j = Σ_i φ_{i,j}(x_i) + base_weight @ x  (residual)

    Key differences from MLP:
    - MLP: fixed activation on nodes, learnable linear on edges
    - KAN: learnable activation on edges, no fixed activation on nodes
    - B-splines provide smooth, localized basis functions

    From: KAT (Kolmogorov-Arnold Transformer), ICLR 2025 Oral.
    """
    ...
    def __init__(
        ...
        # Grid points for B-splines: uniform grid on [-1, 1] extended for boundary
        h = 2.0 / grid_size
        grid = torch.linspace(-1 - spline_order * h, 1 + spline_order * h,
                              grid_size + 2 * spline_order + 1)
        self.register_buffer("grid", grid)

        # Learnable spline coefficients: (in_features, out_features, grid_size + spline_order)
        ...
        # Scale parameter (SiLU activation for base)
    ...
    def b_spline_basis(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: (batch, in_features) values in approximately [-1, 1]
        Returns:
            bases: (batch, in_features, num_basis) B-spline basis values
        """
        x = x.unsqueeze(-1)  # (batch, in_features, 1)
        grid = self.grid      # (num_grid_points,)

        # B-spline order 0: indicator functions
        bases = ((x >= grid[:-1]) & (x < grid[1:])).float()

        # Recursive B-spline computation up to desired order
        ...
        return bases  # (batch, in_features, num_basis)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Normalize input to [-1, 1] range for B-splines
        x_norm = torch.tanh(x)

        # Compute B-spline basis values
        bases = self.b_spline_basis(x_norm)  # (batch, in_features, num_basis)

        # Spline output: contract over in_features and basis
        # spline_weight: (out_features, in_features, num_basis)
        # bases: (batch, in_features, num_basis)
        spline_out = torch.einsum("bin,oin->bo", bases, self.spline_weight)

        # Base linear (with SiLU activation for stability)
        base_out = F.linear(F.silu(x), self.base_weight, self.base_bias)

        # Combine spline + base with learned scale
        # Scale controls how much each input-output pair uses spline vs base
...
class KANClassificationHead(nn.Module):
    """
    Two-layer KAN head for classification.
    Replaces the standard nn.Linear head with KAN layers.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.kan1 = KANLayer(input_dim, hidden_dim, grid_size, spline_order)
        self.norm = nn.LayerNorm(hidden_dim)
        self.kan2 = KANLayer(hidden_dim, num_classes, grid_size, spline_order)
...
class KANGate(nn.Module):
    """KAN-based gating network for neural vs spectral fusion."""

    def __init__(self, input_dim: int, grid_size: int = 5, spline_order: int = 3):
        super().__init__()
        self.kan = KANLayer(input_dim, 2, grid_size, spline_order)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return F.softmax(self.kan(x), dim=-1)
...
def compute_all_losses(
    ...
    """
    Compute all loss components for KANCode.

    Main loss: focal loss on soft-gated fused logits (KAN heads).
    Auxiliary losses: focal on individual KAN head outputs.
    """
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                avg_neural = total_losses.get("neural", 0.0) / num_batches
                lr = self.scheduler.get_last_lr()[0]
                logger.info(
                    f"[Exp19] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
                    f"Loss: {avg_loss:.4f} | Neural: {avg_neural:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"kancode_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP19] Starting KANCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP19] KANCode - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building KANCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP19] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP18_KANCode | runs={len(run_plan)}")
