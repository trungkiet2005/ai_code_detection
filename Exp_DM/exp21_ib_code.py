"""
[EXP21] IBCode: Information Bottleneck for Invariant Code Detection

Core novelty: Variational Information Bottleneck (VIB) compresses representations.
  - VIB forces model to retain ONLY class-relevant information, discarding shortcuts
  - Z ~ N(mu(x), sigma(x)) with KL regularization acts as I(X;Z) minimizer
  - Compression prevents memorizing domain-specific artifacts (variable names, style)
  - Directly targets AICD T1 OOD collapse (val 0.995 -> test 0.29)
  - Beta annealing: start with weak compression, increase over training
  - At test time: use mu (no sampling) for deterministic predictions

Architecture: SpectralCode backbone + VIB compression layer
  loss = L_focal + beta*L_ib_kl + 0.3*L_neural + 0.3*L_spectral

Inspired by: Information Bottleneck (Tishby 2000), VIB (Alemi ICLR 2017),
  IBProtector (NeurIPS 2024)
...
class Config:
    ...
    ib_bottleneck_dim: int = 128       # IBCode: bottleneck dimension
    ib_beta: float = 0.01             # IBCode: initial IB beta (KL weight)
    ib_beta_max: float = 0.1          # IBCode: max beta after annealing
    ib_anneal_epochs: int = 1         # IBCode: anneal beta over first N epochs
    ...
    save_dir: str = "./ibcode_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
class SpectralCode(nn.Module):
    ...
    def __init__(self, config: Config, num_classes: int):
        ...
        # === VIB Compression Layer (Exp21 novelty) ===
        self.vib = VariationalInformationBottleneck(
            fusion_dim, config.ib_bottleneck_dim,
        )
        bottleneck_dim = config.ib_bottleneck_dim

        # === Classification Heads (on compressed representation) ===
        # Neural classifier (from bottleneck)
        self.neural_head = nn.Linear(bottleneck_dim, num_classes)
        ...
        self.gate = nn.Sequential(
            nn.Linear(bottleneck_dim + spectral_dim, 64),
            nn.GELU(),
    ...
    def forward(
        ...
        # === VIB Compression (Exp21) ===
        z_bottleneck, kl_loss = self.vib(h_neural)  # (B, bottleneck_dim), scalar
        ...
        neural_logits = self.neural_head(z_bottleneck)   # (B, num_classes)
        spectral_logits = self.spectral_head(h_spectral)  # (B, num_classes)

        # === Soft Gate ===
        gate_input = torch.cat([z_bottleneck, h_spectral], dim=-1)
...
# ============================================================================
# IBCode: Variational Information Bottleneck (Exp21 novelty)
# ============================================================================


class VariationalInformationBottleneck(nn.Module):
    """
    VIB layer: compresses representation to minimal sufficient statistics.

    Z ~ N(mu(h), sigma(h))
    KL(q(Z|X) || p(Z)) regularizes I(X;Z) — forces compression.
    At test time: Z = mu(h) (deterministic).

    From: Alemi et al. "Deep Variational Information Bottleneck" (ICLR 2017)
    """
    ...
    def forward(self, h: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            z: bottleneck representation (B, bottleneck_dim)
            kl: KL divergence to standard normal (scalar)
        """
        mu = self.mu_head(h)
...
def get_ib_beta(epoch: int, step: int, total_steps_per_epoch: int, config) -> float:
    """Compute annealed IB beta for current training progress."""
    total_anneal_steps = config.ib_anneal_epochs * total_steps_per_epoch
    current_step = epoch * total_steps_per_epoch + step
    if total_anneal_steps <= 0:
        return config.ib_beta_max
    progress = min(1.0, current_step / total_anneal_steps)
    return config.ib_beta + progress * (config.ib_beta_max - config.ib_beta)
...
def compute_all_losses(
    ...
    """
    Compute all loss components for IBCode.

    Main: focal on fused logits (from VIB-compressed representation).
    Aux: IB KL loss (annealed), neural focal, spectral focal.
    """
    ...
    # IBCode: VIB KL loss with annealing
    kl_loss = outputs["kl_loss"]
    ib_beta = get_ib_beta(epoch, step, steps_per_epoch, config)

    total_loss = task_loss + ib_beta * kl_loss + 0.3 * neural_loss + 0.3 * spectral_loss
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            with autocast(device_type=self.config.device, enabled=self.use_amp, dtype=self.amp_dtype):
                ...
                losses = compute_all_losses(self.model, outputs, labels, self.config, self.focal_loss,
                                           epoch=epoch, step=batch_idx, steps_per_epoch=len(self.train_loader))
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                avg_kl = total_losses.get("kl", 0.0) / num_batches
                lr = self.scheduler.get_last_lr()[0]
                logger.info(
                    f"[Exp21] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
                    f"Loss: {avg_loss:.4f} | KL: {avg_kl:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"ibcode_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP21] Starting IBCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP21] IBCode - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building IBCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP21] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP21_IBCode | runs={len(run_plan)}")
