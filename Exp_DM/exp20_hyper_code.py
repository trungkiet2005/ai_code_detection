"""
[EXP20] HyperCode: Hyperbolic Poincaré Embeddings for Hierarchical Code Classification

Core novelty: Embed code representations in Poincaré ball (hyperbolic space).
  - Hyperbolic geometry naturally represents hierarchies with exponential capacity
  - Poincaré distance: d_P(x,y) = arccosh(1 + 2||x-y||^2/((1-||x||^2)(1-||y||^2)))
  - Exponential map projects Euclidean vectors onto Poincaré ball
  - Hyperbolic centroids as class prototypes (Frechet mean approximation)
  - Centering loss: Human class near origin, AI families at periphery
  - Tree-distance regularizer uses FAMILY_TREES for related-family proximity

Architecture: SpectralCode backbone + Poincaré classification head
  loss = L_focal + 0.3*L_hyper_dist + 0.2*L_centering + 0.3*L_spectral

Inspired by: HypStructure (NeurIPS 2024), HELM (NeurIPS 2025)
Baseline: EXP11 SpectralCode | Target: NeurIPS 2026 ORAL
"""
...
class Config:
    ...
    curvature: float = 1.0             # HyperCode: Poincaré ball curvature
    hyper_max_norm: float = 0.95       # HyperCode: max norm on Poincaré ball
    lambda_hyper: float = 0.3          # HyperCode: hyperbolic distance loss weight
    lambda_centering: float = 0.2      # HyperCode: centering loss weight
    ...
    save_dir: str = "./hypercode_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
class SpectralCode(nn.Module):
    ...
    def __init__(self, config: Config, num_classes: int):
        ...
        # === Poincaré Projection (Exp20 novelty) ===
        self.poincare_proj = PoincareLayer(
            fusion_dim, fusion_dim,
            curvature=config.curvature,
            max_norm=config.hyper_max_norm,
        )
        self.hyper_classifier = HyperbolicClassifier(
            fusion_dim, num_classes,
            curvature=config.curvature,
            max_norm=config.hyper_max_norm,
        )
        self.centering_loss_fn = HyperbolicCenteringLoss(curvature=config.curvature)

        # === Classification Heads ===
        # Neural classifier (Euclidean fallback for gated fusion)
    ...
    def forward(
        ...
        # === Poincaré Projection (Exp20) ===
        h_poincare = self.poincare_proj(h_neural)  # (B, fusion_dim) on Poincaré ball
        hyper_logits = self.hyper_classifier(h_poincare)  # (B, num_classes)
        ...
        # === Fused Logits (blend Euclidean gate + hyperbolic) ===
        eucl_logits = gate_weights[:, 0:1] * neural_logits + gate_weights[:, 1:2] * spectral_logits
        logits = 0.5 * eucl_logits + 0.5 * hyper_logits
...
# ============================================================================
# HyperCode: Poincaré Hyperbolic Components (Exp20 novelty)
# ============================================================================


def poincare_distance(x: torch.Tensor, y: torch.Tensor, c: float = 1.0, eps: float = 1e-5) -> torch.Tensor:
    """
    Poincaré ball distance: d(x,y) = (1/sqrt(c)) * arccosh(1 + 2c*||x-y||^2 / ((1-c||x||^2)(1-c||y||^2)))
    """
    x_sqnorm = torch.clamp(torch.sum(x * x, dim=-1), max=1.0 - eps)
    y_sqnorm = torch.clamp(torch.sum(y * y, dim=-1), max=1.0 - eps)
    diff_sqnorm = torch.sum((x - y) ** 2, dim=-1)
    numerator = 2.0 * c * diff_sqnorm
    denominator = (1.0 - c * x_sqnorm) * (1.0 - c * y_sqnorm) + eps
    arg = 1.0 + numerator / denominator
    return torch.acosh(torch.clamp(arg, min=1.0 + eps)) / math.sqrt(c)


def exponential_map(v: torch.Tensor, c: float = 1.0, eps: float = 1e-5) -> torch.Tensor:
    """
    Exponential map at origin: exp_0(v) = tanh(sqrt(c)*||v||/2) * v / (sqrt(c)*||v||)
    Maps Euclidean vector v onto the Poincaré ball.
    """
    sqrt_c = math.sqrt(c)
...
def project_to_ball(x: torch.Tensor, max_norm: float = 0.95, eps: float = 1e-5) -> torch.Tensor:
    """Project points to stay within Poincaré ball of given max norm."""
    norm = torch.norm(x, dim=-1, keepdim=True)
    clamped = torch.clamp(norm, max=max_norm)
    return x * (clamped / (norm + eps))


class PoincareLayer(nn.Module):
    """Projects Euclidean embeddings onto Poincaré ball via exponential map."""

    def __init__(self, input_dim: int, output_dim: int, curvature: float = 1.0, max_norm: float = 0.95):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = self.linear(x)
        h_poincare = exponential_map(h, c=self.curvature)
        return project_to_ball(h_poincare, max_norm=self.max_norm)


class HyperbolicClassifier(nn.Module):
    """Classification via hyperbolic distance to learnable class centroids."""

    def __init__(self, dim: int, num_classes: int, curvature: float = 1.0, max_norm: float = 0.95):
        super().__init__()
        self.curvature = curvature
        self.max_norm = max_norm
        self.num_classes = num_classes
        # Learnable class centroids initialized inside the ball
        init_centroids = torch.randn(num_classes, dim) * 0.01
        self.centroids = nn.Parameter(init_centroids)

    def forward(self, x_poincare: torch.Tensor) -> torch.Tensor:
        """Return logits = -d_P(x, centroid_k) for each class k."""
        centroids = project_to_ball(self.centroids, max_norm=self.max_norm)
        # x_poincare: (B, D), centroids: (C, D)
        # Compute pairwise Poincaré distances
        B = x_poincare.shape[0]
        C = centroids.shape[0]
        x_exp = x_poincare.unsqueeze(1).expand(B, C, -1)
        c_exp = centroids.unsqueeze(0).expand(B, C, -1)
        dists = poincare_distance(x_exp, c_exp, c=self.curvature)  # (B, C)
        return -dists  # negative distance as logits


class HyperbolicCenteringLoss(nn.Module):
    """
    Centering loss: organize class hierarchy in hyperbolic space.
    - Human class (label 0) should be near origin (low norm)
    - AI classes should be at periphery (higher norm)
    - Related families should be closer together
    """
    ...
    def forward(self, x_poincare: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        norms = torch.norm(x_poincare, dim=-1)
        loss = x_poincare.new_zeros(1).squeeze()
        count = 0

        # Human samples (label=0) should have small norm (near origin)
        human_mask = labels == 0
        if human_mask.sum() > 0:
            human_norms = norms[human_mask]
            loss = loss + human_norms.mean()
            count += 1

        # AI samples (label>0) should have larger norm (at periphery)
        ai_mask = labels > 0
        if ai_mask.sum() > 0:
            ai_norms = norms[ai_mask]
            target_norm = 0.7  # target hyperbolic norm for AI classes
            loss = loss + F.relu(target_norm - ai_norms).mean()
            count += 1
...
def compute_all_losses(
    ...
    """
    Compute all loss components for HyperCode.

    Main: focal on fused logits (Euclidean + hyperbolic blend).
    Aux: hyperbolic distance loss + centering loss + spectral focal.
    """
    ...
    # Hyperbolic distance loss: focal on hyperbolic classifier logits
    hyper_dist_loss = focal_loss_fn(outputs["hyper_logits"], labels)

    # Centering loss: organize hierarchy in Poincaré ball
    centering_loss = model.centering_loss_fn(outputs["h_poincare"], labels)

    total_loss = (task_loss
                  + config.lambda_hyper * hyper_dist_loss
                  + config.lambda_centering * centering_loss
                  + 0.3 * spectral_loss)
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                avg_hyper = total_losses.get("hyper_dist", 0.0) / num_batches
                lr = self.scheduler.get_last_lr()[0]
                logger.info(
                    f"[Exp20] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
                    f"Loss: {avg_loss:.4f} | Hyper: {avg_hyper:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"hypercode_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP20] Starting HyperCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP20] HyperCode - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building HyperCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP20] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP20_HyperCode | runs={len(run_plan)}")
