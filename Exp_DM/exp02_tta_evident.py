    ...
    def _tta_adapt_and_predict(self, batch):
        ...
        for _ in range(self.config.tta_steps):
            ...
            with autocast(device_type=self.config.device, enabled=self.use_amp, dtype=self.amp_dtype):
                ...
                # ModernBERT uses tok_embeddings, BERT uses word_embeddings
                embeddings_module = self.model.token_encoder.embeddings
                if hasattr(embeddings_module, 'tok_embeddings'):
                    embed_weight = embeddings_module.tok_embeddings.weight
                elif hasattr(embeddings_module, 'word_embeddings'):
                    embed_weight = embeddings_module.word_embeddings.weight
                else:
                    # Fallback: search for the largest embedding layer
                    embed_weight = next(
                        p for name, p in embeddings_module.named_parameters()
                        if 'embed' in name.lower() and p.dim() == 2
                    )
                mlm_logits = F.linear(hidden, embed_weight)
