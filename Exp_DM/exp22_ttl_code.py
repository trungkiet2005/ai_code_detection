"""
[EXP22] TTLCode: Test-Time LoRA Adaptation for OOD-Robust Code Detection

Core novelty: Adapt model at TEST TIME using LoRA on unlabeled test batches.
  - LoRA (Low-Rank Adaptation): rank-r matrices added to frozen backbone
  - At test time: minimize MLM loss on test batch to adapt LoRA params
  - This shifts the model distribution to match the test domain
  - Directly targets AICD T1 OOD collapse (val 0.995 -> test 0.29)
  - Training: standard SpectralCode with LoRA params trained alongside
  - Test: per-batch TTL adaptation -> classify -> optionally reset

Architecture: SpectralCode + LoRA adapters + MLM test-time head
  Training: L_focal + 0.3*L_neural + 0.3*L_spectral
  Test-time: L_mlm(masked) -> update LoRA -> classify

Inspired by: Test-Time Learning for LLMs (ICML 2025), LoRA (Hu et al. 2022)
...
class Config:
    ...
    lora_rank: int = 4                # TTLCode: LoRA rank
    lora_alpha: int = 16              # TTLCode: LoRA scaling
    ttl_lr: float = 1e-5              # TTLCode: test-time learning rate
    ttl_steps: int = 1                # TTLCode: adaptation steps per test batch
    ttl_mask_prob: float = 0.15       # TTLCode: MLM mask probability
    ttl_enabled: bool = True          # TTLCode: enable test-time adaptation
    ...
    save_dir: str = "./ttlcode_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
# ============================================================================
# TTLCode: LoRA + Test-Time Learning Components (Exp22 novelty)
# ============================================================================
...
class LoRALinear(nn.Module):
    ...
    def __init__(self, original: nn.Linear, rank: int = 4, alpha: int = 16):
        super().__init__()
        self.original = original
        in_f, out_f = original.in_features, original.out_features
        self.lora_A = nn.Parameter(torch.randn(in_f, rank) * (1.0 / math.sqrt(rank)))
...
def inject_lora(model: nn.Module, rank: int = 4, alpha: int = 16):
    """Inject LoRA adapters into query/key/value/dense layers."""
    target_modules = ["query", "key", "value", "dense"]
    replaced = 0
    for name, module in model.named_modules():
        for child_name, child in module.named_children():
            if isinstance(child, nn.Linear) and any(t in child_name.lower() for t in target_modules):
                setattr(module, child_name, LoRALinear(child, rank=rank, alpha=alpha))
                replaced += 1
    logger.info(f"Injected LoRA (rank={rank}, alpha={alpha}) into {replaced} layers")
    return replaced


def get_lora_params(model: nn.Module) -> List[nn.Parameter]:
    """Get only the LoRA parameters for test-time optimization."""
    return [p for n, p in model.named_parameters() if "lora_A" in n or "lora_B" in n]


class MLMHead(nn.Module):
    """Masked Language Model head for test-time adaptation."""
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        h = F.gelu(self.dense(hidden_states))
...
def compute_all_losses(
    ...
    """Compute losses for TTLCode. LoRA adapts at test time (not in train loss)."""
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    f"[Exp22] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"ttlcode_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP22] Starting TTLCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP22] TTLCode - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building TTLCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP22] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP22_TTLCode | runs={len(run_plan)}")
