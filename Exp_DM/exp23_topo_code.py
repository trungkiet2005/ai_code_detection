"""
[EXP23] TopoCode: Topological Persistence Features from AST Graphs

Core novelty: Extract topological features from code AST structure.
  - Simplified persistent homology on AST node sequences
  - Betti numbers (H0=components, H1=cycles) across filtration thresholds
  - AI code produces more regular AST structures (lower topo complexity)
  - Human code has irregular branching -> richer topology
  - TopER-style: evolution rate of graph substructures
  - Lightweight: no external TDA library needed

Architecture: SpectralCode backbone + Topological feature path
  loss = L_focal + 0.3*L_neural + 0.3*L_topo

Inspired by: TopER (NeurIPS 2025), TDA for AI text detection (EMNLP 2021)
...
class Config:
    ...
    topo_num_thresholds: int = 8       # TopoCode: filtration threshold count
    topo_feature_dim: int = 48         # TopoCode: topo feature vector size
    lambda_topo: float = 0.3           # TopoCode: topo path loss weight
    ...
    save_dir: str = "./topocode_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
# ============================================================================
# TopoCode: Topological Persistence Features (Exp23 novelty)
# ============================================================================
...
class UnionFind:
    """Union-Find for counting connected components efficiently."""
    def __init__(self, n):
...
def extract_topo_features_single(ast_seq_np) -> List[float]:
    """Extract topological features from a single AST node sequence."""
    seq = [x for x in ast_seq_np if x > 0]  # remove padding
    n = len(seq)
    if n < 3:
        return [0.0] * TOPO_FEATURE_DIM

    # Get unique node types sorted by frequency (filtration order)
    from collections import Counter
    freq = Counter(seq)
    sorted_types = sorted(freq.keys(), key=lambda t: freq[t], reverse=True)
    num_thresh = min(8, len(sorted_types))
    if num_thresh < 2:
        return [0.0] * TOPO_FEATURE_DIM

    # Filtration: add node types from most to least frequent
    h0_curve = []  # connected components
    h1_curve = []  # estimated cycles
    persistence_lifetimes = []

    for t in range(num_thresh):
        # Include top-(t+1) frequent node types
        allowed = set(sorted_types[:t + 1])
        active = [i for i in range(n) if seq[i] in allowed]
        if len(active) < 2:
            h0_curve.append(len(active))
            h1_curve.append(0)
            continue

        # Build adjacency: connect consecutive active nodes
        uf = UnionFind(len(active))
        edges = 0
        for j in range(1, len(active)):
            if active[j] - active[j-1] <= 2:  # adjacent or near-adjacent
                uf.union(j-1, j)
                edges += 1

        h0 = uf.count
        h1 = max(0, edges - len(active) + h0)  # Euler characteristic
        h0_curve.append(h0)
        h1_curve.append(h1)

    # Pad curves to fixed length
    while len(h0_curve) < 8:
        h0_curve.append(h0_curve[-1] if h0_curve else 0)
    while len(h1_curve) < 8:
        h1_curve.append(h1_curve[-1] if h1_curve else 0)

    h0 = np.array(h0_curve[:8], dtype=np.float32)
    h1 = np.array(h1_curve[:8], dtype=np.float32)
    ...
    for curve in [h0, h1]:
        features.extend([
            ...
            float(np.max(curve)),
            float(np.min(curve)),
            float(curve[-1] - curve[0]),  # total change
            float(np.mean(np.abs(np.diff(curve)))) if len(curve) > 1 else 0.0,  # avg rate
        ])

    # Persistence: how long each component type persists
    for curve in [h0, h1]:
        diffs = np.diff(curve)
        births = np.where(diffs < 0)[0]
        deaths = np.where(diffs > 0)[0]
        if len(births) > 0 and len(deaths) > 0:
            lifetimes = [abs(d - b) for b, d in zip(births[:len(deaths)], deaths[:len(births)])]
            features.extend([float(np.mean(lifetimes)), float(np.max(lifetimes))])
        else:
            features.extend([0.0, 0.0])

    # Complexity scores at multiple scales
    for scale in [2, 4, 6, 8]:
        sub_h0 = h0[:scale]
        sub_h1 = h1[:scale]
        complexity = float(np.std(sub_h0)) + float(np.std(sub_h1))
        features.append(complexity)
        features.append(float(np.sum(np.abs(np.diff(sub_h0)))) + float(np.sum(np.abs(np.diff(sub_h1)))))
        features.append(float(sub_h0.mean() * sub_h1.mean()))

    # Pad/truncate to TOPO_FEATURE_DIM
    features = features[:TOPO_FEATURE_DIM]
    while len(features) < TOPO_FEATURE_DIM:
        features.append(0.0)
    return features


def extract_topo_features(ast_seq: torch.Tensor) -> torch.Tensor:
    """Batch extraction of topological features from AST sequences."""
    batch_size = ast_seq.shape[0]
    all_feats = []
    for i in range(batch_size):
        seq_np = ast_seq[i].cpu().numpy()
        all_feats.append(extract_topo_features_single(seq_np))
    return torch.tensor(np.array(all_feats), dtype=torch.float32, device=ast_seq.device)


class TopologicalEncoder(nn.Module):
    """Encodes topological features into latent representation."""
    def __init__(self, input_dim: int = TOPO_FEATURE_DIM, hidden_dim: int = 128):
        super().__init__()
        self.net = nn.Sequential(
...
def compute_all_losses(
    ...
    """Compute losses for TopoCode: focal + neural + topo."""
    ...
    topo_loss = focal_loss_fn(outputs["spectral_logits"], labels)  # topo head reuses spectral slot
    total_loss = task_loss + 0.3 * neural_loss + config.lambda_topo * topo_loss
    return {"total": total_loss, "task": task_loss, "neural": neural_loss, "topo": topo_loss}
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                avg_topo = total_losses.get("topo", 0.0) / num_batches
                lr = self.scheduler.get_last_lr()[0]
                logger.info(
                    f"[Exp23] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
                    f"Loss: {avg_loss:.4f} | Topo: {avg_topo:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"topocode_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP23] Starting TopoCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP23] TopoCode - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building TopoCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP23] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP23_TopoCode | runs={len(run_plan)}")
