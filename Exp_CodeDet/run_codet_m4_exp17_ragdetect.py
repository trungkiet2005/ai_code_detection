"""
[CoDET-M4] RAGDetect Runner – Full Benchmark Evaluation  (Kaggle standalone)
      Exp17: RAGDetect (Retrieval-Augmented Code Detection)

Core novelty: After standard SpectralCode training, builds a frozen embedding bank
from the training set. At inference, retrieves top-k nearest neighbors by cosine
similarity and softly blends their labels with classifier logits:
    logits_final = (1 - α) * logits_classifier + α * logits_retrieval
    where α = 0.3, k = 32

Benefits:
  - Training-time unchanged (same SpectralCode loss)
  - Test-time: neighbor votes provide distribution-shift robustness
  - OOD generator: novel generator finds similar structural patterns in bank
  - Zero additional parameters at inference (bank is pre-built)

Inspired by: DeTeCtive (NeurIPS 2024, arXiv:2410.20964), kNN-LM, RETRO
Target: ood_generator F1 +5-8% over SpectralCode baseline
...
class SpectralConfig:
    ...
    rag_k: int = 32              # number of nearest neighbors to retrieve
    rag_alpha: float = 0.3       # blend weight for retrieval logits
    rag_bank_subsample: int = 50_000  # subsample train set for bank (memory)
    use_rag: bool = True         # enable RAG at test time
...
class SpectralCode(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, ast_seq, struct_feat, labels=None):
        ...
        return {
            ...
            "embeddings": h_neural,  # for RAG bank
        }
...
# ============================================================================
# Retrieval-Augmented Inference Bank (Exp17 novelty)
# ============================================================================

class EmbeddingBank:
    """
    A frozen embedding bank built from the training set after training.
    Supports cosine-similarity-based top-k retrieval.
    """
    def __init__(self, num_classes: int, k: int = 32, alpha: float = 0.3):
        self.num_classes = num_classes
        self.k = k
        self.alpha = alpha
        self.embeddings: Optional[torch.Tensor] = None  # (N, D) on CPU
        self.labels: Optional[torch.Tensor] = None       # (N,) on CPU
        self.is_built = False

    def build(self, model: nn.Module, dataloader, device: str, max_samples: int = 50_000):
        """Extract embeddings from train set and store in bank."""
        logger.info(f"[RAGDetect] Building embedding bank (max {max_samples} samples)...")
        model.eval()
        all_embs, all_labels = [], []
        count = 0
        ...
        self.is_built = True
        logger.info(f"[RAGDetect] Bank built: {self.embeddings.shape[0]} samples, dim={self.embeddings.shape[1]}")

    @torch.no_grad()
    def retrieve_logits(self, query_embs: torch.Tensor) -> torch.Tensor:
        """
        For a batch of query embeddings, return soft label logits from top-k neighbors.
        Args:
            query_embs: (B, D) L2-normalized embeddings on any device
        Returns:
            (B, num_classes) soft retrieval logits
        """
        if not self.is_built:
            raise RuntimeError("EmbeddingBank.build() must be called before retrieval")

        B = query_embs.shape[0]
        q = F.normalize(query_embs.cpu().float(), p=2, dim=-1)  # (B, D)
        bank = self.embeddings.float()  # (N, D)

        # Cosine similarity: (B, N)
        sims = torch.mm(q, bank.t())

        # Top-k
        topk_sims, topk_idx = sims.topk(min(self.k, bank.shape[0]), dim=-1)  # (B, k)
        topk_labels = self.labels[topk_idx]  # (B, k)

        # Softmax weights over top-k similarities
        weights = F.softmax(topk_sims / 0.1, dim=-1)  # (B, k) temperature=0.1

        # Accumulate weighted votes into class logits
    ...
    def blend_logits(self, classifier_logits: torch.Tensor, query_embs: torch.Tensor) -> torch.Tensor:
        """Blend classifier logits with RAG retrieval logits."""
        rag_logits = self.retrieve_logits(query_embs)
        # Normalize RAG logits to log-scale for stable blending
...
class Trainer:
    ...
    def evaluate(self, dataloader, split_name: str = "Val") -> float:
        ...
        if self.bank is not None and self.bank.is_built and split_name == "Test":
            logger.info(f"[RAGDetect] RAG blending active: k={self.bank.k}, α={self.bank.alpha}")
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"ragdetect_{self.config.task}_{tag}.pt")
...
@torch.no_grad()
def collect_predictions(model: torch.nn.Module, loader: DataLoader, device: str, bank=None) -> Tuple[np.ndarray, np.ndarray]:
    model.eval()
    all_preds, all_labels = [], []
    with torch.no_grad():
        for batch in loader:
            ...
            labels = batch["label"]
            outputs = model(input_ids, attention_mask, ast_seq, struct_feat)
            if bank is not None and bank.is_built:
                logits = bank.blend_logits(outputs["logits"], outputs["embeddings"])
            else:
...
def build_ragdetect_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    if codet_cfg.eval_breakdown:
        ...
        preds, labels = collect_predictions(model, test_loader, exp_cfg.device, bank=trainer.bank)
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_ragdetect_config(tag, cfg.save_root)
