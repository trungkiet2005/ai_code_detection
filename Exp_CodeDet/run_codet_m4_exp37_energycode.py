"""
[CoDET-M4] EnergyCode Runner - Full Benchmark Evaluation  (Kaggle standalone)
      Exp37: EnergyCode (Energy-Based OOD Detection for Code Classification)

Core novelty: Reinterpret classifier as energy-based model for OOD detection.
  - Energy = -log(sum(exp(logits))) as OOD detector
  - Energy margin training with pseudo-OOD samples
  - From: Energy-Based OOD Detection (ICLR 2020/2025)
...
# ============================================================================
# EMA Teacher + Self-Distillation Loss (Exp37 novelty)
# ============================================================================
...
class EMATeacher:
    ...
    def __init__(self, student: nn.Module, momentum: float = 0.995):
        ...
        logger.info(f"[Exp37] EMATeacher initialized: momentum={momentum}")
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp37] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"energycodecode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp37] Starting EnergyCode Training - {self.config.task}")
...
def build_energycode_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_energycode_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp37][IID] EnergyCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_energycode_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp37] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp37_EnergyCode_CoDET_M4", "results": safe},
