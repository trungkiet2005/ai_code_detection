"""
[CoDET-M4] IBCode Runner - Full Benchmark Evaluation  (Kaggle standalone)
      Exp33: IBCode (Information Bottleneck for Invariant Code Detection)

Core novelty: VIB compresses representations to discard shortcuts.
  - Z ~ N(mu(x), sigma(x)) with KL regularization
  - Forces model to retain only class-relevant information
  - Beta annealing for training stability
  - From: VIB (Alemi ICLR 2017), IBProtector (NeurIPS 2024)
...
class SpectralConfig:
    ...
    # IBCode hyperparams
    ib_bottleneck_dim: int = 128
    ib_beta: float = 0.01
    ib_beta_max: float = 0.1
    ib_anneal_epochs: int = 1
...
# ============================================================================
# EMA Teacher + Self-Distillation Loss (Exp33 novelty)
# ============================================================================
...
class EMATeacher:
    ...
    def __init__(self, student: nn.Module, momentum: float = 0.995):
        ...
        logger.info(f"[Exp33] EMATeacher initialized: momentum={momentum}")
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp33] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"ibcodecode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp33] Starting IBCode Training - {self.config.task}")
...
def build_ibcode_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_ibcode_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp33][IID] IBCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_ibcode_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp33] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp33_IBCode_CoDET_M4", "results": safe},
