"""
[CoDET-M4] KANCode Runner - Full Benchmark Evaluation  (Kaggle standalone)
      Exp31: KANCode (Kolmogorov-Arnold Network Classification Heads)

Core novelty: Replace MLP classification heads with KAN (learnable B-spline activations).
  - KAN layers use learnable activation functions on EDGES
  - B-splines with configurable grid size capture nonlinear decision boundaries
...
# ============================================================================
# EMA Teacher + Self-Distillation Loss (Exp31 novelty)
# ============================================================================
...
class EMATeacher:
    ...
    def __init__(self, student: nn.Module, momentum: float = 0.995):
        ...
        logger.info(f"[Exp31] EMATeacher initialized: momentum={momentum}")
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp31] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"kancodecode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp31] Starting KANCode Training - {self.config.task}")
...
def build_kancode_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_kancode_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp31][IID] KANCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_kancode_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp31] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp31_KANCode_CoDET_M4", "results": safe},
