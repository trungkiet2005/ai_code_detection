"""
[CoDET-M4] TTACode Runner – Full Benchmark Evaluation  (Kaggle standalone)
      Exp22: TTACode (Test-Time Adaptation via Entropy Minimization)

Core novelty: At inference time, the model adapts its normalization layer statistics
  by minimizing the entropy of its own predictions on each test batch (TENT-style).
  - Training: identical to SpectralCode baseline (no extra loss)
  - Test time: for each batch, update only LayerNorm/norm params via entropy min
  - 1-3 gradient steps per batch before final prediction
  - Naturally improves OOD robustness without requiring labeled test data

Architecture:
  Train: SpectralCode (identical to Exp11)
  Test:  TTAAdapter wraps model → updates norm params via ∇entropy → predict

Inspired by:
  - TENT: Fully Test-Time Adaptation (ICLR 2021, Wang et al.)
  - SAR: Sharpness-Aware and Reliable Entropy Minimization (ICLR 2023)
  - TTT++: Self-supervised Test-Time Training (NeurIPS 2021)
  - DomainAdapter: Source-free domain adaptation for code analysis (ICML 2025)

Target: OOD generator/source F1 +5-10% | Author IID F1 70%+ with adaptation
...
class SpectralConfig:
    ...
    # TTA hyperparams
    tta_steps: int = 1          # gradient steps per test batch
    tta_lr: float = 1e-4        # learning rate for norm layer updates
    tta_entropy_threshold: float = 0.4 * math.log(6)  # skip high-entropy samples (SAR)
...
class SpectralCode(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, ast_seq, struct_feat, labels=None):
        ...
        return {
            ...
            "embeddings": h_neural,  # for TTA loss
        }
...
# ============================================================================
# Test-Time Adaptation (Exp22 novelty)
# ============================================================================

class TTAAdapter:
    """
    TENT-style Test-Time Adaptation.
    At test time, updates only LayerNorm parameters by minimizing prediction entropy.
    Inspired by TENT (Wang et al., ICLR 2021) and SAR (Niu et al., ICLR 2023).
    """
    def __init__(self, model: nn.Module, tta_lr: float = 1e-4, tta_steps: int = 1,
                 entropy_threshold: float = float("inf")):
        self.model = model
        self.tta_steps = tta_steps
        self.entropy_threshold = entropy_threshold
        # Only adapt normalization layer parameters (affine scale + bias)
        norm_params = []
        ...
        self.optimizer = torch.optim.Adam(norm_params, lr=tta_lr)
        logger.info(f"[Exp22] TTAAdapter: {len(norm_params)} norm params, steps={tta_steps}, lr={tta_lr}")

    def adapt_and_predict(self, input_ids, attention_mask, ast_seq, struct_feat):
        """Runs tta_steps entropy-minimization steps, then returns final logits."""
        self.model.train()  # enable norm layer updates
        for _ in range(self.tta_steps):
            self.optimizer.zero_grad()
            with torch.enable_grad():
                out = self.model(input_ids, attention_mask, ast_seq, struct_feat)
                logits = out["logits"]
                probs = F.softmax(logits, dim=-1)
                entropy = -(probs * (probs + 1e-8).log()).sum(-1)  # (B,)
                # SAR: filter out high-entropy samples to avoid noisy gradients
...
def compute_all_losses(model, outputs, labels, config, focal_loss_fn=None):
    """Training loss: identical to Exp11 SpectralCode baseline (no extra loss at train time).
    TTA happens only at test time via TTAAdapter."""
    ...
    return {"total": total_loss, "task": task_loss, "neural": neural_loss, "spectral": spectral_loss, "aux": torch.zeros(1).squeeze()}
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp22] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"ttacode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp22] Starting TTACode Training - {self.config.task}")
...
def collect_predictions(model: torch.nn.Module, loader: DataLoader, device: str,
                        tta_adapter: Optional["TTAAdapter"] = None) -> Tuple[np.ndarray, np.ndarray]:
    """Standard prediction. If tta_adapter provided, adapts model per batch before predicting."""
    ...
    for batch in loader:
        ...
        if tta_adapter is not None:
            logits = tta_adapter.adapt_and_predict(input_ids, attention_mask, ast_seq, struct_feat)
        else:
...
def build_tta_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_tta_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp22][IID] TTACode | task={task}")
    ...
    if codet_cfg.eval_breakdown:
        logger.info("-" * 50)
        logger.info(f"[Exp22][BREAKDOWN][TTA] task={task} | tta_steps={exp_cfg.tta_steps}")
        # Create TTAAdapter for test-time adaptation
        tta = TTAAdapter(model, tta_lr=exp_cfg.tta_lr, tta_steps=exp_cfg.tta_steps,
                         entropy_threshold=getattr(exp_cfg, "tta_entropy_threshold",
                                                   0.4 * math.log(max(num_classes, 2))))
        preds, labels = collect_predictions(model, test_loader, exp_cfg.device, tta_adapter=tta)
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_tta_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp22] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp22_TTACode_CoDET_M4", "results": safe},
