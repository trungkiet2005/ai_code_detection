"""
[CoDET-M4] TopoCode Runner - Full Benchmark Evaluation  (Kaggle standalone)
      Exp35: TopoCode (Topological Persistence Features from AST Graphs)

Core novelty: Persistent homology features from AST structure.
  - Betti numbers across filtration thresholds
  - AI code = more regular topology, human = richer
  - From: TopER (NeurIPS 2025), TDA for text detection (EMNLP 2021)
...
class SpectralConfig:
    ...
    # TopoCode hyperparams
    topo_num_thresholds: int = 8
    topo_feature_dim: int = 48
...
# ============================================================================
# EMA Teacher + Self-Distillation Loss (Exp35 novelty)
# ============================================================================
...
class EMATeacher:
    ...
    def __init__(self, student: nn.Module, momentum: float = 0.995):
        ...
        logger.info(f"[Exp35] EMATeacher initialized: momentum={momentum}")
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp35] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"topocodecode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp35] Starting TopoCode Training - {self.config.task}")
...
def build_topocode_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_topocode_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp35][IID] TopoCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_topocode_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp35] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp35_TopoCode_CoDET_M4", "results": safe},
