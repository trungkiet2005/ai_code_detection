"""
[CoDET-M4] MoECode Runner – Full Benchmark Evaluation  (Kaggle standalone)
      Exp21: MoECode (Sparse Mixture-of-Experts for Code Authorship Detection)

Core novelty: Sparse Mixture-of-Experts (MoE) router over the neural fusion representation.
  - 4 independent expert FFNs, each specializing in different code style dimensions
  - Top-2 sparse routing: only 2 experts activated per sample (compute efficient)
  - Load-balancing auxiliary loss prevents expert collapse
  - Expert specialization emerges: syntax, semantics, style, frequency experts
  - loss = L_focal + 0.3*L_neural + 0.3*L_spectral + λ_moe*L_load_balance

Architecture:
  h_neural → Router → top-2 expert weights → weighted sum of expert outputs
  Expert FFN: Linear(D, 2D) → GELU → Linear(2D, D) → Dropout  [residual added]

Inspired by:
  - Switch Transformer (NeurIPS 2022), Mixtral (Mistral AI 2023)
  - Soft MoE (NeurIPS 2023), MoEBERT (ACL 2022)
  - Recent code analysis: MoE for multi-language code understanding (ICML 2025)

Target: Author IID Macro-F1 72%+ | OOD robustness via expert specialization
...
class SpectralConfig:
    ...
    num_experts: int = 4
    moe_top_k: int = 2
    lambda_moe: float = 0.1          # load balance loss weight
...
class SpectralCode(nn.Module):
    def __init__(self, config: SpectralConfig, num_classes: int):
        ...
        # Sparse MoE: 4 experts over fusion representation
        self.moe_router = nn.Linear(fusion_dim, config.num_experts)
        self.moe_experts = nn.ModuleList([
            nn.Sequential(
                nn.Linear(fusion_dim, fusion_dim * 2), nn.GELU(),
                nn.Linear(fusion_dim * 2, fusion_dim), nn.Dropout(0.1),
            )
    ...
    def forward(self, input_ids, attention_mask, ast_seq, struct_feat, labels=None):
        ...
        # ── Sparse MoE routing ────────────────────────────────────────────────
        router_logits = self.moe_router(h_neural)                          # (B, E)
        router_weights = F.softmax(router_logits, dim=-1)                  # (B, E)
        top_k_w, top_k_i = router_weights.topk(self.config.moe_top_k, dim=-1)  # (B, K)
        top_k_w = top_k_w / top_k_w.sum(-1, keepdim=True)                 # renorm
        h_moe = torch.zeros_like(h_neural)
        ...
        return {
            ...
            "embeddings": h_neural,
            "router_logits": router_logits,  # for load-balance loss
        }
...
# ============================================================================
# MoE Load-Balance Loss (Exp21 novelty)
# ============================================================================

def compute_moe_load_balance_loss(router_logits: torch.Tensor) -> torch.Tensor:
    """
    Auxiliary load-balance loss to prevent expert collapse.
    Encourages uniform utilization across experts via MSE on mean routing probabilities.
    Inspired by Switch Transformer (NeurIPS 2022): f_i * P_i auxiliary loss.
    """
    router_probs = F.softmax(router_logits, dim=-1)          # (B, num_experts)
    mean_load = router_probs.mean(0)                          # (num_experts,)
    target_load = torch.full_like(mean_load, 1.0 / mean_load.shape[0])
    return F.mse_loss(mean_load, target_load)
...
def compute_all_losses(model, outputs, labels, config, focal_loss_fn=None):
    ...
    # MoE load-balance loss (prevents expert collapse)
    moe_loss = compute_moe_load_balance_loss(outputs["router_logits"])
    lambda_moe = getattr(config, "lambda_moe", 0.1)
    total_loss = task_loss + 0.3 * neural_loss + 0.3 * spectral_loss + lambda_moe * moe_loss
    return {"total": total_loss, "task": task_loss, "neural": neural_loss, "spectral": spectral_loss, "moe": moe_loss}
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                avg_moe = total_losses.get("moe", 0.0) / num_batches
                logger.info(
                    f"[Exp21] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
                    f"Loss: {avg_loss:.4f} | MoE_lb: {avg_moe:.4f} | LR: {lr:.2e}"
                )
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp21] Starting MoECode Training - {self.config.task}")
...
def build_moe_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_moe_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp21][IID] MoECode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_moe_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp21] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp21_MoECode_CoDET_M4", "results": safe},
