"""
[CoDET-M4] SelfDistillCode Runner – Full Benchmark Evaluation  (Kaggle standalone)
      Exp26: SelfDistillCode (BYOL-Style EMA Self-Distillation for Code Detection)

Core novelty: An online encoder (student) is guided by an exponential moving average
  (EMA) teacher (its own past self). The student learns to predict the teacher's
  representation of the same sample → self-distillation without labels.
  - Teacher: EMA copy of student (momentum=0.995, updated each step)
  - Student loss: task_loss + λ_distill * cosine_distill(student_emb, teacher_emb.detach())
  - Teacher provides more stable, calibrated targets than one-hot labels
  - No collapse: teacher never gets gradients → bootstrapping prevents trivial solution

Architecture:
  student: SpectralCode (trained with gradients)
  teacher: EMA(student) (no gradients, updated after each optimizer step)
  distill_loss: 1 - cosine_sim(student_emb, teacher_emb.stop_gradient)

Inspired by:
  - BYOL: Bootstrap Your Own Latent (NeurIPS 2020, Grill et al.)
  - DINO: Self-supervised ViT features (ICCV 2021, Caron et al.)
  - EMA Teacher for semi-supervised learning (Mean Teacher, NeurIPS 2017)
  - SelfDistillation for NLP: Born-Again Networks (ICML 2018)
  - EMA-based code representation learning (EMNLP 2024)

Target: Author IID 72%+ | OOD robustness via stable representation targets
...
class SpectralConfig:
    ...
    # SelfDistill hyperparams
    distill_momentum: float = 0.995     # EMA momentum for teacher update
    lambda_distill: float = 0.3         # weight for self-distillation loss
...
# ============================================================================
# EMA Teacher + Self-Distillation Loss (Exp26 novelty)
# ============================================================================
...
class EMATeacher:
    ...
    def __init__(self, student: nn.Module, momentum: float = 0.995):
        ...
        logger.info(f"[Exp26] EMATeacher initialized: momentum={momentum}")
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp26] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"selfdistillcode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp26] Starting SelfDistillCode Training - {self.config.task}")
...
def build_selfdistill_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_selfdistill_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp26][IID] SelfDistillCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_selfdistill_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp26] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp26_SelfDistillCode_CoDET_M4", "results": safe},
