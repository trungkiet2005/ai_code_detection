"""
[CoDET-M4] GraphStyleCode Runner – Full Benchmark Evaluation  (Kaggle standalone)
      Exp23: GraphStyleCode (Graph Attention Network over Code AST for Stylometry)

Core novelty: Replaces BiLSTM AST encoder with a Graph Attention Network (GAT).
  - AST node sequence → multi-head self-attention (graph-style neighborhood aggregation)
  - GraphSAGE-style concat: [node_feat || aggregated_neighbor_feat] → projection
  - 2-layer stacked GAT with residual connections
  - Captures non-sequential structural dependencies (AST parent-child, sibling patterns)
  - Graph-level readout: attention-weighted global pooling

Architecture:
  AST token IDs → node embeddings → 2-layer GraphAttention → global pool → style vector
  (replaces BiLSTM in SpectralCode baseline)

Inspired by:
  - GAT: Graph Attention Networks (ICLR 2018, Veličković et al.)
  - Code2Vec / Code2Seq (POPL 2019, ASE 2020): AST paths for code representation
  - GraphCodeBERT (ICLR 2021): data flow graph in pre-training
  - CCAG: Cross-code AST Graph (CVPR 2024 workshop on structured prediction)
  - GNN-based stylometry: Authorship via code graphs (AAAI 2025)

Target: Author IID Macro-F1 72%+ | Better GH source robustness via structural features
...
class SpectralConfig:
    ...
    # GraphStyle hyperparams
    gat_heads: int = 4              # multi-head attention in GAT layers
    gat_layers: int = 2             # stacked GAT depth
...
class GATLayer(nn.Module):
    ...
    Treats AST node sequence as a fully-connected graph; multi-head attention
    computes neighbor aggregation (GraphSAGE-style: concat self+agg → project).
    """
    def __init__(self, embed_dim: int, num_heads: int, dropout: float = 0.1):
        super().__init__()
        self.attn = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)
        self.sage_proj = nn.Linear(embed_dim * 2, embed_dim)
        self.norm = nn.LayerNorm(embed_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        # x: (B, N, D)
        agg, _ = self.attn(x, x, x, key_padding_mask=key_padding_mask)  # neighborhood aggregation
        h = self.sage_proj(torch.cat([x, agg], dim=-1))                  # GraphSAGE concat+proj
        return self.norm(x + self.dropout(h))                             # residual + norm


class ASTEncoder(nn.Module):
    """
    Graph Attention-based AST Encoder (replaces BiLSTM for Exp23).
    Stacks 2 GAT layers, then uses attention-weighted global pooling.
    """
    def __init__(self, vocab_size: int, embed_dim: int, hidden_dim: int,
        ...
        self.gat_layers = nn.ModuleList([
            GATLayer(hidden_dim, num_heads, dropout) for _ in range(num_layers)
        ])
    ...
    def forward(self, ast_seq: torch.Tensor) -> torch.Tensor:
        pad_mask = (ast_seq == 0)                                          # (B, N) True=padding
        x = self.input_proj(self.embedding(ast_seq))                       # (B, N, H)
        for layer in self.gat_layers:
            x = layer(x, key_padding_mask=pad_mask)
        # Attention-weighted pooling (ignore padding positions)
        attn_scores = self.pool_attn(x).squeeze(-1)                        # (B, N)
...
class SpectralCode(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, ast_seq, struct_feat, labels=None):
        ...
        return {
            ...
            "embeddings": h_neural,  # for GraphStyle loss
        }
...
# ============================================================================
# GraphStyle: standard loss (novelty is in the GAT encoder, not the loss)
# ============================================================================

def compute_all_losses(model, outputs, labels, config, focal_loss_fn=None):
    """Standard SpectralCode loss — novelty is in the GraphAttention AST encoder."""
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp23] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"graphstylecode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp23] Starting GraphStyleCode Training - {self.config.task}")
...
def build_graphstyle_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_graphstyle_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp23][IID] GraphStyleCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_graphstyle_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp23] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp23_GraphStyleCode_CoDET_M4", "results": safe},
