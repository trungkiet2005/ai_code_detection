"""
[CoDET-M4] CosineProtoCode Runner – Full Benchmark Evaluation  (Kaggle standalone)
      Exp24: CosineProtoCode (Dynamic Cosine Prototype Classification with EMA Updates)

Core novelty: Replaces the linear classification head with a dynamic cosine prototype head.
  - Per-class prototypes live on the unit sphere; classification = cosine similarity
  - Learnable per-class temperature scaling (allows each class to control its margin)
  - EMA prototype updates each training batch: prototype_c ← m*proto + (1-m)*batch_mean
  - Hyperspherical uniformity regularization: maximize geodesic distance between prototypes
  - This directly addresses Nxcode/Qwen collapse: prototypes forced to separate on sphere

Architecture:
  h_neural → L2-normalize → cosine_sim(prototypes) × log_temp → logits
  Prototypes: (num_classes, D), L2-normalized, EMA-updated, orthogonal init

Inspired by:
  - Hyperspherical Uniformity (NeurIPS 2018, Li et al.)
  - CLIP contrastive learning: cosine classifier as temperature-scaled dot product
  - ProtoNet (NeurIPS 2017): prototype-based few-shot learning
  - Dynamic Prototype Networks (ICLR 2023): EMA prototypes for class imbalance
  - CosFace / ArcFace (CVPR 2018/2019): margin-based cosine classification for recognition
  - Feature Normalization for Code Authorship (AAAI 2025)

Target: Author IID Macro-F1 74%+ | Nxcode/Qwen separation via spherical geometry
...
class SpectralConfig:
    ...
    # CosineProto hyperparams
    proto_init_temp: float = 10.0       # initial temperature (log scale)
    proto_momentum: float = 0.9         # EMA momentum for prototype update
    lambda_unif: float = 0.05           # hyperspherical uniformity weight
...
class SpectralCode(nn.Module):
    def __init__(self, config: SpectralConfig, num_classes: int):
        ...
        # Cosine prototype head: per-class prototypes on unit sphere
        self.neural_head = CosineProtoHead(fusion_dim, num_classes,
                                            init_temp=config.proto_init_temp)
    ...
    def forward(self, input_ids, attention_mask, ast_seq, struct_feat, labels=None):
        ...
        return {
            ...
            "embeddings": h_neural,  # for CosinePro loss
        }
...
# ============================================================================
# CosineProtoCode components (Exp24 novelty)
# ============================================================================

class CosineProtoHead(nn.Module):
    """
    Cosine Prototype Classification Head.
    Classifies by cosine similarity to learnable class prototype vectors.
    Prototypes are L2-normalized; a learnable log-temperature scales the similarities.
    Inspired by CosFace/ArcFace (CVPR 2018/2019) and CLIP zero-shot classification.
    """
    def __init__(self, embed_dim: int, num_classes: int, init_temp: float = 10.0):
        super().__init__()
        self.prototypes = nn.Parameter(torch.empty(num_classes, embed_dim))
        nn.init.orthogonal_(self.prototypes)  # maximize initial separation
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sim = torch.mm(x_norm, proto_norm.t())                            # (B, C)
        temp = self.log_temp.exp().clamp(1.0, 100.0)
        return sim * temp

    @torch.no_grad()
    def ema_update(self, embeddings: torch.Tensor, labels: torch.Tensor, momentum: float = 0.9):
        """Update prototypes via EMA of batch class centroids."""
        for c in labels.unique():
            mask = (labels == c)
            if mask.sum() == 0:
                continue
            batch_mean = F.normalize(embeddings[mask].mean(0), p=2, dim=-1)
            c_idx = c.item()
            updated = momentum * self.prototypes[c_idx] + (1 - momentum) * batch_mean
            self.prototypes[c_idx] = F.normalize(updated, p=2, dim=-1)


def hyperspherical_uniformity_loss(prototypes: torch.Tensor) -> torch.Tensor:
    """
    Hyperspherical Uniformity Loss (Li et al., NeurIPS 2018).
    Maximizes pairwise geodesic distances between prototype vectors.
    Acts as regularizer to prevent prototype collapse.
    """
    proto_norm = F.normalize(prototypes, p=2, dim=-1)                     # (C, D)
    cos_sim = torch.mm(proto_norm, proto_norm.t())                        # (C, C)
    # Exclude diagonal (self-similarity = 1)
    mask = ~torch.eye(cos_sim.shape[0], dtype=torch.bool, device=cos_sim.device)
    return cos_sim[mask].mean()                                           # minimize → push apart
...
def compute_all_losses(model, outputs, labels, config, focal_loss_fn=None):
    ...
    if hasattr(model, "neural_head") and isinstance(model.neural_head, CosineProtoHead):
        ...
        unif_loss = torch.zeros(1, device=outputs["logits"].device).squeeze()
    lambda_unif = getattr(config, "lambda_unif", 0.05)
    total_loss = task_loss + 0.3 * neural_loss + 0.3 * spectral_loss + lambda_unif * unif_loss
    return {"total": total_loss, "task": task_loss, "neural": neural_loss, "spectral": spectral_loss, "aux": unif_loss}
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp24] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"cosineprot_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp24] Starting CosineProtoCode Training - {self.config.task}")
...
def build_cosineproto_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_cosineproto_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp24][IID] CosineProtoCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_cosineproto_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp24] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp24_CosineProtoCode_CoDET_M4", "results": safe},
