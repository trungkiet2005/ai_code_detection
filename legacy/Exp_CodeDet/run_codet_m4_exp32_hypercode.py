"""
[CoDET-M4] HyperCode Runner - Full Benchmark Evaluation  (Kaggle standalone)
      Exp32: HyperCode (Hyperbolic Poincare Embeddings for Hierarchical Code Classification)

Core novelty: Embed code in Poincare ball for hierarchy-aware classification.
  - Poincare distance as classification metric
  - Exponential map projects to hyperbolic space
  - Centering loss organizes class hierarchy
  - From: HypStructure (NeurIPS 2024), HELM (NeurIPS 2025)

Architecture: SpectralCode backbone + Poincare classification head
Target: Author IID 72%+ | NeurIPS 2026 ORAL
"""
...
class SpectralConfig:
    ...
    # HyperCode hyperparams
    curvature: float = 1.0
    hyper_max_norm: float = 0.95
    lambda_hyper: float = 0.3
    lambda_centering: float = 0.2
...
# ============================================================================
# EMA Teacher + Self-Distillation Loss (Exp32 novelty)
# ============================================================================
...
class EMATeacher:
    ...
    def __init__(self, student: nn.Module, momentum: float = 0.995):
        ...
        logger.info(f"[Exp32] EMATeacher initialized: momentum={momentum}")
...
class Trainer:
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    ...
                    f"[Exp32] Loss: {avg_loss:.4f} | Aux: {avg_aux:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"hypercodecode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp32] Starting HyperCode Training - {self.config.task}")
...
def build_hypercode_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_hypercode_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp32][IID] HyperCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_hypercode_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp32] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp32_HyperCode_CoDET_M4", "results": safe},
