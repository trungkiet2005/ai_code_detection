"""
[EXP26] WaveCLCode: Discrete Wavelet Transform + Class-Aware Frequency Selection

Core novelty: DWT (wavelet) analysis on token sequences for class-aware features.
  - Discrete Wavelet Transform decomposes signal into multi-resolution components
  - Approximation (low-freq) + detail (high-freq) coefficients at each level
  - Class-Aware Frequency Selection: learn which frequency bands matter per class
  - Human vs AI code differ in frequency characteristics at multiple scales
  - Wavelets capture both frequency AND time localization (unlike FFT)

Architecture: SpectralCode backbone + Wavelet feature path
  loss = L_focal + 0.3*L_neural + 0.3*L_wavelet

Inspired by: CLFD (NeurIPS 2024), SpecDetect (AAAI 2025)
...
class Config:
    ...
    wavelet_levels: int = 4            # WaveCLCode: DWT decomposition levels
    wavelet_feature_dim: int = 64      # WaveCLCode: wavelet feature vector size
    lambda_wavelet: float = 0.3        # WaveCLCode: wavelet path loss weight
    ...
    save_dir: str = "./waveclcode_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
# ============================================================================
# WaveCLCode: Discrete Wavelet Transform Components (Exp26 novelty)
# ============================================================================

WAVELET_FEATURE_DIM = 64


def haar_dwt_1d(signal: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Single-level Haar DWT: returns (approximation, detail) coefficients."""
    n = len(signal)
    if n % 2 != 0:
        signal = np.append(signal, signal[-1])
        n += 1
    approx = (signal[0::2] + signal[1::2]) / math.sqrt(2)
    detail = (signal[0::2] - signal[1::2]) / math.sqrt(2)
    return approx, detail


def multi_level_dwt(signal: np.ndarray, levels: int = 4) -> List[np.ndarray]:
...
def extract_wavelet_features_single(token_ids: np.ndarray, mask: np.ndarray, levels: int = 4) -> List[float]:
    """Extract wavelet features from a single token ID sequence."""
    ids = token_ids[mask.astype(bool)].astype(np.float32)
    if len(ids) < 8:
        return [0.0] * WAVELET_FEATURE_DIM
    ...
    coeffs = multi_level_dwt(ids, levels=levels)
    ...
    for coeff in coeffs:
        ...
        features.extend([
            energy,
            float(np.mean(np.abs(coeff))),
            float(np.std(coeff)),
            float(np.max(np.abs(coeff))),
            float(np.median(np.abs(coeff))),
            float(np.sum(np.abs(coeff) > np.std(coeff))),  # significant coeffs
            float(np.mean(coeff ** 2)),  # mean power
            float(np.percentile(np.abs(coeff), 75)),  # 75th percentile
        ])

    # Cross-level features
    if len(coeffs) >= 2:
        for i in range(min(3, len(coeffs) - 1)):
            e_this = np.sum(coeffs[i] ** 2) + 1e-10
            e_next = np.sum(coeffs[min(i+1, len(coeffs)-1)] ** 2) + 1e-10
            features.append(float(e_this / e_next))  # energy ratio
            features.append(float(np.log(e_this / e_next + 1e-10)))  # log ratio

    # Pad/truncate
    features = features[:WAVELET_FEATURE_DIM]
    while len(features) < WAVELET_FEATURE_DIM:
        features.append(0.0)
    return features


def extract_wavelet_features(input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    """Batch extraction of wavelet features."""
    batch_size = input_ids.shape[0]
    all_feats = []
    for i in range(batch_size):
        ids_np = input_ids[i].cpu().numpy()
        mask_np = attention_mask[i].cpu().numpy()
        all_feats.append(extract_wavelet_features_single(ids_np, mask_np))
    return torch.tensor(np.array(all_feats), dtype=torch.float32, device=input_ids.device)


class WaveletEncoder(nn.Module):
    """Encodes wavelet features with class-aware gating."""
    def __init__(self, input_dim: int = WAVELET_FEATURE_DIM, hidden_dim: int = 128, num_classes: int = 2):
        super().__init__()
        ...
        # Class-aware frequency gate: learn which freq bands matter per class
        self.class_gate = nn.Linear(hidden_dim, hidden_dim)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = self.encoder(x)
        gate = torch.sigmoid(self.class_gate(h))
        return h * gate
...
def compute_all_losses(
    ...
    """Compute losses for WaveCLCode: focal + neural + wavelet."""
    ...
    wavelet_loss = focal_loss_fn(outputs["spectral_logits"], labels)  # wavelet head reuses spectral slot
    total_loss = task_loss + 0.3 * neural_loss + config.lambda_wavelet * wavelet_loss
    return {"total": total_loss, "task": task_loss, "neural": neural_loss, "wavelet": wavelet_loss}
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                avg_wavelet = total_losses.get("wavelet", 0.0) / num_batches
                lr = self.scheduler.get_last_lr()[0]
                logger.info(
                    f"[Exp26] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
                    f"Loss: {avg_loss:.4f} | Wavelet: {avg_wavelet:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"waveclcode_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP26] Starting WaveCLCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP26] WaveCLCode - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building WaveCLCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP26] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP26_WaveCLCode | runs={len(run_plan)}")
