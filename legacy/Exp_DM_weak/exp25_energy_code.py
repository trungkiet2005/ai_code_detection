"""
[EXP25] EnergyCode: Energy-Based OOD Detection for Code Classification

Core novelty: Reinterpret the classifier as an energy-based model for OOD detection.
  - Energy score = -log(sum(exp(logits))) is a principled OOD detector
  - Low energy = in-distribution, high energy = OOD
  - Energy margin loss: push ID samples to low energy, create energy gap
  - At test time: flag high-energy samples for abstention or fallback
  - Lightweight: no extra parameters, just reinterprets existing logits
  - Addresses AICD T1 OOD collapse by knowing when to say "I don't know"

Architecture: SpectralCode backbone + Energy margin training
  loss = L_focal + lambda_energy*L_energy_margin + 0.3*L_neural + 0.3*L_spectral

Inspired by: Energy-Based OOD Detection (ICLR 2020/2025), Liu et al.
...
class Config:
    ...
    lambda_energy: float = 0.1         # EnergyCode: energy margin loss weight
    energy_margin_in: float = -25.0    # EnergyCode: target energy for ID samples
    energy_margin_out: float = -7.0    # EnergyCode: target energy for pseudo-OOD
    ...
    save_dir: str = "./energycode_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
# ============================================================================
# EnergyCode: Energy-Based OOD Components (Exp25 novelty)
# ============================================================================
...
class EnergyMarginLoss(nn.Module):
    """
    Energy margin loss: push in-distribution energy down and create a gap.
    Uses the training labels to define ID samples.
    Creates pseudo-OOD by perturbing features with noise.
    """

    def __init__(self, margin_in: float = -25.0, margin_out: float = -7.0):
        super().__init__()
        self.margin_in = margin_in
        self.margin_out = margin_out

    def forward(self, logits_id: torch.Tensor, logits_ood: Optional[torch.Tensor] = None) -> torch.Tensor:
        energy_id = compute_energy(logits_id)
        loss_in = F.relu(energy_id - self.margin_in).pow(2).mean()

        if logits_ood is not None:
            energy_ood = compute_energy(logits_ood)
            loss_out = F.relu(self.margin_out - energy_ood).pow(2).mean()
            return loss_in + loss_out

        return loss_in


def create_pseudo_ood(embeddings: torch.Tensor, noise_scale: float = 0.5) -> torch.Tensor:
    """Create pseudo-OOD samples by adding Gaussian noise to embeddings."""
...
def compute_all_losses(
    ...
    """Compute losses for EnergyCode: focal + energy margin + spectral."""
    ...
    # Energy margin loss on ID logits
    energy_fn = EnergyMarginLoss(margin_in=config.energy_margin_in, margin_out=config.energy_margin_out)

    # Create pseudo-OOD by noising embeddings and getting logits
    pseudo_ood_emb = create_pseudo_ood(outputs["embeddings"].detach())
    pseudo_ood_logits = model.neural_head(pseudo_ood_emb)
    energy_loss = energy_fn(outputs["logits"], pseudo_ood_logits)

    total_loss = task_loss + config.lambda_energy * energy_loss + 0.3 * neural_loss + 0.3 * spectral_loss
    return {"total": total_loss, "task": task_loss, "neural": neural_loss, "spectral": spectral_loss, "energy": energy_loss}
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                avg_energy = total_losses.get("energy", 0.0) / num_batches
                lr = self.scheduler.get_last_lr()[0]
                logger.info(
                    f"[Exp25] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
                    f"Loss: {avg_loss:.4f} | Energy: {avg_energy:.4f} | LR: {lr:.2e}"
                )
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"energycode_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP25] Starting EnergyCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP25] EnergyCode - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building EnergyCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP25] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP25_EnergyCode | runs={len(run_plan)}")
