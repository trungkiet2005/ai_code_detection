"""
[EXP18] HierTreeCode: Hierarchical Stylometric Detection with Family-Aware Constraints

Core novelty: Hierarchical Affinity Loss models LLM genealogy as a family tree.
  - Fine-tuned models share families (e.g., Nxcode from Qwen, CodeLlama from Llama)
  - Triplet loss: within-family distance + margin < cross-family distance
  - Cosine distance on L2-normalized embeddings
  - Auto-adapts family tree per benchmark/task:
    * AICD T2 (12-class): Google (Gemma+Gemini), Chinese AI (DeepSeek+Qwen+01-ai)
    * DROID T3/T4: Human → Refined → Machine hierarchy
    * Binary tasks (T1): hier loss disabled

Architecture: SpectralCode backbone + HierarchicalAffinityLoss
  loss = L_focal + 0.3*L_neural + 0.3*L_spectral + lambda_hier*L_hier

Inspired by: DETree (arXiv:2510.17489), CosFace/ArcFace (CVPR 2018/2019)
...
class SpectralCode(nn.Module):
    ...
    def forward(
        ...
        return {
            ...
            "embeddings": h_neural,  # for HierarchicalAffinityLoss
        }
...
def compute_all_losses(
    ...
    """
    Compute all loss components for HierTreeCode.

    Main loss: focal loss on soft-gated fused logits.
    Auxiliary losses: focal + hierarchical affinity loss.
    """
    ...
    # HierTreeCode: hierarchical affinity loss
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % self.config.log_every == 0:
                ...
                logger.info(
                    f"[Exp18] Epoch {epoch+1} | Step {batch_idx+1}/{len(self.train_loader)} | "
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"hiertree_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP18] Starting HierTreeCode Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    # Model
    logger.info(f"Building HierTreeCode model...")
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP18] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    try:
        from legacy.Exp_DM._paper_table import emit_paper_table
        emit_paper_table(
