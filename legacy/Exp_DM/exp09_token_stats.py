"""
[EXP09] TokenStat: Statistical Token Distribution + Neural Hybrid Detector for
OOD-Robust AI-Generated Code Detection

Key Innovation:
- Token-level statistical features: entropy, burstiness, repetition rate,
  type-token ratio — capture LLM decoding artifacts
- Dual-head architecture: neural (ModernBERT) + statistical (MLP on stats)
- Soft-gated fusion learns when to trust neural vs statistical signal
- Statistical features are domain-agnostic by construction
- Addresses AICD paper finding: TF-IDF (43%) >> neural (28%) on T1 OOD

Loss: L = L_focal (on gated fusion output)

Reference: Combines AICD paper TF-IDF insight with neural representation
...
Usage on Kaggle:
    ...
    3. Execute the cells or run: python exp09_token_stats.py --task T1
"""
...
class Config:
    ...
    save_dir: str = "./token_stat_checkpoints"
    log_every: int = 100
    eval_every: int = 1000
...
# ============================================================================
# Token Statistics Extraction (domain-agnostic LLM artifact features)
# ============================================================================

NUM_TOKEN_STATS = 12


def extract_token_statistics(input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    """
    Extract statistical features from tokenized code.
    These features capture LLM decoding artifacts (low entropy, repetition, burstiness).

    Returns: (B, NUM_TOKEN_STATS) tensor
    """
    ...
    for i in range(batch_size):
        ...
        if seq_len < 2:
            features.append(np.zeros(NUM_TOKEN_STATS))
            continue

        # 1. Type-token ratio (vocabulary diversity)
        unique_tokens = len(set(ids))
        ttr = unique_tokens / seq_len

        # 2. Token frequency distribution statistics
        freq = Counter(ids)
        freq_values = np.array(list(freq.values()), dtype=float)
        freq_mean = freq_values.mean()
        freq_std = freq_values.std()
        freq_max = freq_values.max()
        freq_skew = float(np.mean(((freq_values - freq_mean) / (freq_std + 1e-8)) ** 3))

        # 3. Token-level entropy
        probs = freq_values / freq_values.sum()
        entropy = -np.sum(probs * np.log2(probs + 1e-10))
        normalized_entropy = entropy / (np.log2(seq_len) + 1e-10)

        # 4. Repetition rate (consecutive repeated tokens)
        repeats = sum(1 for j in range(1, len(ids)) if ids[j] == ids[j - 1])
        repeat_rate = repeats / (seq_len - 1)

        # 5. Burstiness (variance in inter-arrival times of common tokens)
        top_token = freq.most_common(1)[0][0]
        positions = [j for j, t in enumerate(ids) if t == top_token]
        if len(positions) > 1:
            gaps = np.diff(positions).astype(float)
            burstiness = gaps.std() / (gaps.mean() + 1e-8)
        else:
            burstiness = 0.0

        # 6. Hapax legomena ratio (tokens appearing exactly once)
        hapax = sum(1 for v in freq_values if v == 1)
        hapax_ratio = hapax / unique_tokens if unique_tokens > 0 else 0

        # 7. Yule's K (vocabulary richness measure)
        N = seq_len
        freq_of_freq = Counter(freq.values())
        M2 = sum(i * i * vi for i, vi in freq_of_freq.items())
        yule_k = 10000 * (M2 - N) / (N * N + 1e-8) if N > 0 else 0

        features.append([
            ttr, freq_mean, freq_std, freq_max, freq_skew,
            entropy, normalized_entropy,
            repeat_rate, burstiness, hapax_ratio, yule_k,
            float(seq_len),
        ])
...
# ============================================================================
# Full Model: TokenStat
# ============================================================================

class TokenStat(nn.Module):
    """
    Dual-head detector: neural features + token statistics, soft-gated fusion.

    Neural path: ModernBERT + AST + structural features -> cross-attention fusion
    Statistical path: token distribution stats + structural features -> MLP
    ...
    def __init__(self, config: Config, num_classes: int):
        ...
        self.stat_encoder = nn.Sequential(
            nn.Linear(NUM_TOKEN_STATS + STRUCTURAL_FEATURE_DIM, 256),
            nn.LayerNorm(256),
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(256, 128),
    ...
    def forward(
        ...
        # === Statistical Path ===
        token_stats = extract_token_statistics(input_ids, attention_mask)  # (B, NUM_TOKEN_STATS)
        stat_input = torch.cat([token_stats, struct_feat], dim=-1)  # (B, NUM_TOKEN_STATS + STRUCTURAL_FEATURE_DIM)
...
def compute_all_losses(
    ...
    """
    Compute all loss components for TokenStat.
...
class Trainer:
    def __init__(self, config: Config, model: TokenStat, train_loader, val_loader, test_loader, class_weights=None):
        self.config = config
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"tokenstat_{self.config.task}_{tag}.pt")
        torch.save({
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[EXP09] Starting TokenStat Training - Task {self.config.task}")
...
def main(task: str = "T1", config: Optional[Config] = None):
    ...
    logger.info(f"{'='*60}")
    logger.info(f"[EXP09] TokenStat - AI-Generated Code Detection")
    ...
    # Model
    logger.info(f"Building TokenStat model...")
    model = TokenStat(config, num_classes)
...
def run_benchmark_suite(run_plan: List[Tuple[str, str]], base_config: Optional[Config] = None):
    ...
    for bench, bench_task in run_plan:
        ...
        logger.info("\n" + "=" * 70)
        logger.info(f"[EXP09] BENCHMARK SUITE RUN: {bench.upper()} | task={bench_task}")
        logger.info("=" * 70)
    ...
    logger.info("\n=== SUITE_RESULTS_START ===")
    logger.info(f"timestamp={ts} | method=EXP09_TokenStat | runs={len(run_plan)}")
