"""
[EXP11] SpectralCode: Unified Runner for ALL 3 Benchmarks
    AICD-Bench  |  DroidCollection  |  CoDET-M4

Architecture: ModernBERT + AST BiLSTM + 22 Structural Features + FFT Spectral
              -> CrossAttentionFusion -> Dual heads with learned soft gate
Loss: L_focal(gate) + 0.3 * L_focal(neural) + 0.3 * L_focal(spectral)

Supported RUN_MODE values:
  "full"        - All 3 benchmarks (AICD T1/T2/T3 + Droid T3/T4 + CoDET IID+OOD)
  "aicd_only"   - AICD-Bench tasks only (T1, T2, T3)
  "droid_only"  - DroidCollection tasks only (T3, T4)
  "codet_only"  - CoDET-M4 suite only (IID binary/author + OOD LOO)
  "single"      - One specific (benchmark, task) pair
...
class Config:
    # ---- Benchmark selection ----
    benchmark: str = "aicd"       # "aicd" | "droid" | "codet_m4"
    task: str = "T1"              # AICD: T1/T2/T3 | Droid: T1/T3/T4 | CoDET: binary/author
    ...
    # ---- CoDET-M4-specific ----
    codet_dataset_id: str = "DaniilOr/CoDET-M4"
    codet_code_field_priority: Tuple[str, ...] = ("cleaned_code", "code")
    codet_split_field: str = "split"
    codet_eval_breakdown: bool = True
...
# ============================================================================
# Structural Feature Extraction (22 features)
# ============================================================================
...
# ============================================================================
# Spectral Feature Extraction (FFT, 64-dim)
# ============================================================================
...
class CodeDataset(TorchDataset):
    """Universal dataset for all 3 benchmarks. Expects {code, label} dicts."""
...
class Trainer:
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"spectralcode_{self.config.benchmark}_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp11] SpectralCode | {self.config.benchmark.upper()} | task={self.config.task}")
...
def _sample_dataset(dataset: Dataset, max_samples: int, seed: int = 42) -> Dataset:
...
def _make_loaders(train_data, val_data, test_data, tokenizer, config: Config):
    train_ds = CodeDataset(train_data, tokenizer, config.max_length, config.ast_seq_len)
    val_ds = CodeDataset(val_data, tokenizer, config.max_length, config.ast_seq_len)
    test_ds = CodeDataset(test_data, tokenizer, config.max_length, config.ast_seq_len)
...
def load_aicd_data(config: Config):
    ...
    train_data = _sample_dataset(ds["train"], config.max_train_samples, config.seed)
    val_data = _sample_dataset(ds["validation"], config.max_val_samples, config.seed + 1)
    test_data = _sample_dataset(ds["test"], config.max_test_samples, config.seed + 2)
...
def load_droid_data(config: Config):
    if not _supports_task_for_benchmark("droid", config.task):
        raise ValueError(f"Task {config.task} unsupported for Droid. Use T1/T3/T4.")
    ...
    def _convert_split(split_name: str, max_samples: int):
        ...
        ds = _sample_dataset(ds, max_samples, config.seed)

        def _convert_row(row):
            return {
                "code": row.get("Code", "") or "",
                "label": _map_droid_label_to_task(row, config.task),
            }
...
def _convert_codet_split(split_ds: Dataset, config: Config, author_vocab: Dict[str, int]) -> Dataset:
    def _convert_row(row):
        code = _extract_code(row, config.codet_code_field_priority)
        if config.task == "binary":
            label = _map_binary_label(row)
        elif config.task == "author":
            label = _map_author_label(row, author_vocab)
        else:
            raise ValueError(f"Unsupported CoDET task: {config.task}")
        target = _normalize_target(row.get("target", ""))
        return {
            "code": code,
            "label": label,
...
def _load_codet_raw_splits(config: Config):
    logger.info(f"Loading dataset: {config.codet_dataset_id}")
    ds = load_dataset(config.codet_dataset_id, split="train")

    has_split = config.codet_split_field in ds.column_names
    if has_split:
        logger.info("Using built-in split column from CoDET-M4")
        train_raw = ds.filter(lambda x: str(x.get(config.codet_split_field, "")).lower() == "train")
        val_raw = ds.filter(lambda x: str(x.get(config.codet_split_field, "")).lower() in {"val", "validation", "dev"})
        test_raw = ds.filter(lambda x: str(x.get(config.codet_split_field, "")).lower() == "test")
    else:
        logger.warning("No split column; creating 80/10/10 with seed=%d", config.seed)
...
def load_codet_m4_data(config: Config):
    train_raw, val_raw, test_raw = _load_codet_raw_splits(config)

    author_vocab = _build_author_vocab(train_raw) if config.task == "author" else {}
    if config.task == "author":
        logger.info("Author vocab (%d generators): %s", len(author_vocab), sorted(author_vocab.keys()))

    train_data = _convert_codet_split(train_raw, config, author_vocab)
    val_data = _convert_codet_split(val_raw, config, author_vocab)
    test_data = _convert_codet_split(test_raw, config, author_vocab)
...
def load_codet_m4_loo(config: Config, hold_out_field: str, hold_out_value: str):
    train_raw, val_raw, test_raw = _load_codet_raw_splits(config)
    ...
    train_data = _convert_codet_split(train_in, config, author_vocab)
    val_data = _convert_codet_split(val_in, config, author_vocab)
    test_data = _convert_codet_split(test_ood, config, author_vocab)
...
# ============================================================================
# Breakdown Evaluation (CoDET-M4: per-language, per-source, per-generator)
# ============================================================================
...
def preflight_single(config: Config) -> Dict[str, object]:
    logger.info("\n" + "=" * 70)
    logger.info(f"PREFLIGHT | benchmark={config.benchmark} | task={config.task}")
    logger.info("=" * 70)
    ...
    if not _supports_task_for_benchmark(config.benchmark, config.task):
        raise PreflightError(f"Unsupported pair: benchmark={config.benchmark}, task={config.task}")
    ...
    elif config.benchmark == "codet_m4":
        train_data, val_data, test_data, num_classes = load_codet_m4_data(config)
    else:
        raise PreflightError(f"Unknown benchmark: {config.benchmark}")
    ...
    enc = tokenizer(probe_code, max_length=config.max_length, padding="max_length", truncation=True, return_tensors="pt")
    ...
    report = {
        "benchmark": config.benchmark, "task": config.task,
        ...
        "device": config.device, "gpu": _get_gpu_name(),
    }
    logger.info(f"PREFLIGHT OK | {config.benchmark.upper()} | classes={num_classes} | sizes={report['sizes']}")
    return report


def preflight_suite(run_plan: List[Tuple[str, str]], base_config: Config):
    reports = {}
    for bench_mode, task in run_plan:
        bench = bench_mode.replace("_ood", "").replace("codet_m4", "codet_m4")
        if bench_mode.endswith("_ood"):
            bench = "codet_m4"
            # OOD preflight: just check IID binary loads
            cfg = Config(**vars(base_config))
            cfg.benchmark = bench
            cfg.task = "binary"
        else:
            cfg = Config(**vars(base_config))
            cfg.benchmark = bench_mode
            cfg.task = task
        cfg = apply_hardware_profile(cfg)
        key = f"{bench_mode}_{task}"
        reports[key] = preflight_single(cfg)
    logger.info("\n=== PREFLIGHT COMPLETE: %d checks passed ===", len(reports))
    return reports
...
def _run_single_train(config: Config) -> Dict[str, Any]:
    """Core training routine shared by all benchmarks."""
    set_seed(config.seed)

    logger.info("=" * 70)
    logger.info(f"[Exp11] SpectralCode | {config.benchmark.upper()} | task={config.task}")
    logger.info(f"GPU={_get_gpu_name()} | precision={config.precision} | batch={config.batch_size}x{config.grad_accum_steps} | epochs={config.epochs}")
    logger.info("=" * 70)
    ...
    # CoDET-M4 breakdown eval
    if config.benchmark == "codet_m4" and config.codet_eval_breakdown:
        logger.info("-" * 50)
        logger.info(f"[BREAKDOWN] task={config.task}")
        preds, labels = collect_predictions(model, test_loader, config.device)
        results["breakdown"] = run_breakdown_eval(preds, labels, test_data, config.task)

    return results


def run_aicd_or_droid(config: Config) -> Dict[str, Any]:
    return _run_single_train(config)


def run_codet_iid(task: str, config: Config) -> Dict[str, Any]:
    cfg = Config(**vars(config))
    cfg.benchmark = "codet_m4"
    cfg.task = task
    cfg.save_dir = os.path.join(config.save_dir, f"codet_m4_{task}")
    cfg = apply_hardware_profile(cfg)
    return _run_single_train(cfg)


# ============================================================================
# CoDET-M4 OOD Leave-One-Out
# ============================================================================

def _run_single_loo(hold_out_field: str, hold_out_value: str, config: Config) -> Dict[str, Any]:
    cfg = Config(**vars(config))
    cfg.benchmark = "codet_m4"
    cfg.task = "binary"
    cfg.save_dir = os.path.join(config.save_dir, f"codet_ood_{hold_out_field}_{hold_out_value}")
    cfg = apply_hardware_profile(cfg)
    set_seed(cfg.seed)
    ...
    model = SpectralCode(cfg, num_classes)
    trainer = Trainer(cfg, model, train_loader, val_loader, test_loader, class_weights)
    results = trainer.train()

    preds, labels = collect_predictions(model, test_loader, cfg.device)
...
def run_ood_generator(config: Config) -> Dict[str, Dict]:
    generators = ["codellama", "gpt", "llama3.1", "nxcode", "qwen1.5"]
    results = {}
    for gen in generators:
        logger.info(f"\n{'='*70}\n[OOD-GEN] Holding out generator={gen}\n{'='*70}")
        try:
            results[gen] = _run_single_loo("generator", gen, config)
...
def run_ood_language(config: Config) -> Dict[str, Dict]:
    languages = ["cpp", "java", "python"]
    results = {}
    for lang in languages:
        logger.info(f"\n{'='*70}\n[OOD-LANG] Holding out language={lang}\n{'='*70}")
        try:
            results[lang] = _run_single_loo("language", lang, config)
        except RuntimeError as e:
...
def run_ood_source(config: Config) -> Dict[str, Dict]:
    sources = ["cf", "gh", "lc"]
    results = {}
    for src in sources:
        logger.info(f"\n{'='*70}\n[OOD-SRC] Holding out source={src}\n{'='*70}")
        try:
            results[src] = _run_single_loo("source", src, config)
        except RuntimeError as e:
...
FULL_RUN_PLAN = [
    ...
    # CoDET-M4 IID
    ("codet_m4", "binary"),     # IID binary (Table 2)
    ("codet_m4", "author"),     # IID authorship (Table 7)
    # CoDET-M4 OOD Leave-One-Out
    ("codet_m4_ood", "generator"),   # proxy Table 8
    ("codet_m4_ood", "language"),    # proxy Table 10/12
    ("codet_m4_ood", "source"),      # proxy Table 9
]


def run_suite(run_plan: List[Tuple[str, str]], base_config: Config) -> Dict[str, Any]:
    all_results: Dict[str, Any] = {}
    original_save_dir = base_config.save_dir

    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp11] UNIFIED BENCHMARK SUITE: {len(run_plan)} evaluation modes")
    ...
    for mode, task in run_plan:
        ...
        if mode in ("aicd", "droid"):
            cfg = Config(**vars(base_config))
            cfg.benchmark = mode
            cfg.task = task
            cfg.save_dir = os.path.join(original_save_dir, f"{mode}_{task}")
            cfg = apply_hardware_profile(cfg)
            all_results[key] = run_aicd_or_droid(cfg)

        elif mode == "codet_m4":
            all_results[key] = run_codet_iid(task, base_config)

        elif mode == "codet_m4_ood":
            if task == "generator":
                all_results[key] = run_ood_generator(base_config)
            elif task == "language":
                all_results[key] = run_ood_language(base_config)
            elif task == "source":
                all_results[key] = run_ood_source(base_config)
            else:
                logger.warning(f"Unknown OOD task: {task}")
        else:
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"UNIFIED SUITE COMPLETE | {ts}")
    logger.info("=" * 70)

    # AICD + DROID IID
    logger.info("\n=== AICD + DROID RESULTS ===")
    for mode, task in run_plan:
        if mode not in ("aicd", "droid"):
            continue
        key = f"{mode}_{task}"
        r = all_results.get(key, {})
        logger.info(
            f"  {mode.upper():>8s} {task}: macro_f1={r.get('test_f1', 0):.4f}  "
            f"weighted_f1={r.get('test_weighted_f1', 0):.4f}  "
            f"val_f1={r.get('best_val_f1', 0):.4f}"
        )

    # CoDET IID
    logger.info("\n=== CoDET-M4 IID RESULTS ===")
    for mode, task in run_plan:
        if mode != "codet_m4":
            continue
        key = f"codet_m4_{task}"
        r = all_results.get(key, {})
        logger.info(
    ...
    # CoDET OOD
    for mode, task in run_plan:
        if mode != "codet_m4_ood":
            continue
        key = f"codet_m4_ood_{task}"
        ood_block = all_results.get(key, {})
        if not isinstance(ood_block, dict):
            continue
        logger.info(f"\n=== CoDET-M4 OOD {task.upper()} ===")
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp11_SpectralCode_Unified", "results": safe},
            ensure_ascii=True, default=str,
        ))
    except (TypeError, ValueError):
        logger.info("(JSON serialization skipped)")
...
if __name__ == "__main__":
    # ── Run mode ──────────────────────────────────────────────────────
    # "full"        -> All 3 benchmarks (10 entries)
    # "aicd_only"   -> AICD T1/T2/T3
    # "droid_only"  -> Droid T3/T4
    # "codet_only"  -> CoDET-M4 IID + OOD
    # "single"      -> One (BENCHMARK, TASK) pair
    RUN_MODE = "full"
    RUN_PREFLIGHT_CHECK = True

    # Only for RUN_MODE == "single":
    BENCHMARK = "aicd"          # "aicd" | "droid" | "codet_m4"
    TASK = "T1"                 # depends on benchmark

    # ── Task semantics ────────────────────────────────────────────────
    # AICD:    T1 (2-cls) | T2 (12-cls) | T3 (4-cls)
    # Droid:   T3 (3-cls) | T4 (4-cls)
    # CoDET:   binary (2-cls) | author (6-cls)
    # ──────────────────────────────────────────────────────────────────
    ...
    if RUN_MODE == "full":
        plan = FULL_RUN_PLAN
    elif RUN_MODE == "aicd_only":
        plan = [e for e in FULL_RUN_PLAN if e[0] == "aicd"]
    elif RUN_MODE == "droid_only":
        plan = [e for e in FULL_RUN_PLAN if e[0] == "droid"]
    elif RUN_MODE == "codet_only":
        plan = [e for e in FULL_RUN_PLAN if e[0].startswith("codet")]
