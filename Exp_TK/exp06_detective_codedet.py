"""
[CoDET-M4] DeTeCtiveCode Runner – Full Benchmark Evaluation  (Kaggle standalone)
      Exp27: DeTeCtiveCode (Multi-Level SupCon + HierTree + kNN blend)

Builds on **Exp18 HierTreeCode** (SOTA Author F1 on CoDET-M4) and adds:
  1. **Multi-level supervised contrastive learning** (neural path + spectral path),
     inspired by DeTeCtive (NeurIPS 2024), SupCon (Khosla et al., NeurIPS 2020),
     and multi-view metric learning (CVPR/ICCV line).
  2. **HierarchicalAffinityLoss** unchanged from Exp18 (generator family tree).
  3. **Test-time kNN retrieval blend** (Exp17-style embedding bank; optional),
     for distribution-shift robustness (VLDB/KDD-style retrieval augmentation).

Loss: L_focal + 0.3*L_neural + 0.3*L_spectral + λ_hier*L_hier + λ_supcon*(L_sup_n + L_sup_s)/2

Cross-bench: pair with `Exp_DM/exp27_detective_code.py` for AICD + Droid (same method).
"""
...
def supcon_loss(z: torch.Tensor, labels: torch.Tensor, temperature: float = 0.07) -> torch.Tensor:
    """Supervised contrastive (Khosla et al., NeurIPS 2020). Multi-level variant in DeTeCtiveCode."""
    if z.shape[0] < 2:
        return z.new_zeros(())
...
# ============================================================================
# Retrieval bank (same protocol as Exp17 RAGDetect; optional at test)
# ============================================================================
...
class EmbeddingBank:
    ...
    def build(self, model: nn.Module, dataloader, device: str, max_samples: int = 50_000):
        ...
        self.is_built = True
        logger.info("[Exp27] Bank: %d samples, dim=%d", self.embeddings.shape[0], self.embeddings.shape[1])
...
def compute_all_losses(model, outputs, labels, config, focal_loss_fn=None):
    ...
    return {
        "total": total_loss, "task": task_loss, "neural": neural_loss, "spectral": spectral_loss,
        "hier": hier_loss, "supcon": sup_mean, "sup_neural": sup_n, "sup_spec": sup_s,
    }
...
class Trainer:
    ...
    def evaluate(self, dataloader, split_name: str = "Val") -> float:
        ...
        if self.bank is not None and self.bank.is_built and split_name == "Test":
            logger.info("[Exp27] kNN blend active: k=%s α=%s", self.bank.k, self.bank.alpha)
    ...
    def save_checkpoint(self, tag: str = "latest"):
        ...
        path = os.path.join(self.config.save_dir, f"detectivcode_{self.config.task}_{tag}.pt")
    ...
    def train(self):
        logger.info("=" * 60)
        logger.info(f"[Exp27] Starting DeTeCtiveCode Training - {self.config.task}")
...
def build_detective_config(task_tag: str, save_root: str) -> SpectralConfig:
...
def run_iid(task: str = "binary", codet_cfg: Optional[CoDETM4Config] = None, run_preflight: bool = True) -> Dict[str, Any]:
    ...
    exp_cfg = build_detective_config(f"CoDET_{task}", codet_cfg.save_root)

    logger.info("=" * 70)
    logger.info(f"[Exp27][IID] DeTeCtiveCode | task={task}")
...
def _run_single_loo(hold_out_field: str, hold_out_value: str, codet_cfg: CoDETM4Config) -> Dict[str, Any]:
    ...
    exp_cfg = build_detective_config(tag, cfg.save_root)
...
def run_suite(
    ...
    logger.info("\n" + "=" * 70)
    logger.info(f"[Exp27] CoDET-M4 FULL BENCHMARK SUITE: {len(run_plan)} evaluation modes")
...
def _log_final_summary(all_results: Dict[str, Any], run_plan: List[Tuple[str, str]]):
    ...
    try:
        logger.info("\nSUITE_RESULTS_JSON=" + json.dumps(
            {"timestamp": ts, "method": "Exp27_DeTeCtiveCode_CoDET_M4", "results": safe},
