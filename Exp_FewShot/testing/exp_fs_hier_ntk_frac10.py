"""
exp_fs_inline_hier_ntk.py -- ONE-FILE self-contained inline for Kaggle T4.

Method: FS-Hier-NTK (HierTree family prior + NTK target-kernel alignment combo (most likely to beat NTKAlign 5%=0.665))
...
Output: /kaggle/working/results/exp_fs_inline_hier_ntk_<label>_seed<S>.json
"""
...
logger = logging.getLogger('exp_fs_inline_hier_ntk')
...
def parse_sweep_configs():
    ...
    raw_ks = os.environ.get("FS_SWEEP_KS", "").strip()
    raw_fr = os.environ.get("FS_SWEEP_FRACS", "0.10").strip()
    out = []
    if raw_ks:
...
METHODS = {
    ...
    'hier_ntk'      : ('FS-Hier-NTK', 'exp_fs_inline_hier_ntk', 'hier_ntk', False),
