...
logger = logging.getLogger("exp_n09_pac_bayes_floor")
...
def pac_bayes_loss(outputs, labels, lambda_psf=0.1, n_classes=6,
                    class_weights=None):
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = pac_bayes_loss(out, y, lambda_psf=lambda_method, n_classes=cfg.n_classes, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_LAMBDA_PSF", "0.1"))
