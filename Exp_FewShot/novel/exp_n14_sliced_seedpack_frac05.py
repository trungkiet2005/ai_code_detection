...
logger = logging.getLogger("exp_n14_sliced_wasserstein")
...
def sliced_wasserstein_loss(outputs, labels, lambda_swc=0.1, n_slices=64,
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = sliced_wasserstein_loss(out, y, lambda_swc=lambda_method, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_LAMBDA_SWC", "0.1"))
