...
logger = logging.getLogger("exp_n04_etf_simplex")
...
def build_etf(n_classes: int, dim: int, device, dtype=torch.float32):
...
def train(cfg, model, train_l, val_l, test_l, lambda_method):
    ...
    # Pre-compute frozen ETF on the training device (used by all batches).
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = etf_cosine_loss(out, y, etf_w, scale=16.0, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_ETF_SCALE", "16.0"))
