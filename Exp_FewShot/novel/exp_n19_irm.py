...
logger = logging.getLogger("exp_n19_irm")
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = irm_loss(out, y, b["sources"], lambda_irm=lambda_method, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_LAMBDA_IRM", "1.0"))
