...
logger = logging.getLogger("exp_n13_tree_wasserstein")
...
# Genealogy tree edge weights. 6 leaves -> 5 internal -> root.
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = tree_wasserstein_loss(out, y, lambda_twg=lambda_method, n_classes=cfg.n_classes, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_LAMBDA_TWG", "0.4"))
