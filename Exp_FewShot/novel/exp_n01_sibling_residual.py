# =============================================================================
# Novel-Track exp n01 -- Sibling-Residual Discriminant (SRD).
#
# Open problem this attacks: at K=32, codellama (class 1) and nxcode (class 4)
# -- direct fine-tune siblings -- both have F1 in [0.00, 0.04] across all 14
# methods we ran (CE, NTK, SupCon, Frozen, and combinations). Cross-entropy
# never receives a signal that separates them: their conditional distributions
# overlap in the unprojected feature space. The "sibling block" is the
# dominant error mass in the confusion matrix at every data budget below 5%.
#
# Single new mathematical object: a per-batch family-conditional residual
# scatter ratio, computed only over codellama-vs-nxcode pairs in the
# residual subspace orthogonal to the family centroid. Used as a closed-form
# auxiliary loss whose gradient targets the sibling axis directly.
#
# NAME           : SRD (Sibling-Residual Discriminant).
# ONE-LINE CLAIM : Under the hierarchical generative model
#                  z_iv = mu_F(i) + r_iv,  r_iv perp mu_F,
#                  the sibling-pair signal lives ENTIRELY in r_iv (the
#                  family residual). Maximising the Fisher ratio
#                  Var_between(siblings) / Var_within(siblings) on r_iv --
#                  and ONLY on r_iv -- produces a gradient that is orthogonal
#                  to the family-mean direction and so cannot harm
#                  cross-family separability. Predicted to lift K=32 sibling
#                  F1 from <0.05 to >0.20 without degrading other classes.
# EQUATION       : Backbone -> z = ntk_proj(emb), L2-normalised.
#                  Family map  F: codellama=A, nxcode=A, qwen=B, llama3.1=C,
#                              gpt=D, human=H. (codellama-nxcode is the only
#                              non-trivial family with multiple children.)
#                  For each sample with class y in {codellama, nxcode}:
#                      mu_A = mean of all in-batch z with F(y) = A
#                      r_i = z_i - mu_A          (family-residual, orthogonal
#                                                  to mu_A by construction
#                                                  after one Gram-Schmidt step)
#                      r_i_perp = r_i - <r_i, mu_A> * mu_A / ||mu_A||^2
#                  Per-class residual mean:
#                      m_codellama = mean of r_i_perp where y=codellama
#                      m_nxcode    = mean of r_i_perp where y=nxcode
#                  Within-class residual scatter (sum of per-class variances):
#                      S_W = sum_{c in {codellama, nxcode}}
#                              (1/N_c) sum_{i: y_i=c} ||r_i_perp - m_c||^2
#                  Between-class residual scatter:
#                      S_B = ||m_codellama - m_nxcode||^2
#                  Loss term (maximise Fisher = minimise -Fisher):
#                      L_SRD = -lambda * S_B / (S_W + eps)
#                  Total loss:
#                      L = CE(logits, y) + lambda_srd * L_SRD
#
# THEORY HOOK    : Hierarchical Neural Collapse (Galanti and Poggio,
#                  arXiv:2501.09211, 2025): for hierarchically-labelled data
#                  the optimal feature geometry is an equiangular tight frame
#                  (ETF) WITHIN each parent simplex, and sibling children
#                  are orthogonal in the parent's tangent space. Our object
#                  is the empirical estimator of "tangent-space orthogonality
#                  margin" for the codellama-nxcode pair: maximise the
#                  Fisher ratio in the residual subspace == push the
#                  empirical ETF towards the optimal one.
# WHY NOT BEFORE : The CoDET-M4 6-class authorship task has exactly one
#                  non-trivial fine-tune family (codellama -> nxcode) and the
#                  errors are concentrated there. Standard hierarchical losses
#                  (Khosla SupCon, Galanti's ETF projection) operate on the
#                  full embedding; they receive the family signal and the
#                  residual signal mixed together at small K, where the
#                  family signal dominates the gradient. Restricting the
#                  Fisher-ratio loss TO THE RESIDUAL SUBSPACE for ONLY the
#                  sibling pair is the smallest possible specialisation,
#                  and it has not been used before for code-author detection.
# FALSIFIER      : (a) K=32 sibling F1 (mean of class-1 codellama + class-4
#                      nxcode) >= 0.20. If not -> theorem inapplicable here
#                      (residual subspace too noisy at K=32 for the Fisher
#                      ratio to be a useful gradient).
#                  (b) Non-sibling per-class F1 (human, gpt, llama3.1, qwen)
#                      MUST NOT regress by more than 0.02 vs the FS-NTKAlign
#                      reference at the same data budget. If it does -> the
#                      orthogonality constraint leaked into other classes.
#                  (c) Diagnostic: log per-batch S_B / (S_W + eps) during
#                      training. It must MONOTONICALLY INCREASE in the second
#                      half of training. If it doesn't -> the loss is
#                      ineffective and the orthogonality assumption is wrong.
# COMPUTE        : ~50 min on Kaggle T4 for the default sweep (K=32,128,
#                  frac=0.01, frac=0.05). Same as every other inline file.
#
# Paste this entire file into a Kaggle cell, run. No `git clone` needed,
...
logger = logging.getLogger("exp_n01_sibling_residual")
...
# Family map: codellama (1) and nxcode (4) share family "A". Other singletons.
# Class indices from the verified vocab order:
#   human=0, codellama=1, gpt=2, llama3.1=3, nxcode=4, qwen1.5=5
SIBLING_FAMILY = {1, 4}  # the only non-trivial sibling pair


def srd_loss(outputs, labels, lambda_srd=0.4, eps=1e-6, class_weights=None):
    """The single new mathematical object of n01.

    Returns dict with `total`, `ce`, `srd`, `s_b`, `s_w`, `n_pair`.
    """
    ce = F.cross_entropy(outputs["logits"], labels, weight=class_weights)
    z = outputs["ntk_proj"]                              # (B, D), L2-normalised

    # Mask the codellama-nxcode sibling subset of the batch.
    sib_mask = torch.zeros_like(labels, dtype=torch.bool)
    sib_mask = sib_mask | (labels == 1) | (labels == 4)
    n_sib = int(sib_mask.sum().item())
    if n_sib < 4:
        # Need at least 2 from each class to estimate residual statistics.
        return {"total": ce, "ce": ce, "srd": z.new_zeros(()),
                "s_b": z.new_zeros(()), "s_w": z.new_zeros(()), "n_pair": 0}

    z_sib = z[sib_mask]                                  # (N_sib, D)
    y_sib = labels[sib_mask]
    n1 = int((y_sib == 1).sum().item())
    n4 = int((y_sib == 4).sum().item())
    if n1 < 2 or n4 < 2:
        return {"total": ce, "ce": ce, "srd": z.new_zeros(()),
                "s_b": z.new_zeros(()), "s_w": z.new_zeros(()), "n_pair": n_sib}
    ...
    return {"total": ce + lambda_srd * srd, "ce": ce, "srd": srd,
...
def train(cfg, model, train_l, val_l, test_l, lambda_srd):
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = srd_loss(out, y, lambda_srd=lambda_srd, eps=cfg.eps, class_weights=cw)
                loss = losses["total"]
            ...
            sb_trace.append(float(losses["s_b"].item())); sw_trace.append(float(losses["s_w"].item()))
            npair_trace.append(int(losses["n_pair"]))
            if step % 10 == 0:
    ...
    # Falsifier diagnostic: did Fisher ratio (s_b / (s_w + eps)) increase?
    half = max(1, len(sb_trace) // 2)
    fisher_first = sum(sb_trace[:half]) / max(1, sum(sw_trace[:half]) + cfg.eps * half)
    fisher_second = sum(sb_trace[half:]) / max(1, sum(sw_trace[half:]) + cfg.eps * (len(sb_trace) - half))
    fisher_increased = fisher_second > fisher_first
    logger.info(f"[falsifier-c] Fisher first-half={fisher_first:.4f} second-half={fisher_second:.4f} "
                f"increased={fisher_increased}")
    return {
        ...
        "fisher_first_half": fisher_first, "fisher_second_half": fisher_second,
        "fisher_increased": int(fisher_increased),
        "n_pair_avg": float(np.mean(npair_trace)) if npair_trace else 0.0,
    }
...
def emit(method, exp_id, cfg, results):
    ...
    payload = {"method": method, "exp_id": exp_id, "regime": rl,
               ...
               "config": {"benchmark": cfg.benchmark, "n_classes": cfg.n_classes,
                          ...
                          "max_length": cfg.max_length, "lambda_srd": cfg.lambda_srd},
...
METHOD_NAME = "FS-SRD-SiblingResidual"
EXP_ID = "exp_n01_sibling_residual"


def run_one(kind, value, base_seed, lambda_srd, benchmark="codet_m4"):
    cfg = FSConfig(
        benchmark=benchmark,
        k_shot=value if kind == "kshot" else 0,
        train_fraction=value if kind == "fraction" else 0.0,
        fs_seed=base_seed, lambda_srd=lambda_srd,
    )
    cfg = apply_hardware_profile(cfg)
    logger.info(f"\n{'=' * 60}\n[{EXP_ID}] {kind}={value} lambda_srd={lambda_srd}\n{'=' * 60}")
    train_l, val_l, test_l, counts, vocab = build_loaders(cfg)
    model = FSClassifier(cfg)
    results = train(cfg, model, train_l, val_l, test_l, lambda_srd)
    results["lambda_srd"] = lambda_srd
    results["train_per_class"] = counts
    emit(METHOD_NAME, EXP_ID, cfg, results)
    return results["test_macro_f1"], results["val_macro_f1"], results["wall_time_s"], results


def main():
    base_seed = int(os.environ.get("FS_SEED", "42"))
    lambda_srd = float(os.environ.get("FS_LAMBDA_SRD", "0.4"))
    ...
    logger.info(f"[{EXP_ID}] benchmark={benchmark} sweep={configs} "
                f"seed={base_seed} lambda_srd={lambda_srd}")
    summary = []
    for kind, value in configs:
        t, v, w, r = run_one(kind, value, base_seed, lambda_srd, benchmark=benchmark)
        sib_f1 = (r["per_class_f1"].get("class_1", 0.0) + r["per_class_f1"].get("class_4", 0.0)) / 2.0
    ...
    print(f"{'config':<14}{'test_F1':>10}{'val_F1':>10}{'gap':>10}"
          f"{'sibF1':>10}{'fisher_up':>11}{'wall':>10}")
    print("-" * 75)
    for k, v, t, vl, w, sib, fup in summary:
        lbl = f"K={v}" if k == "kshot" else f"frac={v:.4f}"
        print(f"{lbl:<14}{t:>10.4f}{vl:>10.4f}{(vl - t):>+10.4f}"
              f"{sib:>10.4f}{fup:>11d}{w:>10.0f}")
    print("-" * 75)
    print(f"{len(summary)} configs. Falsifier (a): K=32 sibF1 must be >= 0.20.")
    print(f"Falsifier (c): fisher_up=1 means Fisher ratio increased during training.")
