...
def run_one(kind, value, base_seed, lambda_srd):
    cfg = FSConfig(
...
def main():
    ...
    configs = parse_sweep()
    logger.info(f"[{EXP_ID}] sweep={configs} seed={base_seed} lambda_srd={lambda_srd}")
    summary = []
    for kind, value in configs:
        t, v, w, r = run_one(kind, value, base_seed, lambda_srd)
