...
logger = logging.getLogger("exp_n17_prototypical")
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = prototypical_loss(out, y, n_classes=cfg.n_classes, scale=10.0, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_PROTO_SCALE", "10.0"))
