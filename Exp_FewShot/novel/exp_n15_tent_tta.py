...
logger = logging.getLogger("exp_n15_tent_tta")
...
def tent_entropy_loss(outputs, labels, lambda_tent=0.0, class_weights=None):
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = tent_entropy_loss(out, y, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_LAMBDA_TENT", "0.0"))
