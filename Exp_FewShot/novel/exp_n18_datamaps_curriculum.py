...
logger = logging.getLogger("exp_n18_datamaps_curriculum")
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = datamaps_loss(out, y, sample_ids=None, lambda_dmc=lambda_method, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_LAMBDA_DMC", "0.5"))
