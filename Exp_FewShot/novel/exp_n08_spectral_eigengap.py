...
logger = logging.getLogger("exp_n08_spectral_eigengap")
...
def spectral_eigengap_loss(outputs, labels, n_classes, lambda_eg=0.1,
    ...
    for _ in range(cfg.epochs):
        ...
        for b in train_l:
            ...
            with autocast(device_type="cuda" if dev.type == "cuda" else "cpu",
                          enabled=dev.type == "cuda", dtype=dt):
                out = model(ids, mask)
                losses = spectral_eigengap_loss(out, y, n_classes=cfg.n_classes, lambda_eg=lambda_method, class_weights=cw)
                loss = losses["total"]
...
def main():
    ...
    lambda_method = float(os.environ.get("FS_LAMBDA_EG", "0.1"))
