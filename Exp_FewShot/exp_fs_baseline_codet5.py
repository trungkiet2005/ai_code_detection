"""
exp_fs_baseline_codet5.py -- ONE-FILE self-contained baseline reimplementation for Kaggle T4.

Method: FS-Baseline-CodeT5
Encoder: Salesforce/codet5-base
CodeT5 (Wang et al. EMNLP'21) -- encoder-decoder; we use the encoder side. 62.45 full-data in paper Table 7
Paper full-data baseline: **62.45 Macro-F1** (Author IID, CoDET-M4).
...
Output: /kaggle/working/results/exp_fs_baseline_codet5_<label>_seed<S>.json
"""
...
logger = logging.getLogger('exp_fs_inline_baseline')
...
class FSConfig:
    ...
    n_classes: int = 6
    fs_seed: int = 42
    encoder_name: str = 'Salesforce/codet5-base'
    max_length: int = 384
    epochs: int = 1
    batch_size: int = 16
...
class FSClassifier(nn.Module):
    def __init__(self, cfg: FSConfig):
        super().__init__()
        self.cfg = cfg
        # CodeT5 is encoder-decoder; use T5EncoderModel for the encoder side only.
        if "codet5" in cfg.encoder_name.lower() or "t5" in cfg.encoder_name.lower():
            self.encoder = T5EncoderModel.from_pretrained(cfg.encoder_name)
        else:
...
METHODS = {
    "baseline":       ('FS-Baseline-CodeT5',   'exp_fs_baseline_codet5',   "ce",     False),
