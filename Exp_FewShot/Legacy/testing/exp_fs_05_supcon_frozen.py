...
def run_one(kind, value, base_seed, lr_heads, lambda_supcon, temperature):
    cfg = FSConfig(
    ...
    logger.info(f"\n{'=' * 60}\n[{EXP_ID}] {kind}={value} FROZEN+SupCon lambda={lambda_supcon}\n{'=' * 60}")
...
def main():
    ...
    for kind, value in configs:
        test_f1, val_f1, wall = run_one(kind, value, base_seed, lr_heads, lambda_supcon, temperature)
