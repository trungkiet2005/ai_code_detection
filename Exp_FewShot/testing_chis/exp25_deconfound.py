"""
================================================================================
Theory-Track exp -- Deconfounded Representation Learning (DRL):
Causal intervention for removing source confound in code representations.

ARXIV_ID      : ICML 2019 Causal Inference (1905.09376); ICLR 2020 IRM (1906.08533)
NAME          : Deconfounded Representation Learning (DRL)
ONE-LINE CLAIM: Applying causal interventions to remove source confound improves
                OOD generalization by learning invariant rather than spurious features.
EQUATION      : P(Y | do(X)) = Σ_s P(Y | X, S=s) P(s)
                IRM penalty: λ ||∇_w log P(Y|X,w)||²
PROPERTY      : IRM encourages representations that work across all environments,
                removing source-specific shortcuts.
WHY NOT BEFORE: Standard CE can exploit source-specific features that don't
                generalize to OOD test distributions.
FALSIFIER     : If DRL does NOT improve OOD accuracy more than IID accuracy,
                the causal intervention is not helping.
================================================================================
...
class DRLNet(nn.Module):
    """Encoder withIRM-based deconfounded representation."""
...
def compute_irm_penalty(logits, labels):
    """IRM penalty: gradient of log loss w.r.t. classifier weights."""
    ...
    for ep in range(cfg.epochs):
        ...
        for b in pbar:
            ...
            with _autocast_ctx(dev):
                ...
                # IRM penalty (after anneal steps)
                if global_step > cfg.penalty_anneal:
                    penalty = compute_irm_penalty(logits, labs)
                    loss = loss_ce + cfg.irm_lambda * penalty
                else:
                    penalty = torch.tensor(0.0, device=dev)
                    loss = loss_ce

            ep_ce.append(loss_ce.item())
            ep_irm.append(penalty.item())
            ...
            sched.step()
            global_step += 1
            pbar.set_postfix({"ce": f"{loss_ce.item():.3f}", "irm": f"{penalty.item():.3f}"})

        tr_met = eval_m(model, tr_dl, dev)
        vr = eval_m(model, vl_dl, dev)
        train_history.append({
            "epoch": ep + 1, "loss": round(np.mean(ep_loss), 6),
            "ce_loss": round(np.mean(ep_ce), 6), "irm_penalty": round(np.mean(ep_irm), 6),
