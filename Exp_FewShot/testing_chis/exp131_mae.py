# exp131 — MAE-CODE
# NAME       : MAE-CODE (Masked AutoEncoder for code authorship)
# REFERENCE  : new for code authorship; MAE (He et al. 2021,
#              arXiv:2111.06377), BEiT (Bao 2021), data2vec (Baevski 2022).
#              MAE was originally for vision; never applied to AUTHORSHIP.
# CLAIM      : The right pretext for authorship is to mask 75% of code
#              tokens and reconstruct them via an ASYMMETRIC architecture:
#              a large encoder sees only the UNMASKED 25%, a tiny decoder
#              fills in the rest. Forcing the encoder to produce
#              representations that allow 75% reconstruction from 25%
#              visible context teaches it to capture the high-level
#              structural patterns that an author repeatedly uses — the
#              authorial fingerprint that survives heavy masking.
# EQUATION   : Pretext phase:
#                visible_25 = random 25% of non-pad tokens
#                z_visible = encoder(input_with_masked_zeroed)
#                full_seq = decoder(z_visible + mask_tokens at masked positions)
#                L_recon = CE(full_seq, original_tokens) only at masked positions
#              Finetune phase:
#                Discard decoder; standard TRACO supcon + CE on full sequence.
# WHY NEW    : CodeBERT/UniXcoder use 15% MLM. We use 75% MAE.
#              The high mask ratio FORCES the encoder to internalise
#              structural style (cannot reconstruct 75% from 25% without
#              understanding the author's patterns). No prior code-
#              attribution paper has tried 75%-mask self-supervision.
# WOW HOOK   : "The encoder learns to fill in 75% blanks. What survives
#              from such heavy masking IS authorship — the high-level
#              structural patterns the author repeats can be inferred
#              from a quarter of the code."
# FALSIFIER  : (F1) Reconstruction perplexity on test > 1.5x random
#              baseline AND < 5x (not collapsed, not perfect).
#              (F2) MAE-pretrained encoder + TRACO finetune beats
#              MLM-pretrained UniXcoder + TRACO finetune by ≥ 0.005
#              at 1% slot. (F3) Removing the MAE pretext (cold-start
#              encoder) loses ≥ 0.02 at 1%.
...
logger = logging.getLogger("exp131_mae")
...
# =============================================================================
# CARGO structural augmentations (reused verbatim from exp84 for the
# TRACO-finetune phase of MAE-CODE — the contrastive 2-view objective needs
# semantic-preserving structural rewrites identical to CARGO's library).
# =============================================================================
...
# =============================================================================
# Tokenization helper (UniXcoder <encoder_only>)
# =============================================================================

def _tokenize(tok, code, seq_len):
    """UniXcoder <encoder_only> tokenization wrapper.
    Falls back to standard tokenization if the special token is unavailable."""
    try:
        enc = tok(code, max_length=seq_len, padding="max_length",
...
class MAEDecoder(nn.Module):
    """Tiny decoder: 2-layer transformer + linear head to vocab.
    Asymmetric MAE: decoder is much smaller than encoder."""
    def __init__(self, hidden_size, vocab_size, decoder_dim=256, n_layers=2, max_len=512):
        super().__init__()
        self.proj_in = nn.Linear(hidden_size, decoder_dim)
        layer = nn.TransformerEncoderLayer(
            d_model=decoder_dim, nhead=4,
            dim_feedforward=decoder_dim * 2, batch_first=True)
        self.transformer = nn.TransformerEncoder(layer, num_layers=n_layers)
        self.head = nn.Linear(decoder_dim, vocab_size)
        self.mask_token = nn.Parameter(torch.randn(1, 1, decoder_dim) * 0.02)
        self.pos_embed = nn.Parameter(torch.randn(1, max_len, decoder_dim) * 0.02)

    def forward(self, z_visible, mask_positions, attention_mask):
        """z_visible: (B, L, h_enc); zeroed at masked positions.
        mask_positions: (B, L) bool — True where masked.
        attention_mask: (B, L) — 1 for non-pad."""
        x = self.proj_in(z_visible)
        B, L, _ = x.shape
        mask_tok = self.mask_token.expand(B, L, -1)
        x = torch.where(mask_positions.unsqueeze(-1), mask_tok, x)
...
class MAEModel(nn.Module):
    """UniXcoder encoder + tiny MAE decoder + classification head.
    Encoder is shared between pretext (with masked input) and finetune (full input)."""
    def __init__(self, enc_name, n_cls, vocab_size, emb_dim=256, decoder_dim=256, max_len=512):
        super().__init__()
        self.encoder = AutoModel.from_pretrained(os.path.join(KAGGLE_MODELS, enc_name), local_files_only=True)
        h = self.encoder.config.hidden_size
        self.decoder = MAEDecoder(h, vocab_size, decoder_dim=decoder_dim, n_layers=2, max_len=max_len)
    ...
    def encode_for_clf(self, ids, mask):
        H = self.encode_full(ids, mask)
        mask_f = mask.unsqueeze(-1).float()
        sem = (H * mask_f).sum(1) / mask_f.sum(1).clamp(min=1)
        z = F.normalize(self.proj(sem), dim=-1)
        return z, self.clf(z)

    def reconstruct(self, ids, mask, mask_positions):
        """MAE forward: zero hidden states at masked positions before decoder."""
        H = self.encode_full(ids, mask)
        H_visible = H * (~mask_positions).unsqueeze(-1).float()
        recon_logits = self.decoder(H_visible, mask_positions, mask)
        return recon_logits
...
def mae_random_mask(attention_mask, mask_ratio=0.75):
    """Per-sample random masking of `mask_ratio` of non-pad tokens.
    Returns (B, L) boolean of which positions are masked."""
    B, L = attention_mask.shape
    masked = torch.zeros_like(attention_mask, dtype=torch.bool)
    for b in range(B):
        valid_idx = (attention_mask[b] > 0).nonzero(as_tuple=True)[0]
        n_valid = valid_idx.size(0)
        n_mask = int(n_valid * mask_ratio)
        if n_mask > 0:
            perm = torch.randperm(n_valid, device=attention_mask.device)[:n_mask]
...
def _hw(cfg):
    if torch.cuda.is_available():
        ...
        # MAE: encoder + decoder + 2-view finetune = heavier
        if mem >= 80: cfg.bs, cfg.seq = 64, 512
        elif mem >= 40: cfg.bs, cfg.seq = 48, 512
        elif mem >= 20: cfg.bs, cfg.seq = 32, 384
        elif mem >= 10: cfg.bs, cfg.seq = 24, 384
        else: cfg.bs, cfg.seq = 16, 256
        logger.info(f"[hw] mem={mem:.1f}GB bs={cfg.bs} (MAE: HALVED for encoder+decoder+2-view) seq={cfg.seq}")
    return cfg
...
class FSDS_PRETEXT(TD):
    """Single-view dataset for MAE pretext phase (no augmentation)."""
    def __init__(self, data, tok, seq_len, frac=1.0, seed=42):
        self.data = data; self.tok = tok; self.seq_len = seq_len; self.seed = seed
        if frac < 1.0:
            ...
            logger.info(f"[FSDS_PRETEXT] Sampled {len(self.data)} samples ({frac*100:.0f}%)")
    ...
    def __getitem__(self, i):
        ...
        ids0, mask0 = _tokenize(self.tok, code, self.seq_len)
        return {"ids0": ids0, "mask0": mask0,
                "label": r["label"], "language": lang,
                "source": r.get("source", "") or ""}


class FSDS_FINETUNE(TD):
    """2-view dataset for TRACO finetune (same as FSDS_CARGO in exp84)."""
    def __init__(self, data, tok, seq_len, frac=1.0, seed=42, do_aug=True):
        ...
        if frac < 1.0:
            ...
            for lbl in labels:
                ...
                keep.extend(rng.sample(idx, min(max(1, int(len(idx)*frac)), len(idx))))
            self.data = self.data.select(keep)
            logger.info(f"[FSDS_FINETUNE] Sampled {len(self.data)} samples ({frac*100:.0f}%)")
    ...
    def __getitem__(self, i):
        ...
        if self.do_aug:
            ...
            ids1, mask1 = _tokenize(self.tok, code_aug, self.seq_len)
        else:
...
# =============================================================================
# Eval (uses encode_for_clf — no augmented view needed at eval)
# =============================================================================
...
def eval_pack(model, loader, cfg, sib_mask_np, dist_mat_cpu):
    ...
    for b in tqdm(loader, desc="Eval"):
        ...
        _, logits = model.encode_for_clf(ids0, mask0)
...
# =============================================================================
# Training loops (pretext + finetune)
# =============================================================================

def pretext_epoch(model, loader, opt, sch, scaler, cfg, tok):
    """One MAE pretext epoch: mask 75%, reconstruct masked positions."""
    model.train()
    tot, n_tok = 0.0, 0
    pad_id = tok.pad_token_id if tok.pad_token_id is not None else 0
    for b in tqdm(loader, desc="Pretext"):
        ids = b["ids0"].to(cfg.device); mask = b["mask0"].to(cfg.device)
        mask_pos = mae_random_mask(mask, mask_ratio=cfg.mask_ratio)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(cfg.device == "cuda")):
            recon_logits = model.reconstruct(ids, mask, mask_pos)
            B, L, V = recon_logits.shape
            ce_per_tok = F.cross_entropy(
                recon_logits.view(B*L, V), ids.view(B*L),
                ignore_index=pad_id,
                reduction='none'
            ).view(B, L)
            n_masked = mask_pos.float().sum().clamp(min=1)
            loss = (ce_per_tok * mask_pos.float()).sum() / n_masked
        scaler.scale(loss).backward()
        scaler.unscale_(opt)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        scaler.step(opt); scaler.update(); opt.zero_grad(); sch.step()
        tot += loss.item() * n_masked.item()
        n_tok += n_masked.item()
    return tot / max(1, n_tok)


def train_epoch_finetune(model, loader, opt, sch, scaler, cfg, dist_mat):
    """Standard TRACO finetune: CE + tree-weighted SupCon on doubled batch."""
    ...
    for b in tqdm(loader, desc="Finetune"):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(cfg.device == "cuda")):
            z0, logits = model.encode_for_clf(ids0, mask0)
            z1, _ = model.encode_for_clf(ids1, mask1)
...
@torch.no_grad()
def compute_test_recon_loss(model, loader, cfg, tok):
    """Measure MAE reconstruction loss on test set (for F1 falsifier)."""
    model.eval()
    tot, n_tok = 0.0, 0
    pad_id = tok.pad_token_id if tok.pad_token_id is not None else 0
    for b in tqdm(loader, desc="Recon-test"):
        ids = b["ids0"].to(cfg.device); mask = b["mask0"].to(cfg.device)
        mask_pos = mae_random_mask(mask, mask_ratio=cfg.mask_ratio)
        recon_logits = model.reconstruct(ids, mask, mask_pos)
        B, L, V = recon_logits.shape
        ...
        n_masked = mask_pos.float().sum().clamp(min=1)
        tot += (ce * mask_pos.float()).sum().item()
        n_tok += n_masked.item()
    return tot / max(1, n_tok)
...
def run_exp(cfg, tag):
    ...
    # Datasets
    pretext_ds = FSDS_PRETEXT(tr_data, tok, cfg.seq, frac=cfg.frac, seed=cfg.seed)
    tr_ds = FSDS_FINETUNE(tr_data, tok, cfg.seq, frac=cfg.frac, seed=cfg.seed, do_aug=True)
    vl_ds = FSDS_FINETUNE(vl_data, tok, cfg.seq, frac=1.0, seed=cfg.seed+1, do_aug=False)
    ts_ds = FSDS_FINETUNE(ts_data, tok, cfg.seq, frac=1.0, seed=cfg.seed+2, do_aug=False)

    logger.info(f"[sched] frac={cfg.frac} pretext_ep={cfg.pretext_epochs} ft_ep={cfg.epochs} "
    ...
    pretext_loader = DataLoader(pretext_ds, shuffle=True, **loader_cfg)
    tr_dl = DataLoader(tr_ds, shuffle=True, **loader_cfg)
    vl_dl = DataLoader(vl_ds, shuffle=False, **loader_cfg)
    ts_dl = DataLoader(ts_ds, shuffle=False, **loader_cfg)

    model = MAEModel(cfg.enc, cfg.n_cls, vocab_size,
                     emb_dim=cfg.emb_dim, decoder_dim=cfg.decoder_dim,
                     max_len=cfg.seq).to(cfg.device)

    # ===== Phase 1: MAE pretext =====
    pretext_total = max(1, len(pretext_loader)) * cfg.pretext_epochs
    opt_pretext = torch.optim.AdamW(model.parameters(), lr=cfg.lr_enc, weight_decay=cfg.wd)
    sch_pretext = get_cosine_schedule_with_warmup(
        opt_pretext, max(1, int(pretext_total*0.1)), pretext_total)
    scaler_pretext = GradScaler()
    mae_recon_history = []
    for ep in range(cfg.pretext_epochs):
        recon_loss = pretext_epoch(model, pretext_loader, opt_pretext, sch_pretext,
                                   scaler_pretext, cfg, tok)
        mae_recon_history.append(recon_loss)
        logger.info(f"[pretext ep{ep+1}] recon_loss={recon_loss:.4f}")

    # ===== Phase 2: TRACO finetune =====
    finetune_total = max(1, len(tr_dl)) * cfg.epochs
    ...
    sch_ft = get_cosine_schedule_with_warmup(
        opt_ft, max(1, int(finetune_total * cfg.warmup)), finetune_total)
    scaler_ft = GradScaler()
    best_val, best_state, val_hist = 0.0, None, []
    for ep in range(cfg.epochs):
        loss, ce, sc = train_epoch_finetune(model, tr_dl, opt_ft, sch_ft, scaler_ft, cfg, dist_mat)
        val_met = eval_pack(model, vl_dl, cfg, sib_mask_np, dist_mat_cpu)
        v = val_met["overall"]["macro_f1"]
        val_hist.append(v)
        logger.info(f"[finetune ep{ep+1}] loss={loss:.4f} ce={ce:.4f} supcon={sc:.4f} val={v:.4f}")
        if v > best_val:
            best_val = v
    ...
    # F1 falsifier: reconstruction perplexity on test
    test_recon_loss = compute_test_recon_loss(model, ts_dl, cfg, tok)
    random_baseline_ppl = math.log(vocab_size)
    test_recon_ppl = math.exp(test_recon_loss)

    logger.info(f"[final] val={best_val:.4f} test={test_macro:.4f} gap={gap:+.4f} "
                f"recon_loss={test_recon_loss:.4f} recon_ppl={test_recon_ppl:.2f} "
                f"random_ppl={math.exp(random_baseline_ppl):.2f}")

    return {
        "tag": tag, "method": "MAE-CODE",
        "note": f"MAE pretext ({cfg.pretext_epochs}ep, mask_ratio={cfg.mask_ratio}) then TRACO finetune",
        "enc": cfg.enc, "bench": cfg.benchmark, "frac": cfg.frac,
        "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
        "mask_ratio_used": cfg.mask_ratio,
        ...
        "mae_recon_loss_history": mae_recon_history,
        "mae_final_recon_loss": mae_recon_history[-1] if mae_recon_history else None,
        "test_recon_loss": test_recon_loss,
        "test_recon_perplexity": test_recon_ppl,
        "random_perplexity_baseline": math.exp(random_baseline_ppl),
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp131_mae_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
                    ...
                    logger.info(f"[{tag}] test={res['macro']:.4f} ({res['dpaper']:+.4f}) "
                                ...
                                f"recon_ppl={res['test_recon_perplexity']:.2f} "
    ...
    with open(os.path.join(out_dir, "exp131_mae_results.json"), "w") as f:
        json.dump(results, f, indent=2)

    print("\n" + "="*160)
    print(f"{'Encoder':<22} {'Bench':<12} {'Frac':>6} {'Ep':>4} {'Val-F1':>8} {'Test-F1':>8} "
          f"{'Gap':>8} {'dPaper':>9} {'ReconLoss':>10} {'ReconPpl':>10} {'Wall':>8}")
    print("-"*160)
    for r in results:
        print(f"{r['enc']:<22} {r['bench']:<12} {r['frac']:>6.0%} {r['epochs']:>4d} "
              f"{r['val_macro']:>8.4f} {r['macro']:>8.4f} {r['val_test_gap']:>+8.4f} "
              f"{r['dpaper']:>+9.4f} {r['test_recon_loss']:>10.4f} "
              f"{r['test_recon_perplexity']:>10.2f} {r['wall']:>8.0f}s")
    print("="*160)
