...
def run_exp(cfg: Cfg, tag: str):
    set_seed(cfg.seed)
    cfg = _hw(cfg)
    logger.info(f"[exp39] STO: {tag} | frac={cfg.frac}")
    ...
    for epoch in range(cfg.epochs):
        ...
        logger.info(f"  E{epoch+1}: loss={loss:.4f} ce={loss_ce:.4f} sto={loss_sto:.4f} | val={val_met['macro']:.4f}")
...
def main():
    ...
    for bench, task, n_cls in benchmarks:
        for frac in fracs:
            ...
            tag = f"exp39_sto_{enc}_{bench}_f{frac:.2f}"
            try:
                r = run_exp(cfg, tag)
