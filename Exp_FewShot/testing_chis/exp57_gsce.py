# =============================================================================
# Theory-Track exp -- GSCE (Genealogy-Smoothed Cross-Entropy)
#
# TRACKER INSIGHT: SSL (exp42) wins CoDET-M4 (+0.0163);  LabelSmooth wins AICD-T2
#                  at 20% (+0.014 vs CE).  Standard label smoothing uses a UNIFORM
#                  target q = (1-eps)*onehot + eps/K.  In a 12-class genealogy
#                  (AICD-T2: 4 families x 3 generators), uniform smoothing spends
#                  mass on distant families that should be EASY to separate, and
#                  starves mass from siblings -- exactly where confusion happens.
# NAME           : GSCE  (Genealogy-Smoothed Cross-Entropy)
# ONE-LINE CLAIM : Replace the uniform smoothing prior by a genealogy-tempered
#                  prior that puts more mass on siblings than on distant classes.
# EQUATION       : q_c = (1 - eps) * 1[c == y]
#                        + eps * exp(-alpha * d_tree(y, c)) / Z(y)
#                  where Z(y) = sum_{c != y} exp(-alpha * d_tree(y, c)).
#                  Loss = KL(q || softmax(logits)).
# PROPERTY       : Uniform smoothing  <==>  GSCE with alpha = 0.
#                  Sibling-confusion penalty (SSL)  <==>  GSCE on the *target*
#                  side rather than the *prediction* side.  GSCE is therefore
#                  the SOFT-LABEL dual of SSL's HARD reweighting.
# WHY NOT BEFORE : Standard LS is class-agnostic.  GSCE is undefined unless the
#                  label space carries a tree metric -- this is specific to
#                  generator attribution with known genealogy.
# FALSIFIER      : If GSCE improves CoDET-M4 but not AICD-T2, the gain is from
#                  smoothing alone, not the genealogy prior.
...
logger = logging.getLogger("exp57_gsce")
...
def build_smoothing_targets(n_cls, adj, eps=0.1, alpha=1.0):
    """Q[y, c] is the target probability assigned to class c when the true label is y.

    Q[y, y] = 1 - eps
    Q[y, c] = eps * exp(-alpha * d_tree(y, c)) / Z(y)    for c != y
    """
...
# =============================================================================
# AST features (legacy-aligned 22-feature)
# =============================================================================
...
def gsce_loss(logits, labels, smoothing_targets):
    log_p = F.log_softmax(logits, dim=-1)
    q = smoothing_targets[labels]
    return -(q * log_p).sum(dim=-1).mean()


# =============================================================================
# Config + Regime-Adaptive Schedule (same logic as exp56)
# =============================================================================
...
def train_epoch(model, loader, opt, sch, scaler, cfg, Q):
    model.train()
    total = 0.0
...
def run_exp(cfg, tag):
    ...
    logger.info(
        ...
        f"warmup={cfg.warmup} eps={cfg.eps} alpha={cfg.alpha} total_steps={total_steps}"
    )
    ...
    model = GSCEModel(cfg.enc, cfg.n_cls).to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        loss = train_epoch(model, tr_dl, opt, sch, scaler, cfg, Q)
        val_met = eval_model(model, vl_dl, cfg)
        val_hist.append(val_met["macro"])
        logger.info(f"[epoch {epoch+1}] loss={loss:.4f} val_macro={val_met['macro']:.4f}")
    ...
    return {
        "tag": tag, "method": "GSCE", "enc": cfg.enc, "bench": cfg.benchmark,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp57_gsce_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "exp57_gsce_results.json"), "w") as f:
