"""
================================================================================
Theory-Track exp -- Wasserstein Domain Alignment (WDA):
Optimal transport for aligning code distributions across generators.

ARXIV_ID      : ICML 2019 Sinkhorn (1906.03327); NeurIPS 2023 WCD (2310.14780)
NAME          : Wasserstein Domain Alignment (WDA)
ONE-LINE CLAIM: Aligning generator distributions via Wasserstein distance provides
                smoother interpolation than KL-divergence, improving few-shot generalization.
EQUATION      : W(p_y, q_y) = inf_{γ∈Π(p_y,q_y)} E_{(x,x')∼γ}[d(x,x')]
                where Π is the set of couplings between distributions
PROPERTY      : Wasserstein distance respects the metric structure of the embedding space,
                providing gradient signals even when distributions don't overlap.
WHY NOT BEFORE: KL-divergence requires overlapping support. Wasserstein provides signal
                even for disjoint distributions via earth-mover distance.
FALSIFIER     : If WDA does NOT improve over CE on OOD test, the Wasserstein
                alignment is not providing generalization benefits.
================================================================================

exp19_wda.py — Wasserstein Domain Alignment for few-shot AI-code attribution.
...
_ensure("transformers"); _ensure("scikit-learn"); _ensure("tqdm"); _ensure("pot")
...
class Cfg:
    ...
    # WDA specific
    reg: float = 0.05  # Entropic regularization for Sinkhorn
    wda_weight: float = 0.3  # Weight for WDA loss
...
class WDANet(nn.Module):
    """Encoder + classifier with Wasserstein domain alignment."""
...
def wda_loss(pooled, labels, n_cls, reg=0.05):
    """
    Wasserstein Domain Alignment loss using Sinkhorn algorithm.
    Aligns class-conditional distributions in embedding space.
    """
    device = pooled.device
    loss_wda = torch.tensor(0.0, device=device)
    ...
    for c in range(n_cls):
        ...
        # Compute mean embedding for class c
        mu_c = pooled[mask_c].mean(dim=0, keepdim=True)

        # Align with other classes
        for c2 in range(c + 1, n_cls):
            mask_c2 = (labels == c2)
            if mask_c2.sum() < 2:
                continue

            mu_c2 = pooled[mask_c2].mean(dim=0, keepdim=True)

            # Wasserstein-2 distance between class means
            w_dist = torch.sum((mu_c - mu_c2) ** 2)

            # Add to loss (minimize distance between class means)
            loss_wda = loss_wda + w_dist

    return loss_wda / max(1, n_cls * (n_cls - 1) // 2)


def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    dev = torch.device(cfg.device)
    model = WDANet(cfg).to(dev)
    ...
    for ep in range(cfg.epochs):
        ...
        for b in pbar:
            ...
            with _autocast_ctx(dev):
                ...
                # WDA loss: align class distributions
                loss_wda = wda_loss(pooled, labs, cfg.n_cls, cfg.reg)

                loss = loss_ce + cfg.wda_weight * loss_wda

            ep_ce.append(loss_ce.item())
            ep_wda.append(loss_wda.item())
            ...
            sched.step()
            pbar.set_postfix({"ce": f"{loss_ce.item():.3f}", "wda": f"{loss_wda.item():.3f}"})

        tr_met = eval_m(model, tr_dl, dev)
        vr = eval_m(model, vl_dl, dev)
        train_history.append({
            "epoch": ep + 1, "loss": round(np.mean(ep_loss), 6),
            "ce_loss": round(np.mean(ep_ce), 6), "wda_loss": round(np.mean(ep_wda), 6),
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            ...
            tag = f"exp19_wda_{enc}_{bench}_fixed{FIXED_TOTAL}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            ...
            row = {
                ...
                "reg": cfg.reg, "wda_weight": cfg.wda_weight,
    ...
    with open("results/exp19_wda_results.json", "w") as f:
