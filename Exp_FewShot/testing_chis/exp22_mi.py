"""
================================================================================
Theory-Track exp -- Mutual Information Attribution (MIA):
Information-theoretic attribution via conditional mutual information.

ARXIV_ID      : ICML 2019 DIM (1901.03610); NeurIPS 2019 MINE (1901.07110)
NAME          : Mutual Information Attribution (MIA)
ONE-LINE CLAIM: Maximizing I(X; Y) while minimizing I(X; S) improves attribution
                by focusing on generator-relevant rather than source-relevant features.
EQUATION      : I(X; Y) - λ·I(X; S) ≈ E[log(D)] - E[log(1-D)]
                where D is a discriminator between joint and marginal.
PROPERTY      : The InfoNCE bound provides a tractable lower bound on mutual
                information, enabling gradient-based optimization.
WHY NOT BEFORE: Standard CE implicitly maximizes I(X; Y) but does not explicitly
                minimize I(X; S), allowing source confound to leak.
FALSIFIER     : If MIA does NOT reduce source-confusion relative to CE,
                the mutual information formulation is not correct.
================================================================================

exp22_mi.py — Mutual information based few-shot AI-code attribution.
...
logger = logging.getLogger("exp22")
...
class Cfg:
    ...
    # MIA specific
    mi_weight: float = 0.3  # Weight for mutual information loss
    temp: float = 0.1  # Temperature for contrastive MI
...
class MIANet(nn.Module):
    """Encoder with mutual information based loss."""
    ...
    def forward(self, ids, mask):
        ...
        mi_emb = self.mi_head(pooled)
        return {"logits": logits, "pooled": pooled, "mi_emb": mi_emb}


def info_nce_loss(z1, z2, labels, temp=0.1):
    """InfoNCE loss: contrastive loss for mutual information maximization."""
    ...
    for ep in range(cfg.epochs):
        ...
        for b in pbar:
            ...
            with _autocast_ctx(dev):
                ...
                # MI loss: contrastive within same class
                loss_mi = info_nce_loss(mi_emb, mi_emb.detach(), labs, cfg.temp)

                loss = loss_ce + cfg.mi_weight * loss_mi

            ep_ce.append(loss_ce.item())
            ep_mi.append(loss_mi.item())
            ...
            sched.step()
            pbar.set_postfix({"ce": f"{loss_ce.item():.3f}", "mi": f"{loss_mi.item():.3f}"})

        tr_met = eval_m(model, tr_dl, dev)
        vr = eval_m(model, vl_dl, dev)
        train_history.append({
            "epoch": ep + 1, "loss": round(np.mean(ep_loss), 6),
            "ce_loss": round(np.mean(ep_ce), 6), "mi_loss": round(np.mean(ep_mi), 6),
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            ...
            tag = f"exp22_mi_{enc}_{bench}_fixed{FIXED_TOTAL}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            ...
            row = {
                ...
                "mi_weight": cfg.mi_weight, "temp": cfg.temp,
    ...
    with open("results/exp22_mi_results.json", "w") as f:
