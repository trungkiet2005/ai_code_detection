# exp72_tieh — Tree-Isometric Hyperbolic Embedding (TIEH)
# =============================================================================
# Theory-Track exp -- TIEH (Tree-Isometric Hyperbolic Embedding)
#
# ROLE           : Paradigm-different competitor to GENEPRINT.  Whereas GENEPRINT
#                  factorises into 3 EUCLIDEAN channels, TIEH embeds the WHOLE
#                  code representation into the Poincare ball B^d (d=64).
#                  Hyperbolic space has the geometric property that volume grows
#                  exponentially with radius -- exactly what a tree-structured
#                  label space needs.  We place CLASS PROTOTYPES p_k in B^d
#                  and constrain them via a TREE-ISOMETRY loss so that
#                    d_H(p_i, p_j) ~ scale * d_tree(i, j)   for all i, j.
#                  The encoder maps code to B^d; classification is by nearest
#                  prototype under Poincare distance.
#
#                  If TIEH beats GENEPRINT, the paradigm shift is hyperbolic.
#                  If GENEPRINT beats TIEH, disentangled-Euclidean is the
#                  right structure.  Either way the paper has a clean comparison.
# NAME           : TIEH  (Tree-Isometric Hyperbolic Embedding)
# ARXIV_ID       : extends Poincare embeddings (Nickel & Kiela 2017) to
#                  AI-code attribution; novel pairing is the LEARNED-but-iso-
#                  CONSTRAINED prototypes derived from the LLM genealogy tree.
# ONE-LINE CLAIM : AI-code genealogy is approximately hyperbolic; embedding
#                  encoder output into a Poincare ball with tree-isometric
#                  prototypes yields a one-channel attribution model that
#                  competes with disentangled multi-channel Euclidean.
# EQUATION       : z_E = phi(x) in R^d  (Euclidean head)
#                  z   = expmap0(z_E)   in B^d  (Poincare ball)
#                  p_k in B^d           (learnable prototype, ball-clipped)
#                  d_H(x, y) = acosh( 1 + 2 ||x-y||^2 /
#                                       ((1-||x||^2)(1-||y||^2)) )
#                  Classifier:  p(y=k | x) = softmax_k(-d_H(z, p_k) / tau)
#                  Loss:  L_ce  + lambda_iso * MSE( D_H(p) / scale,
#                                                   d_tree / D_max )
# WHY NOT BEFORE : Hyperbolic prototype networks exist but ours is the first
#                  to FORCE prototype geometry to match a known external tree
#                  (the LLM genealogy) -- the iso loss removes a hyperparameter
#                  and ties geometry to problem structure.
# FALSIFIER      : (F1) Spearman(d_H(p_i, p_j), d_tree(i, j)) after training.
#                       If < 0.8, isometry failed.
#                  (F2) Effective curvature: average ||p_k|| at end.  If
#                       < 0.2 (prototypes hugging origin), Euclidean would
#                       have worked equally well.
#                  (F3) Per-class test radius: mean ||z|| of test samples
#                       per class.  Family-uniform large radius indicates
#                       hyperbolic structure is being used.
# REPORTS        : Full eval pack + (F1) prototype Spearman
#                                 + (F2) mean prototype norm
#                                 + (F3) per-class mean z norm
# =============================================================================
...
logger = logging.getLogger("exp72_tieh")
...
def clip_ball(x):
    """Project to inside unit ball with margin."""
...
def expmap0(v):
    """Exponential map at origin: R^d -> B^d."""
    norm = v.norm(dim=-1, keepdim=True).clamp(min=EPS)
    return clip_ball(torch.tanh(norm) * v / norm)


def poincare_distance(x, y):
    """Pairwise Poincare distance.

    x: (N, d) or (B, d);   y: (M, d) or (B, d)
    Returns (N, M) or (B, B) distances.
    """
    ...
    denom = (1 - x_norm_sq).unsqueeze(1) * (1 - y_norm_sq).unsqueeze(0)  # (N, M)
...
class TIEHModel(nn.Module):
    def __init__(self, enc_name, n_cls, hyp_dim=64, tau=1.0):
        super().__init__()
        self.encoder = AutoModel.from_pretrained(os.path.join(KAGGLE_MODELS, enc_name), local_files_only=True)
        hidden = self.encoder.config.hidden_size
        self.proj = nn.Sequential(
            nn.Linear(hidden, 256), nn.GELU(), nn.Dropout(0.1),
            nn.Linear(256, hyp_dim)
        )
        # Prototypes: learnable but with norm-constraint via expmap of underlying R^d coords
        self.proto_E = nn.Parameter(torch.randn(n_cls, hyp_dim) * 0.1)
        self.tau = tau
        self.n_cls = n_cls
        self.hyp_dim = hyp_dim

    def get_prototypes(self):
        return expmap0(self.proto_E)                  # (n_cls, hyp_dim) in B^d

    def encode_hyp(self, ids, mask):
        out = self.encoder(input_ids=ids, attention_mask=mask)
        sem = (out.last_hidden_state * mask.unsqueeze(-1)).sum(1) / mask.sum(1, keepdim=True).clamp(min=1)
        z_E = self.proj(sem)
        z = expmap0(z_E)
        return z                                        # (B, hyp_dim) in B^d

    def forward(self, ids, mask):
        z = self.encode_hyp(ids, mask)
        protos = self.get_prototypes()                  # (n_cls, hyp_dim)
        # logits = -d_H(z, p) / tau   (closer = higher logit)
        d = poincare_distance(z, protos)                # (B, n_cls)
        return -d / self.tau, z, protos


def proto_iso_loss(protos, dist_mat, D_max=4.0, scale_per_unit=1.0):
    """MSE between pairwise hyperbolic distances of prototypes and tree distances."""
    n = protos.size(0)
    if n < 2: return protos.sum() * 0.0
    d_h = poincare_distance(protos, protos)              # (n, n)
    eye = torch.eye(n, device=protos.device, dtype=torch.bool)
    mask = ~eye
    target = dist_mat / D_max * scale_per_unit          # normalize tree distance
    target_max = max(target[mask].max().item(), 1e-6)
    d_h_norm = d_h / max(d_h[mask].max().item(), 1e-6)
    target_norm = target / target_max
    return F.mse_loss(d_h_norm[mask], target_norm[mask])
...
class Cfg:
    ...
    lr_enc: float = 2e-5; lr_head: float = 1e-4; lr_proto: float = 1e-3
    warmup: float = 0.1; wd: float = 0.01
    lambda_iso: float = 0.5
    hyp_dim: int = 64; tau: float = 1.0
...
def eval_pack(model, loader, cfg, sib_mask_np, dist_mat_cpu, collect_falsifier=False):
    ...
    for b in tqdm(loader, desc="Eval"):
        ...
        logits, z, _ = model(ids, mask)
        preds.extend(logits.argmax(dim=-1).cpu().tolist())
        labels.extend(labs.tolist() if torch.is_tensor(labs) else list(labs))
        if collect_falsifier:
            zn = z.norm(dim=-1).cpu().float().numpy()
            for li, n in zip(labs.tolist() if torch.is_tensor(labs) else list(labs), zn):
                z_norms_per_lab.setdefault(int(li), []).append(float(n))
    ...
    if collect_falsifier:
        # (F1) prototype Spearman vs tree
        with torch.no_grad():
            protos = model.get_prototypes().cpu().float()
            d_h = poincare_distance(protos, protos).cpu().numpy()
        n = n_cls
        tri = np.triu_indices(n, k=1)
        rho, _ = spearmanr(d_h[tri], dist_mat_cpu[tri])
        # (F2) mean prototype norm
        proto_norms = model.get_prototypes().norm(dim=-1).detach().cpu().float().numpy()
        # (F3) per-class mean z norm
        per_class_zn = {str(c): {"mean_z_norm": float(np.mean(v)) if v else 0.0,
                                  "std_z_norm": float(np.std(v)) if v else 0.0,
                                  "n": len(v)}
                        for c, v in z_norms_per_lab.items()}
        out["falsifier"] = {
            "proto_spearman_F1": float(rho) if not np.isnan(rho) else 0.0,
            "proto_norm_mean_F2": float(proto_norms.mean()),
            "proto_norm_max_F2": float(proto_norms.max()),
            "proto_norms_per_class_F2": proto_norms.tolist(),
            "per_class_z_norm_F3": per_class_zn,
        }
    return out


def train_epoch(model, loader, opt, sch, scaler, cfg, dist_mat):
    model.train(); tot, ce_s, iso_s = 0.0, 0.0, 0.0
    for b in tqdm(loader, desc="Train"):
        ids = b["ids"].to(cfg.device); mask = b["mask"].to(cfg.device); labs = b["label"].to(cfg.device)
        # Use full fp32 inside hyperbolic math for stability; bf16 only for encoder forward
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(cfg.device == "cuda")):
            out = model.encoder(input_ids=ids, attention_mask=mask)
            sem = (out.last_hidden_state * mask.unsqueeze(-1)).sum(1) / mask.sum(1, keepdim=True).clamp(min=1)
        # Move to fp32 from here onward (hyperbolic acosh / division on bf16 is unstable)
        sem32 = sem.float()
        z_E = model.proj(sem32)
        z = expmap0(z_E)
        protos = model.get_prototypes()
        d = poincare_distance(z, protos)
        logits = -d / model.tau
        loss_ce = F.cross_entropy(logits, labs)
        loss_iso = proto_iso_loss(protos, dist_mat)
        loss = loss_ce + cfg.lambda_iso * loss_iso
        scaler.scale(loss).backward()
        scaler.unscale_(opt)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        scaler.step(opt); scaler.update(); opt.zero_grad(); sch.step()
        tot += loss.item(); ce_s += loss_ce.item(); iso_s += loss_iso.item()
    n = len(loader)
    return tot/n, ce_s/n, iso_s/n
...
def run_exp(cfg, tag):
    ...
    logger.info(f"[sched] frac={cfg.frac} epochs={cfg.epochs} lr_enc={cfg.lr_enc} warmup={cfg.warmup} "
                f"lambda_iso={cfg.lambda_iso} hyp_dim={cfg.hyp_dim} tau={cfg.tau}")
    loader_cfg = dict(batch_size=cfg.bs, num_workers=4, pin_memory=True)
    tr_dl = DataLoader(tr_ds, shuffle=True, **loader_cfg)
    vl_dl = DataLoader(vl_ds, shuffle=False, **loader_cfg)
    ts_dl = DataLoader(ts_ds, shuffle=False, **loader_cfg)
    model = TIEHModel(cfg.enc, cfg.n_cls, cfg.hyp_dim, cfg.tau).to(cfg.device)
    enc_params = list(model.encoder.parameters())
    enc_ids = {id(p) for p in enc_params}
    proto_params = [model.proto_E]
    head_params = [p for n, p in model.named_parameters()
                   if id(p) not in enc_ids and not n.startswith("proto_E")]
    opt = torch.optim.AdamW([
        {"params": enc_params, "lr": cfg.lr_enc},
        {"params": head_params, "lr": cfg.lr_head},
        {"params": proto_params, "lr": cfg.lr_proto, "weight_decay": 0.0}], weight_decay=cfg.wd)
    ...
    for epoch in range(cfg.epochs):
        loss, ce, iso = train_epoch(model, tr_dl, opt, sch, scaler, cfg, dist_mat)
        val_met = eval_pack(model, vl_dl, cfg, sib_mask_np, dist_mat_cpu)
        v = val_met["overall"]["macro_f1"]; val_hist.append(v)
        with torch.no_grad():
            protos = model.get_prototypes()
            proto_norm = float(protos.norm(dim=-1).mean().item())
        logger.info(f"[epoch {epoch+1}] loss={loss:.4f} ce={ce:.4f} iso={iso:.4f} val={v:.4f} proto_norm={proto_norm:.3f}")
        if v > best_val:
            best_val = v
    ...
    logger.info(f"[final] val={best_val:.4f} test={test_macro:.4f} gap={gap:+.4f} "
                f"proto_rho={fa['proto_spearman_F1']:.3f} "
                f"proto_norm_mean={fa['proto_norm_mean_F2']:.3f}")
    return {"tag": tag, "method": "TIEH", "enc": cfg.enc, "bench": cfg.benchmark,
            "frac": cfg.frac, "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
            "lambda_iso": cfg.lambda_iso, "hyp_dim": cfg.hyp_dim, "tau": cfg.tau,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp72_tieh_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
                    ...
                    logger.info(f"[{tag}] test={res['macro']:.4f} ({res['dpaper']:+.4f}) "
                                ...
                                f"rho_proto={fa['proto_spearman_F1']:+.3f} "
                                f"|p|_mean={fa['proto_norm_mean_F2']:.3f} "
    ...
    with open(os.path.join(out_dir, "exp72_tieh_results.json"), "w") as f:
    ...
    for r in results:
        ...
        print(f"{r['enc']:<22} {r['bench']:<12} {r['frac']:>6.0%} {r['epochs']:>4d} "
              ...
              f"{fa['proto_spearman_F1']:>+7.3f} {fa['proto_norm_mean_F2']:>7.3f} {r['wall']:>8.0f}s")
    print("="*140)
