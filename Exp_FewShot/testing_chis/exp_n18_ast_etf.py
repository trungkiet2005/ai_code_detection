"""
exp_n18_ast_etf.py — ETF-Simplex + AST features

NAME : Equiangular Tight Frame with AST (ETF-AST)
ONE-LINE CLAIM: ETF simplex classifier with AST structural features
               for optimal K-class geometry + structural signal.
EQUATION : W_etf[i] = sqrt(K/(K-1)) * (e_i - 1/K * 1)
PROPERTY : ETF gives optimal K-class separation; AST adds structure signal.
WHY NOT BEFORE : Prior ETF uses only text; AST provides orthogonal signal.
FALSIFIER : If ETF+AST outperforms ETF text-only, AST is discriminative.

Self-contained. Runs: unixcoder-base × [codet_m4, aicd_t2] × [1%, 5%, 20%] = 6 runs.

Usage:
  python exp_n18_ast_etf.py
"""
...
class ETFNetAST(nn.Module):
    """ETF classifier with AST features."""
    ...
    def forward(self, ids, mask, ast_feat):
        ...
        # ETF classification
        logits_etf = proj @ self.W_etf.T * self.clf_scale
        logits_residual = self.clf_residual(proj)
        logits = logits_etf + 0.1 * logits_residual
    ...
    def groups(self):
        return [
            ...
            {"params": list(self.ast_encoder.parameters()) + list(self.proj.parameters()) +
                       list(self.clf_residual.parameters()) + [self.clf_scale],
...
def eval_m(model, loader, dev):
    ...
    for b in loader:
        ...
        labs = b["labels"]
        with torch.no_grad(), autocast(enabled=(dev.type == "cuda")):
...
def run_exp(cfg: Cfg, tag: str):
    cfg = _hw(cfg)
    logger.info(f"[exp_n18] {tag} | frac={cfg.frac}")
...
def main():
    ...
    for bench, task, n_cls in benchmarks:
        for frac in fracs:
            ...
            tag = f"exp_n18_ast_etf_{enc}_{bench}_f{frac:.2f}"
            try:
                r = run_exp(cfg, tag)
                results.append(r)
