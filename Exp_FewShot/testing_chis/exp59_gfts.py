# =============================================================================
# Theory-Track exp -- GFTS (Genealogy-Family Temperature Scaling)
#
# TRACKER INSIGHT: AICD-T2 (12 classes = 4 families x 3 generators) collapses
#                  at 1% across ALL methods (~0.19-0.21).  At 20% nothing breaks
#                  +0.025 vs CE.  The shared regime is "too many sibling pairs
#                  for one global temperature".  CoDET-M4 (6 classes, 1 family
#                  cluster) does not suffer this -- consistent with the diagnosis.
# NAME           : GFTS  (Genealogy-Family Temperature Scaling)
# ONE-LINE CLAIM : One softmax temperature per genealogy family.  Dense subtrees
#                  (lots of siblings) get a LOWER temperature => sharper margins
#                  between siblings; isolated classes get standard temperature.
# EQUATION       : tau_c = tau_0 * exp(beta * s_c)
#                  where s_c = (#siblings of c in tree) / max_siblings, beta < 0.
#                  p_y = softmax(z / tau)_y                     (per-sample tau)
#                  Loss = NLL with this tempered softmax.
# PROPERTY       : tau_c < 1 in dense families -> probabilities concentrate
#                  more sharply, gradient on hard sibling pairs grows.
#                  Reduces to vanilla CE when beta = 0.
#                  tau_0, beta are learnable (constrained: tau > 0 via softplus).
# WHY NOT BEFORE : Per-class temperature has been seen in calibration literature,
#                  but always learned post-hoc on a frozen model.  Here the
#                  temperature schedule is FIXED by genealogy TOPOLOGY (#siblings
#                  per node) and ONLY tau_0, beta are learned.
# FALSIFIER      : If GFTS helps AICD-T2 but not CoDET-M4, the gain is from the
#                  per-family parameterisation (independent of genealogy).  An
#                  ablation with shuffled #siblings would falsify the topology
#                  link.
# REPORTS        : val_macro, test_macro, val_test_gap.
# =============================================================================
...
logger = logging.getLogger("exp59_gfts")
...
def sibling_density_vector(n_cls, adj):
    """s_c = (#siblings of c) / max(#siblings).

    A class with no siblings has s_c = 0 -> tau_c = tau_0  (vanilla).
    The most-connected class has s_c = 1 -> tau_c = tau_0 * exp(beta).
    """
    counts = np.array([len(adj.get(c, [])) for c in range(n_cls)], dtype=np.float32)
    m = counts.max()
    if m <= 0:
        return torch.zeros(n_cls)
    return torch.tensor(counts / m, dtype=torch.float32)
...
# =============================================================================
# Model with learnable per-family temperature
# =============================================================================

class GFTSModel(nn.Module):
    def __init__(self, enc_name, n_cls, sibling_density, ast_dim=64):
        super().__init__()
        ...
        # learnable global params; sibling density vector is fixed
        self.register_buffer("sibling_density", sibling_density)
        self.log_tau0 = nn.Parameter(torch.tensor(0.0))   # exp(0) = 1 => start at vanilla
        self.beta = nn.Parameter(torch.tensor(-0.5))      # negative => denser families colder

    def family_temperature(self):
        # tau_c > 0 via exp; tau_c = exp(log_tau0 + beta * s_c)
        return torch.exp(self.log_tau0 + self.beta * self.sibling_density)  # (n_cls,)
    ...
    def forward(self, ids, mask, ast_feat):
        ...
        # Per-class temperature: divide each logit by its own tau.
        tau = self.family_temperature()                    # (n_cls,)
        return logits / tau.unsqueeze(0), proj             # (B, n_cls)


def gfts_loss(scaled_logits, labels):
...
class Cfg:
    ...
    lr_proj: float = 1e-4
    lr_head: float = 1e-4
    lr_temp: float = 1e-2     # temperature params learn quickly
    warmup: float = 0.1
    wd: float = 0.01
    device: str = "cuda"
...
def train_epoch(model, loader, opt, sch, scaler, cfg):
    ...
    for b in tqdm(loader, desc="Train"):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16,
            ...
            scaled_logits, _ = model(ids, mask, ast_feat)
            loss = gfts_loss(scaled_logits, labs)
...
def eval_model(model, loader, cfg):
    ...
    for b in tqdm(loader, desc="Eval"):
        ...
        labs = b["label"]
        scaled_logits, _ = model(ids, mask, ast_feat)
        preds.extend(scaled_logits.argmax(dim=-1).cpu().tolist())
...
def run_exp(cfg, tag):
    ...
    s_vec = sibling_density_vector(cfg.n_cls, cfg.gene_adj).to(cfg.device)
    logger.info(f"[gfts] sibling_density={s_vec.cpu().tolist()}")
    ...
    logger.info(
        ...
        f"lr_temp={cfg.lr_temp} warmup={cfg.warmup} total_steps={total_steps}"
    )
    ...
    model = GFTSModel(cfg.enc, cfg.n_cls, s_vec).to(cfg.device)
    temp_params = [model.log_tau0, model.beta]
    other_params = [p for n, p in model.named_parameters()
                    if n not in {"log_tau0", "beta"}]
    enc_params = [p for n, p in model.encoder.named_parameters()]
    ...
    opt = torch.optim.AdamW([
        ...
        {"params": temp_params, "lr": cfg.lr_temp, "weight_decay": 0.0},
    ], weight_decay=cfg.wd)
    ...
    for epoch in range(cfg.epochs):
        ...
        tau = model.family_temperature().detach().cpu().tolist()
        logger.info(
            f"[epoch {epoch+1}] loss={loss:.4f} val_macro={val_met['macro']:.4f} "
            f"tau0={model.log_tau0.exp().item():.3f} beta={model.beta.item():+.3f} "
            f"tau_min={min(tau):.3f} tau_max={max(tau):.3f}"
        )
    ...
    final_tau = model.family_temperature().detach().cpu().tolist()
    logger.info(
        f"[final] val={best_val:.4f}  test={ts_met['macro']:.4f}  gap={val_test_gap:+.4f}"
    )
    return {
        "tag": tag, "method": "GFTS", "enc": cfg.enc, "bench": cfg.benchmark,
        "frac": cfg.frac, "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
        "tau_per_class": final_tau,
        "log_tau0": model.log_tau0.item(), "beta": model.beta.item(),
        "val_macro": best_val, "macro": ts_met["macro"], "weighted": ts_met["weighted"],
        "acc": ts_met["acc"], "val_test_gap": val_test_gap,
        "dpaper": ts_met["macro"] - PAPER_BASELINE,
        "per_class_f1": ts_met["per_class"], "val_history": val_hist,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp59_gfts_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "exp59_gfts_results.json"), "w") as f:
