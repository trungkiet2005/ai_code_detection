...
# =============================================================================
# UPSTREAM    : "DA-MTL: Multi-task Learning for AI-generated Code Detection"
#               SecureComm 2025
# FAITHFULNESS: Shared RobertaModel encoder (CLS token), dual linear heads
#               (head1=binary, head2=K-class), sequential backward with
#               retain_graph=True, AdamW lr=1e-5. UniXcoder tokenisation
#               protocol (CLS <encoder_only> SEP ... SEP).
# BINARY LABEL: CoDET-M4: 0=human, 1=AI.
#               AICD-T2: label % 2 (even/odd grouping proxy; all samples are AI
#               so no true binary signal; preserves the DA-MTL training recipe).
# =============================================================================
...
class Cfg:
    ...
    lr_enc: float = 1e-5; warmup: float = 0.10; wd: float = 0.01
    device: str = "cuda"; gene_adj: dict = field(default_factory=dict)


def adaptive_schedule(cfg):
    f = cfg.frac
    if f <= 0.02:   cfg.epochs, cfg.lr_enc, cfg.warmup = 10, 1e-5, 0.20
    elif f <= 0.10: cfg.epochs, cfg.lr_enc, cfg.warmup = 6,  1e-5, 0.15
    else:           cfg.epochs, cfg.lr_enc, cfg.warmup = 5,  1e-5, 0.10
    return cfg
...
def _conv_codet(split, task, vocab):
    def row(r):
        ...
        binary_label = 0 if _is_human(r.get("target", "")) else 1
        return {"code": code, "label": label, "binary_label": binary_label,
...
def _tokenize(code, tokenizer, max_len):
    """UniXcoder tokenisation: CLS <encoder_only> SEP ... SEP (faithful to the
    upstream codebase tokenisation used by all exp7x–exp8x methods)."""
...
class FSDS(TD):
    """Few-shot dataset for DA-MTL.  Returns input_ids, label, binary_label,
    language, source so that eval_pack can compute per-language / per-source
    breakdowns."""
    ...
    def __getitem__(self, i):
        ...
        return {
            ...
            "label":        r["label"],
            "binary_label": r.get("binary_label", r["label"] % 2),
...
# =============================================================================
# Model — faithful to multi_supervised.py
# =============================================================================

class MultiTaskModel(nn.Module):
    """Shared RobertaModel encoder with two linear heads.
    head1: binary classification (2 classes).
    head2: K-class attribution.
    Faithful to DA-MTL multi_supervised.py: shared_model outputs
    last_hidden_state[:, 0, :] (CLS token)."""

    def __init__(self, encoder, hidden, n_cls_task1, n_cls_task2):
        super().__init__()
        self.encoder = encoder
        self.head1   = nn.Linear(hidden, n_cls_task1)
        self.head2   = nn.Linear(hidden, n_cls_task2)

    def forward(self, input_ids, attention_mask=None):
        out = self.encoder(input_ids, attention_mask=attention_mask)
        cls = out.last_hidden_state[:, 0, :]
        return self.head1(cls), self.head2(cls)
...
@torch.no_grad()
def eval_pack(model, loader, cfg, pad_id, sib_mask_np, dist_mat_cpu):
    model.eval()
    preds, labels, langs, sources = [], [], [], []
    for b in tqdm(loader, desc="Eval"):
        ids  = b["input_ids"].to(cfg.device)
        mask = ids.ne(pad_id)
        _, logits2 = model(ids, attention_mask=mask)
        preds_batch = logits2.argmax(dim=-1).cpu().tolist()
...
def train_epoch(model, loader, opt, sch, scaler, cfg, pad_id):
    model.train(); tot = 0.0
    for b in tqdm(loader, desc="Train"):
        ids   = b["input_ids"].to(cfg.device)
        mask  = ids.ne(pad_id)
        labs  = b["label"]
        blabs = b["binary_label"]
        if not torch.is_tensor(labs):  labs  = torch.tensor(labs,  dtype=torch.long)
        if not torch.is_tensor(blabs): blabs = torch.tensor(blabs, dtype=torch.long)
        labs  = labs.to(cfg.device)
        blabs = blabs.to(cfg.device)
        opt.zero_grad()
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16,
                            enabled=(cfg.device == "cuda")):
            logits1, logits2 = model(ids, attention_mask=mask)
            loss1 = F.cross_entropy(logits1, blabs)
            loss2 = F.cross_entropy(logits2, labs)
        # Sequential backward — faithful to multi_supervised.py fine_tune_multi_task_model
        scaler.scale(loss1).backward(retain_graph=True)
        scaler.scale(loss2).backward()
        scaler.unscale_(opt)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        scaler.step(opt); scaler.update(); sch.step()
        tot += loss1.item() + loss2.item()
...
def run_exp(cfg, tag):
    ...
    model   = MultiTaskModel(encoder, config.hidden_size, 2, cfg.n_cls).to(cfg.device)
    ...
    for ep in range(cfg.epochs):
        loss = train_epoch(model, tr_dl, opt, sch, scaler, cfg, pad_id)
        val_met = eval_pack(model, vl_dl, cfg, pad_id, sib_mask_np, dist_mat_cpu)
    ...
    ts_met     = eval_pack(model, ts_dl, cfg, pad_id, sib_mask_np, dist_mat_cpu)
    ...
    return {
        "tag":          tag,
        "method":       "DA-MTL-K",
        "upstream":     "SecureComm 2025",
        "note":         ("Dual-head MTL, sequential backward(retain_graph=True). "
                         "head1=binary (CoDET: human/AI; AICD: label%2). "
                         "head2=K-class attribution."),
        "enc":          cfg.enc,
        "bench":        cfg.benchmark,
        "frac":         cfg.frac,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"ext_damtl_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "ext_damtl_results.json"), "w") as f:
