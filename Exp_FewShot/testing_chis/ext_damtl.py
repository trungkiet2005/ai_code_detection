...
# FAITHFULNESS: Shared encoder + dual heads, sequential backward(retain_graph=True)
#   Task1=binary proxy (even/odd label grouping since no human data), Task2=K-class
# =============================================================================
...
logger=logging.getLogger("ext_damtl"); PAPER_BASELINE=0.6633

@dataclass
class Cfg:
    benchmark:str="codet_m4";task:str="author";enc:str="unixcoder-base"
    frac:float=0.20;n_cls:int=6;seed:int=42;bs:int=64;seq:int=512;epochs:int=6
    lr_enc:float=1e-5;warmup:float=0.10;wd:float=0.01;device:str="cuda"
...
class FSDS(TD):
    ...
    def __getitem__(self,i):
        ...
        label=r["label"]
        return {"input_ids":torch.tensor(ids,dtype=torch.long),"label":label,"binary_label":label%2}

class MultiTaskModel(nn.Module):
    """Faithful to multi_supervised.py: shared encoder + dual linear heads."""
    def __init__(self,encoder,hidden,n_cls_task1,n_cls_task2):
        super().__init__();self.encoder=encoder;self.head1=nn.Linear(hidden,n_cls_task1);self.head2=nn.Linear(hidden,n_cls_task2)
    def forward(self,input_ids,attention_mask=None):
        out=self.encoder(input_ids,attention_mask=attention_mask)
        cls=out[0][:,0,:]  # CLS token (faithful to multi_supervised.py)
...
def eval_pack(model,loader,cfg):
    ...
    for b in tqdm(loader,desc="Eval"):
        ids=b["input_ids"].to(cfg.device);mask=ids.ne(0)
        _,logits2=model(ids,attention_mask=mask)
        preds.extend(logits2.argmax(-1).cpu().tolist())
...
def run_exp(cfg,tag):
    ...
    for ep in range(cfg.epochs):
        ...
        for b in tqdm(tr_dl,desc=f"Train ep{ep+1}"):
            ids=b["input_ids"].to(cfg.device);mask=ids.ne(0)
            labs=b["label"];blabs=b["binary_label"]
            ...
            labs=labs.to(cfg.device);blabs=blabs.to(cfg.device)
            opt.zero_grad()
            with torch.autocast(device_type="cuda",dtype=torch.bfloat16,enabled=(cfg.device=="cuda")):
                logits1,logits2=model(ids,attention_mask=mask)
                # Sequential backward: faithful to multi_supervised.py
            ...
            scaler.step(opt);scaler.update();sch.step();tot+=(loss1.item()+loss2.item())
    ...
    return {"tag":tag,"method":"DA-MTL-K","upstream":"SecureComm 2025",
            "note":"Dual-head MTL, sequential backward(retain_graph=True). Task1=binary proxy (even/odd label).",
            "enc":cfg.enc,"bench":cfg.benchmark,"frac":cfg.frac,"epochs":cfg.epochs,"lr_enc":cfg.lr_enc,
...
def main():
    ...
    for bench,task,n_cls in [("codet_m4","author",6),("aicd_t2","t2",12)]:
        for frac in [0.01,0.05,0.20]:
            cfg=Cfg(benchmark=bench,task=task,frac=frac,n_cls=n_cls);tag=f"ext_damtl_{cfg.enc}_{bench}_f{frac}"
    ...
    with open(os.path.join(out_dir,"ext_damtl_results.json"),"w") as f:json.dump(results,f,indent=2)
    print("\n"+"="*100)
