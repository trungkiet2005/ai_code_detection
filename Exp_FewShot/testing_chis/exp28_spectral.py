...
logger = logging.getLogger("exp28")
...
class Cfg:
    ...
    # SCL specific
    scl_weight: float = 0.5  # Weight for spectral contrastive loss
    temp: float = 0.07  # Temperature for contrastive loss
...
class SCLNet(nn.Module):
    """Encoder with spectral contrastive learning."""
    ...
    def forward(self, ids, mask):
        ...
        # Spectral contrastive embedding (L2 normalized)
        scl_emb = self.scl_head(pooled)
        scl_emb = F.normalize(scl_emb, dim=1)

        logits = self.head(pooled)
        return {"logits": logits, "scl_emb": scl_emb}


def spectral_contrastive_loss(emb, labels, temp=0.07):
    """Spectral contrastive loss: InfoNCE on same-class pairs."""
    n = emb.size(0)
    ...
    for ep in range(cfg.epochs):
        ...
        for b in pbar:
            ...
            with _autocast_ctx(dev):
                ...
                loss_scl = spectral_contrastive_loss(scl_emb, labs, cfg.temp)
                loss = loss_ce + cfg.scl_weight * loss_scl

            ep_ce.append(loss_ce.item())
            ep_scl.append(loss_scl.item())
            ...
            sched.step()
            pbar.set_postfix({"ce": f"{loss_ce.item():.3f}", "scl": f"{loss_scl.item():.3f}"})

        tr_met = eval_m(model, tr_dl, dev)
        vr = eval_m(model, vl_dl, dev)
        train_history.append({
            "epoch": ep + 1, "loss": round(np.mean(ep_loss), 6),
            "ce_loss": round(np.mean(ep_ce), 6), "scl_loss": round(np.mean(ep_scl), 6),
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            ...
            tag = f"exp28_scl_{enc}_{bench}_fixed{FIXED_TOTAL}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            ...
            row = {
                ...
                "scl_weight": cfg.scl_weight, "temp": cfg.temp,
    ...
    with open("results/exp28_scl_results.json", "w") as f:
