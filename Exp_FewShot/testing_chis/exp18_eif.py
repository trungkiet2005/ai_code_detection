"""
================================================================================
Theory-Track exp -- Energy-Based Identity Factorization (EIF):
Energy-based models with identity factorization for few-shot attribution.

ARXIV_ID      : ICLR 2021 EBM (2011.08891); NeurIPS 2020 CvG (2006.07710)
NAME          : Energy-Based Identity Factorization (EIF)
ONE-LINE CLAIM: Modeling generator identity as an energy function decomposes
                attribution into disentangled factors, improving few-shot generalization.
EQUATION      : E(x, y) = f_θ(x)^T W_y + b_y
                L_contrastive = exp(-E(x, y)) / (exp(-E(x, y)) + Σ_{y'≠y} exp(-E(x, y')))
PROPERTY      : The energy landscape smooths class boundaries, making attribution
                more robust to distribution shift between train and test.
WHY NOT BEFORE: Standard classifiers output hard probabilities; EIF outputs energies
                that can be compared across classes without normalization.
FALSIFIER     : If OOD test accuracy does NOT improve relative to IID accuracy,
                the energy-based approach is not providing generalization benefits.
================================================================================

exp18_eif.py — EIF-based few-shot AI-code attribution.
...
class Cfg:
    ...
    # EIF specific
    energy_margin: float = 1.0  # Margin for contrastive energy loss
...
class EIFNet(nn.Module):
    """Energy-Based Identity Factorization network."""
    def __init__(self, cfg: Cfg):
        ...
        # Energy function: f_θ(x)^T W_y + b_y
        self.energy_weight = nn.Linear(hidden, cfg.n_cls, bias=False)
        self.energy_bias = nn.Parameter(torch.zeros(cfg.n_cls))

    def energy(self, x):
        """Compute energy for each class."""
        return self.energy_weight(x) + self.energy_bias

    def forward(self, ids, mask):
        out = self.encoder(input_ids=ids, attention_mask=mask)
        pooled = out.last_hidden_state[:, 0]
        energies = self.energy(pooled)
        return {"logits": -energies, "energies": energies, "pooled": pooled}


def energy_contrastive_loss(energies, labels, margin=1.0):
    """
    Contrastive energy loss: push correct class energy down,
    push wrong classes energy up.
    """
    n = energies.size(0)
    correct_energy = energies.gather(1, labels.unsqueeze(1)).squeeze(1)
    wrong_energy = energies.scatter(1, labels.unsqueeze(1), float('-inf'))
    hardest_wrong_energy, _ = wrong_energy.max(dim=1)

    # Margin ranking loss
    loss = F.relu(correct_energy - hardest_wrong_energy + margin)
    return loss.mean()
    ...
    for ep in range(cfg.epochs):
        ...
        for b in pbar:
            ...
            with _autocast_ctx(dev):
                ...
                loss_energy = energy_contrastive_loss(energies, labs, cfg.energy_margin)

                # Combined: CE + energy regularization
                loss = loss_ce + 0.1 * loss_energy

            ep_ce.append(loss_ce.item())
            ep_energy.append(loss_energy.item())
            ...
            sched.step()
            pbar.set_postfix({"ce": f"{loss_ce.item():.3f}", "E": f"{loss_energy.item():.3f}"})

        tr_met = eval_m(model, tr_dl, dev)
        vr = eval_m(model, vl_dl, dev)
        train_history.append({
            "epoch": ep + 1, "loss": round(np.mean(ep_loss), 6),
            "ce_loss": round(np.mean(ep_ce), 6), "energy_loss": round(np.mean(ep_energy), 6),
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            ...
            tag = f"exp18_eif_{enc}_{bench}_fixed{FIXED_TOTAL}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            ...
            row = {
                ...
                "energy_margin": cfg.energy_margin,
    ...
    with open("results/exp18_eif_results.json", "w") as f:
