# exp70_decofp — Decoding-fingerprint regressor + conditional attribution (DECOFP)
# =============================================================================
# Theory-Track exp -- DECOFP (Decoding-Fingerprint Regressor)
#
# ROLE           : S2 says: LLM decoding parameters (temperature, top-p,
#                  repetition penalty) leak into generated code.  We do NOT
#                  have ground-truth decoding metadata, so we use three
#                  COMPUTABLE SURROGATES:
#                    rep_entropy(x)   = entropy of 4-gram-repetition distribution
#                                       (low T -> more repetition -> low entropy)
#                    vocab_richness(x)= type-token ratio over identifiers
#                                       (high T -> more variety -> high TTR)
#                    burst_score(x)   = std of token frequencies across windows
#                                       (correlates with top-p)
#                  An auxiliary regressor predicts this 3-vector from the
#                  encoder embedding; the classifier is conditioned on
#                  [z ; predicted_fingerprint].  Multi-task supervision forces
#                  the encoder to ENCODE the decoding-style channel
#                  explicitly.
# NAME           : DECOFP  (Decoding-Fingerprint conditioning)
# ARXIV_ID       : novel for code; closest is Binoculars (text) which uses
#                  cross-perplexity but not decoding-parameter regression.
# EQUATION       : z = phi(x)
#                  d_hat = MLP_d(z)   in R^3 (predicted decoding surrogates)
#                  d_true = surrogate(x) computed from raw code
#                  L = CE(W [z; d_hat], y)  +  lambda_d * MSE(d_hat, d_true)
# WHY NOT BEFORE : Existing attribution treats decoding randomness as noise.
#                  DECOFP treats it as a SIGNAL: the decoder forces the
#                  encoder to predict computable decoding-stats, and the
#                  classifier sees the predicted fingerprint explicitly.
# FALSIFIER      : If the per-class mean of d_hat is identical across all
#                  classes (Bhattacharyya distance between class-conditional
#                  fingerprint distributions ~ 0), the fingerprint channel is
#                  not discriminating authors and DECOFP collapses to plain
#                  CE.  Reported: per-class fingerprint mean + spread.
# REPORTS        : Full eval pack + per-class fingerprint (mean & std).
# =============================================================================
...
logger = logging.getLogger("exp70_decofp")
...
def extract_decoding_surrogate(code: str) -> List[float]:
    """Three computable surrogates for decoding-parameter fingerprint.

    Returns [rep_entropy, vocab_richness, burst_score] each roughly in [0, 1].
    """
    tokens = _re.findall(r"\w+|[^\w\s]", code)
    n = max(len(tokens), 1)
    # 1) rep_entropy: entropy of 4-gram distribution (low = repetitive = low T)
    if n >= 4:
        ...
        rep_entropy = 0.0
    # 2) vocab_richness: type-token ratio of identifiers (high = more diverse)
    identifiers = _re.findall(r"\b[a-zA-Z_]\w*\b", code)
    ttr = len(set(identifiers)) / max(len(identifiers), 1)
    # 3) burst_score: std of token frequencies across non-overlapping 50-token windows
    if n >= 100:
        ws = 50
        n_w = n // ws
        freqs = []
        for w in range(n_w):
            chunk = tokens[w*ws:(w+1)*ws]
            c = Counter(chunk)
            # Use mean frequency of top-3 tokens in window as a "burst" proxy
        ...
        burst = 0.0
    return [
        min(max(rep_entropy, 0.0), 1.0),
        min(max(ttr, 0.0), 1.0),
        min(max(burst * 5.0, 0.0), 1.0),  # rescale burst into ~[0, 1]
    ]
...
class DECOFPModel(nn.Module):
    def __init__(self, enc_name, n_cls, ast_dim=64, z_dim=256, dec_dim=3):
        super().__init__()
        ...
        self.decoder_regressor = nn.Sequential(
            nn.Linear(z_dim, 128), nn.GELU(),
            nn.Linear(128, dec_dim), nn.Sigmoid()  # outputs in [0, 1]
        )
        self.clf = nn.Linear(z_dim + dec_dim, n_cls)
    ...
    def forward(self, ids, mask, ast_feat):
        ...
        d_hat = self.decoder_regressor(z)
        logits = self.clf(torch.cat([z, d_hat], dim=-1))
        return logits, d_hat
...
@torch.no_grad()
def eval_pack(model, loader, cfg, sib_mask_np, collect_fingerprint=False):
    ...
    for b in tqdm(loader, desc="Eval"):
        ...
        logits, d_hat = model(ids, mask, ast_feat)
        preds.extend(logits.argmax(dim=-1).cpu().tolist())
        labels.extend(labs.tolist() if torch.is_tensor(labs) else list(labs))
        if collect_fingerprint:
            dec_hats.append(d_hat.detach().cpu().float().numpy())
            dec_labels.extend(labs.tolist() if torch.is_tensor(labs) else list(labs))
    ...
    if collect_fingerprint and dec_hats:
        D_arr = np.concatenate(dec_hats, axis=0)
        labels_arr = np.array(dec_labels)
        per_class_fp = {}
        for c in range(n_cls):
            sel = (labels_arr == c)
            if sel.sum() < 2: continue
            per_class_fp[str(c)] = {
                "mean": D_arr[sel].mean(axis=0).tolist(),
                "std": D_arr[sel].std(axis=0).tolist(),
                "n": int(sel.sum()),
            }
        out["fingerprint_per_class"] = per_class_fp
    return out


def train_epoch(model, loader, opt, sch, scaler, cfg):
    model.train(); tot, ce_sum, dec_sum = 0.0, 0.0, 0.0
    for b in tqdm(loader, desc="Train"):
        ids = b["ids"].to(cfg.device); mask = b["mask"].to(cfg.device)
        ast_feat = b["ast_feat"].to(cfg.device); labs = b["label"].to(cfg.device)
        dec_true = b["dec_true"].to(cfg.device)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(cfg.device == "cuda")):
            logits, d_hat = model(ids, mask, ast_feat)
            loss_ce = F.cross_entropy(logits, labs)
            loss_dec = F.mse_loss(d_hat, dec_true)
            loss = loss_ce + cfg.lambda_d * loss_dec
        scaler.scale(loss).backward()
        scaler.unscale_(opt)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        scaler.step(opt); scaler.update(); opt.zero_grad(); sch.step()
        tot += loss.item(); ce_sum += loss_ce.item(); dec_sum += loss_dec.item()
    n = len(loader)
    return tot/n, ce_sum/n, dec_sum/n
...
def run_exp(cfg, tag):
    ...
    logger.info(f"[sched] frac={cfg.frac} epochs={cfg.epochs} lr_enc={cfg.lr_enc} warmup={cfg.warmup} lambda_d={cfg.lambda_d}")
    loader_cfg = dict(batch_size=cfg.bs, num_workers=4, pin_memory=True)
    tr_dl = DataLoader(tr_ds, shuffle=True, **loader_cfg)
    vl_dl = DataLoader(vl_ds, shuffle=False, **loader_cfg)
    ts_dl = DataLoader(ts_ds, shuffle=False, **loader_cfg)
    model = DECOFPModel(cfg.enc, cfg.n_cls).to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        loss, ce, dec = train_epoch(model, tr_dl, opt, sch, scaler, cfg)
        val_met = eval_pack(model, vl_dl, cfg, sib_mask_np)
        v = val_met["overall"]["macro_f1"]; val_hist.append(v)
        logger.info(f"[epoch {epoch+1}] loss={loss:.4f} ce={ce:.4f} dec_mse={dec:.4f} val={v:.4f}")
        if v > best_val:
            best_val = v
            best_state = {k: v_.cpu().clone() for k, v_ in model.state_dict().items()}
    model.load_state_dict(best_state)
    ts_met = eval_pack(model, ts_dl, cfg, sib_mask_np, collect_fingerprint=True)
    ...
    return {"tag": tag, "method": "DECOFP", "enc": cfg.enc, "bench": cfg.benchmark,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp70_decofp_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "exp70_decofp_results.json"), "w") as f:
