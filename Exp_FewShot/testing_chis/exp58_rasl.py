# =============================================================================
# Theory-Track exp -- RASL (Residual-Aware Sibling Loss)
#
# TRACKER INSIGHT: GRA (exp45) wins +0.0152 at 20% CoDET-M4 via the genealogical
#                  residual R(x) = AST(x) - E[AST | gene(x)].
#                  SSL (exp42) wins +0.0163 via genealogy-weighted CE.
#                  Both winners exploit the SAME tree but on different surfaces
#                  (AST residual space vs. logit-error space).
#                  RASL couples them under a single residual-pair objective.
# NAME           : RASL  (Residual-Aware Sibling Loss)
# ONE-LINE CLAIM : Sibling pairs should share the same intra-family residual
#                  pattern in AST space; cross-family pairs are free.
# EQUATION       : R(x) = AST(x) - mu_gene(y(x))               (running mean)
#                  L_rasl = E_{(i,j)} [ w_tree(y_i, y_j) * ||R(x_i) - R(x_j)||^2 ]
#                  where w_tree = +1 for siblings, 0 elsewhere (pull only).
#                  Total: L = CE + lambda * L_rasl
# PROPERTY       : (i) Reduces to identity when y_i, y_j not siblings (zero grad
#                  from RASL term -- no spurious push).
#                  (ii) Equivalent to a tree-conditional variance penalty on R.
# WHY NOT BEFORE : Generic sibling-pull (contrastive) loss is class-agnostic;
#                  RASL operates on the AST RESIDUAL, which is only defined when
#                  the label space carries genealogy (so mu_gene is well-defined).
# FALSIFIER      : If RASL helps CoDET-M4 (6-class, dense subtrees) but not
#                  AICD-T2 (12-class, 4 disjoint families), the residual-pair
#                  structure is exploited only when siblings are abundant.
...
logger = logging.getLogger("exp58_rasl")
...
class RASLModel(nn.Module):
    """Encoder + classifier + ast residualiser.

    `self.gene_mean[c]` is a running estimate of E[AST_proj | y = c],
    updated with EMA in `train_epoch`.  The residual is AST_proj - gene_mean[y].
    """
    def __init__(self, enc_name, n_cls, ast_proj_dim=64):
        super().__init__()
        ...
        self.n_cls = n_cls
        self.ast_proj_dim = ast_proj_dim

        self.ast_encoder = nn.Sequential(
            nn.Linear(64, 128), nn.GELU(), nn.Linear(128, ast_proj_dim)
        )
        self.proj = nn.Sequential(
            nn.Linear(hidden + ast_proj_dim, 256), nn.GELU(), nn.Dropout(0.1)
        )
        self.clf = nn.Linear(256, n_cls)

        # Running family mean of ast_proj (no grad, updated with EMA).
        self.register_buffer("gene_mean", torch.zeros(n_cls, ast_proj_dim))
        self.register_buffer("gene_count", torch.zeros(n_cls))
...
def rasl_loss(ast_emb, labels, gene_mean, sibling_mask):
    """L_rasl = mean over sibling pairs (i, j) of ||R_i - R_j||^2.

    R_i = ast_emb[i] - gene_mean[labels[i]]   (gradient flows through ast_emb)
    sibling_mask[a, b] in {0, 1}.

    Implementation: pairwise within-batch via outer subtraction.  O(B^2 * D).
    """
    B, D = ast_emb.shape
    if B < 2:
        return ast_emb.new_zeros(())
    gm = gene_mean[labels].detach()
    R = ast_emb - gm                              # (B, D)
    diff = R.unsqueeze(0) - R.unsqueeze(1)        # (B, B, D)
    sq = (diff * diff).sum(-1)                    # (B, B)
    W = sibling_mask[labels][:, labels]           # (B, B)
    denom = W.sum().clamp_min(1.0)
    return (W * sq).sum() / denom


@torch.no_grad()
def update_gene_mean(model, ast_emb, labels, momentum=0.9):
    """EMA update of model.gene_mean per class."""
    for c in labels.unique().tolist():
        mask = (labels == c)
        if mask.sum() == 0:
            continue
        m_c = ast_emb[mask].mean(0)
        old = model.gene_mean[c]
        model.gene_mean[c] = momentum * old + (1 - momentum) * m_c
        model.gene_count[c] += mask.sum().float()
...
def train_epoch(model, loader, opt, sch, scaler, cfg, sibling_mask):
    model.train()
    total_ce, total_rasl = 0.0, 0.0
    for b in tqdm(loader, desc="Train"):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16,
            ...
            logits, ast_emb = model(ids, mask, ast_feat)
            loss_ce = F.cross_entropy(logits, labs)
            loss_rasl = rasl_loss(ast_emb.float(), labs, model.gene_mean, sibling_mask)
            loss = loss_ce + cfg.lambda_rasl * loss_rasl
        ...
        update_gene_mean(model, ast_emb.detach().float(), labs, momentum=cfg.ema_momentum)
        total_ce += loss_ce.item()
        total_rasl += loss_rasl.item() if torch.is_tensor(loss_rasl) else float(loss_rasl)

    n = len(loader)
    return total_ce / n, total_rasl / n
...
def run_exp(cfg, tag):
    ...
    sibling_mask = build_sibling_mask(cfg.n_cls, cfg.gene_adj).to(cfg.device)
    ...
    logger.info(
        ...
        f"warmup={cfg.warmup} lambda={cfg.lambda_rasl} ema={cfg.ema_momentum} "
    ...
    model = RASLModel(cfg.enc, cfg.n_cls).to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        loss_ce, loss_rasl = train_epoch(model, tr_dl, opt, sch, scaler, cfg, sibling_mask)
        val_met = eval_model(model, vl_dl, cfg)
        val_hist.append(val_met["macro"])
        logger.info(
            f"[epoch {epoch+1}] ce={loss_ce:.4f} rasl={loss_rasl:.4f} "
    ...
    return {
        "tag": tag, "method": "RASL", "enc": cfg.enc, "bench": cfg.benchmark,
        "frac": cfg.frac, "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
        "lambda_rasl": cfg.lambda_rasl, "ema_momentum": cfg.ema_momentum,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp58_rasl_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "exp58_rasl_results.json"), "w") as f:
