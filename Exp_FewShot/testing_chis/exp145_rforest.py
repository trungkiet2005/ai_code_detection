# exp145 — RFOREST
# =============================================================================
# NAME       : RFOREST (Random Forest on 50+ stylometric features for code authorship)
# REFERENCE  : Breiman 2001 ("Random Forests", Machine Learning 45:5-32).
#              Brennan & Greenstadt 2009 ("Practical attacks against
#              authorship recognition techniques"). Caliskan et al. 2015
#              ("De-anonymizing Programmers via Code Stylometry", USENIX
#              Security) — used stylometric features for PROGRAMMER
#              attribution but for HUMAN code, never LLM-generated code.
# CLAIM      : The most honest classical-ML baseline. Extract 50+ hand-
#              crafted stylometric features (line length distribution,
#              indent depth, identifier patterns, operator usage,
#              comment density, AST proxy features) and feed into a
#              Random Forest (200 trees, default depth). This is the
#              "what would a 2010 stylometrician do?" baseline that
#              every authorship paper needs but few report.
# EQUATION   : phi(x) = [
#                line_length_{mean, std, max, min, q25, q75},
#                indent_depth_{mean, max, change_rate},
#                identifier_length_{mean, std},
#                operator_count_{=, +, -, *, ==, !=, etc} normalized,
#                keyword_density_{if, for, while, return, def, ...},
#                comment_{count, density, avg_length},
#                punctuation_density,
#                whitespace_to_alpha_ratio,
#                camelCase_vs_snake_case_ratio,
#                ... ~50 features ...
#              ] in R^50
#              RF: 200 trees, sklearn defaults, fit on phi -> y.
#              y_hat = RF.predict(phi(query))
# WHY NEW    : Caliskan 2015 used stylometric features for HUMAN code
#              attribution (5 humans, GCJ). RFOREST extends this to
#              LLM-generated code attribution with a comparable but
#              broader feature set — first comprehensive stylometric
#              baseline for AI code attribution.
# WOW HOOK   : "A 2010 stylometrician with 50 hand-crafted features and a
#              random forest catches LLM authors. We test whether the
#              field's neural methods are recapitulating what humans
#              could engineer 15 years ago."
# FALSIFIER  : (F1) RFOREST composite > GZIP-NCD baseline by >= 0.02 (hand-
#              crafted features beat parameter-free).
#              (F2) Top-10 feature importance via RF.feature_importances_:
#              the most-important features should make INTERPRETABLE sense
#              (e.g., comment density, line length).
#              (F3) Per-feature ablation: removing top-5 features should
#              cost >= 0.01 in composite — those features are LOAD-BEARING.
# =============================================================================
...
logger = logging.getLogger("exp145_rforest")
...
def extract_features(code):
    """Return ~50-dim feature vector (matches FEATURE_NAMES order)."""
    code = code[:8000]
    lines = code.split("\n")
...
def extract_features_parallel(codes, n_workers=None, desc="feat"):
    """Extract features for a list of code snippets using multiprocessing."""
...
class Cfg:
    ...
    max_chars: int = 4000
    # Ablation: how many top features to remove for F3
...
def run_exp(cfg, tag):
    ...
    # Replace any nan/inf for safety
    for X in (X_train, X_val, X_test):
        np.nan_to_num(X, copy=False, nan=0.0, posinf=0.0, neginf=0.0)

    # Train RF
    t0 = time.time()
    rf = train_rf(X_train, train_labels,
                  n_estimators=cfg.n_estimators, max_depth=cfg.max_depth,
                  n_jobs=n_workers, seed=cfg.seed)
    train_time = time.time() - t0
    logger.info(f"[rf] trained {cfg.n_estimators} trees in {train_time:.0f}s")
    ...
    # F2: top-10 feature importance
    importances = rf.feature_importances_
    order = np.argsort(importances)[::-1]
    top10 = [(FEATURE_NAMES[i], float(importances[i])) for i in order[:10]]
    logger.info(f"[F2] top-10 importance: " +
                ", ".join(f"{n}={v:.3f}" for n, v in top10[:5]) + " ...")

    # F3: per-feature ablation — remove top-k features (mask to zero in train+test),
    # retrain, evaluate.
    top_k_idx = order[:cfg.ablation_top_k].tolist()
    X_train_abl = X_train.copy(); X_train_abl[:, top_k_idx] = 0.0
    X_test_abl  = X_test.copy();  X_test_abl[:,  top_k_idx] = 0.0
    rf_abl = train_rf(X_train_abl, train_labels,
                      n_estimators=cfg.n_estimators, max_depth=cfg.max_depth,
                      n_jobs=n_workers, seed=cfg.seed)
    abl_preds = rf_abl.predict(X_test_abl)
    abl_macro = float(f1_score(test_labels, abl_preds, average="macro", zero_division=0))
    abl_drop = test_macro - abl_macro
    logger.info(f"[F3] ablation top-{cfg.ablation_top_k} removed: "
                f"macro={abl_macro:.4f} (drop={abl_drop:+.4f})")

    ts_met["falsifier_F2_top10_feature_importance"] = top10
    ts_met["falsifier_F3_ablation_top_k_removed"]   = int(cfg.ablation_top_k)
    ts_met["falsifier_F3_ablation_macro"]           = abl_macro
    ts_met["falsifier_F3_ablation_drop"]            = abl_drop

    return {
        "tag": tag, "method": "RFOREST",
        "note": ("Random Forest on 50+ hand-crafted stylometric features. "
                 "Classical-ML baseline; no neural encoder; CPU-only. "
                 "Inspired by Caliskan 2015 stylometry."),
        ...
        "top10_feature_importance": top10,
        "ablation_top5_removed_macro": abl_macro,
        "ablation_top5_drop": abl_drop,
        "comparison_vs_gzip_ncd_macro": None,
...
def main():
    ...
    for bench, task, n_cls in benchmarks:
        for frac in fracs:
            ...
            tag = f"exp145_rforest_{bench}_f{frac}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            try:
                res = run_exp(cfg, tag)
                ...
                logger.info(f"[{tag}] test={res['macro']:.4f} ({res['dpaper']:+.4f}) "
                            ...
                            f"abl_drop={res['ablation_top5_drop']:+.4f}")
            except Exception as e:
    ...
    with open(os.path.join(out_dir, "exp145_rforest_results.json"), "w") as f:
        json.dump(results, f, indent=2)
    print("\n" + "="*140)
    print(f"{'Benchmark':<12} {'Frac':>6} {'Trees':>6} {'Train':>8} {'Test':>8} "
          f"{'Val-F1':>8} {'Test-F1':>8} {'Gap':>8} {'dPaper':>9} "
          f"{'AblMacro':>10} {'AblDrop':>10} {'Wall':>8}")
    print("-"*140)
    for r in results:
        print(f"{r['bench']:<12} {r['frac']:>6.0%} {r['n_estimators']:>6d} "
              f"{r['train_size_after_cap']:>8d} {r['test_size_after_cap']:>8d} "
              f"{r['val_macro']:>8.4f} {r['macro']:>8.4f} {r['val_test_gap']:>+8.4f} "
              f"{r['dpaper']:>+9.4f} {r['ablation_top5_removed_macro']:>10.4f} "
              f"{r['ablation_top5_drop']:>+10.4f} {r['wall']:>8.0f}s")
    print("="*140)
