# =============================================================================
# Theory-Track exp -- HUMCLUST (Intra-Human Substructure)
#
# EXPLOITS       : S10 (human is not a flat class).  In CoDET-M4, "human" is
#                  ~half the data and aggregates many writing styles (skill
#                  levels, languages, sources).  A single class label for
#                  human forces the network to memorise this mixture as one
#                  prototype, leaking discriminative capacity that could be
#                  spent on the smaller model classes (1-5).
# NAME           : HUMCLUST  (Intra-Human Substructure)
# ONE-LINE CLAIM : Decomposing the human class into K substructure clusters
#                  during training (then collapsing back at inference) gives
#                  the model the flexibility to fit human variance without
#                  bleeding it onto sibling model classes.
# EQUATION       : K_h = 4 substructure clusters for human (class 0).
#                  Pre-train: 1-epoch CE-only on (encoder + AST + classifier)
#                             with original n_cls labels.
#                  Cluster:   k-means(K_h) on the AST 22-feature vector of
#                             human samples only.  Assign each human sample
#                             a sub-label in {0..K_h-1}.
#                  Re-label:  new class space has (K_h + n_models - 1) labels:
#                               0..K_h-1   : human sub-clusters
#                               K_h..end   : original model classes (1..n_models-1)
#                  Fine-tune: full RAS schedule with (K_h + n_models - 1) head.
#                  Inference: predict K_h+n_models-1 ways, then COLLAPSE all
#                             human-subcluster predictions back to class 0
#                             before metrics.
# WHY S10-SPECIFIC: Substructure of the HUMAN class is real only because
#                   humans are diverse generators -- no model class needs
#                   substructure (each model is a single generator).  The
#                   asymmetric split only makes sense in a HUMAN-vs-MODEL
#                   classification, which is the AI-code attribution setting.
# WHY HIGH-IMPACT: Per-class F1 in our current results shows class 0 (human)
#                  at >0.98 but class 4-5 (Nxcode, Qwen) at 0.4-0.5.  The
#                  bottleneck is NOT human recall; it is the headroom that
#                  human-class memorisation costs the rare classes.
# FALSIFIER      : If COLLAPSED macro-F1 <= baseline, substructure was harmful.
#                  If COLLAPSED macro-F1 > baseline AND human-subclusters do
#                  NOT separate cleanly under k-means (silhouette <= 0.05),
#                  the gain is from softmax denominator change, not real
#                  substructure -- log silhouette per run.
# REPORTS        : Full eval pack on the COLLAPSED 6-way (or AICD 12-way)
#                  label space.  Plus k-means silhouette for the human cluster.
# =============================================================================
...
logger = logging.getLogger("exp75_humclust")

PAPER_BASELINE = 0.6633

# =============================================================================
# Genealogy + AST features (same template as exp60-66)
# =============================================================================
...
class HumClustModel(nn.Module):
    def __init__(self, enc_name, n_cls_expanded, ast_dim=64):
        super().__init__()
        ...
        self.clf = nn.Linear(256, n_cls_expanded)
    ...
    def forward(self, ids, mask, ast_feat):
        ...
        fused = torch.cat([sem, ast_emb], dim=-1)
...
class Cfg:
    ...
    frac: float = 0.20
    n_cls: int = 6                       # original (collapsed) label count
    n_cls_expanded: int = 9              # 4 human subclusters + 5 model classes
    K_human: int = 4
    seed: int = 42
    bs: int = 256
    seq: int = 512
...
class FSDS(TD):
    """FSDS that also stores a writable per-sample `expanded_label` field.

    Initially expanded_label = label.  After running k-means on human samples
    we overwrite expanded_label for human samples to one of {0..K_h-1}, and
    shift model labels up by (K_h - 1).
    """
    def __init__(self, data, tok, seq_len, ast_dim=64, frac=1.0, seed=42):
        ...
        # init expanded_label to label
        self._expanded = list(self.data["label"])

    def set_expanded(self, indices, values):
        for i, v in zip(indices, values):
            self._expanded[i] = int(v)
    ...
    def __getitem__(self, i):
        ...
        return {
            ...
            "label": r["label"],
            "expanded_label": self._expanded[i],
...
# =============================================================================
# Human sub-clustering (k-means on AST features of training-human samples)
# =============================================================================

def cluster_human(train_ds: FSDS, K_h: int, seed: int = 42):
    """K-means on AST features of human samples; return cluster ids and silhouette.

    Operates only on the FSDS-sampled training data (so does NOT leak test info).
    """
    human_idx = [i for i in range(len(train_ds)) if train_ds.data[i]["label"] == 0]
    if len(human_idx) < K_h * 2:
        logger.warning(f"[hc] Only {len(human_idx)} human samples, < {K_h*2}; using K_h=1")
        return human_idx, [0] * len(human_idx), 0.0
    # Extract AST features (cheap; same routine as FSDS)
    feats = []
    for i in human_idx:
        code = train_ds.data[i]["code"][:5000]
        feats.append(extract_ast_features(code, max_len=64))
    feats = np.array(feats, dtype=np.float32)
    km = KMeans(n_clusters=K_h, random_state=seed, n_init=10)
    sub_labels = km.fit_predict(feats)
    try:
        sil = float(silhouette_score(feats, sub_labels))
    except Exception:
        sil = 0.0
    logger.info(f"[hc] {len(human_idx)} human samples -> {K_h} clusters; silhouette={sil:.3f}")
    return human_idx, sub_labels.tolist(), sil


def apply_expanded_labels(train_ds, human_idx, sub_labels, K_h, n_orig_cls):
    """Rewrite train_ds.expanded_label:
       human (label 0)  ->  sub_label in {0..K_h-1}
       model  (label k>=1) -> (K_h - 1) + k   in {K_h..K_h+n_orig_cls-2}
    """
    # human reassignment
    train_ds.set_expanded(human_idx, sub_labels)
    # shift model labels
    for i in range(len(train_ds)):
        orig = train_ds.data[i]["label"]
        if orig >= 1:
            train_ds.set_expanded([i], [(K_h - 1) + orig])


def collapse_pred(pred_expanded, K_h, n_orig_cls):
    """Map expanded prediction in [0, K_h + n_orig_cls - 1] back to original [0, n_orig_cls-1]."""
    out = []
    for p in pred_expanded:
        if p < K_h:
            out.append(0)
        else:
            out.append(p - (K_h - 1))
...
# =============================================================================
# Eval pack (operates on COLLAPSED label space)
# =============================================================================

@torch.no_grad()
def eval_pack(model, loader, cfg, sib_mask_np):
    model.eval()
    preds_exp, labels_orig, langs, sources = [], [], [], []
    for b in tqdm(loader, desc="Eval"):
        ...
        preds_exp.extend(logits.argmax(dim=-1).cpu().tolist())
        labels_orig.extend(labs.tolist() if torch.is_tensor(labs) else list(labs))
    ...
    preds = np.array(collapse_pred(preds_exp, cfg.K_human, cfg.n_cls))
...
def train_epoch(model, loader, opt, sch, scaler, cfg):
    ...
    for b in tqdm(loader, desc="Train"):
        ...
        labs_exp = b["expanded_label"].to(cfg.device) if torch.is_tensor(b["expanded_label"]) \
                   else torch.tensor(b["expanded_label"], dtype=torch.long, device=cfg.device)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16,
                            enabled=(cfg.device == "cuda")):
            logits, _ = model(ids, mask, ast_feat)
            loss = F.cross_entropy(logits, labs_exp)
...
def run_exp(cfg, tag):
    ...
    # Set expanded label space size based on n_cls (original) + K_human - 1
    cfg.n_cls_expanded = cfg.K_human + cfg.n_cls - 1
    ...
    # Cluster the human class of TRAINING set; then assign expanded labels.
    human_idx, sub_labels, silhouette = cluster_human(tr_ds, cfg.K_human, seed=cfg.seed)
    apply_expanded_labels(tr_ds, human_idx, sub_labels, cfg.K_human, cfg.n_cls)

    # Val/Test do not need expanded labels -- they predict in expanded space
    # then collapse for metrics.
    ...
    logger.info(
        ...
        f"warmup={cfg.warmup} K_human={cfg.K_human} "
        f"n_cls_expanded={cfg.n_cls_expanded} silhouette={silhouette:.3f} "
    ...
    model = HumClustModel(cfg.enc, cfg.n_cls_expanded).to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        ...
        logger.info(
            ...
            f"sib_conf={val_met['sibling_confusion_rate']:.4f}"
        )
        if v > best_val:
            best_val = v
    ...
    logger.info(
        ...
        f"silhouette={silhouette:.3f} sib_conf={ts_met['sibling_confusion_rate']:.4f}"
    )
    return {
        "tag": tag, "method": "HUMCLUST", "enc": cfg.enc, "bench": cfg.benchmark,
        "frac": cfg.frac, "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
        "K_human": cfg.K_human, "n_cls_expanded": cfg.n_cls_expanded,
        "human_silhouette": silhouette,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp75_humclust_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
                    ...
                    logger.info(
                        ...
                        f"gap={res['val_test_gap']:+.4f} sil={res['human_silhouette']:.3f} "
    ...
    with open(os.path.join(out_dir, "exp75_humclust_results.json"), "w") as f:
        json.dump(results, f, indent=2)

    print("\n" + "=" * 140)
    print(f"{'Encoder':<22} {'Benchmark':<12} {'Frac':>6} {'Ep':>4} "
          f"{'Val-F1':>8} {'Test-F1':>8} {'Gap':>8} {'dPaper':>9} "
          f"{'Sil':>6} {'SibConf':>8} {'Wall':>8}")
    print("-" * 140)
    for r in results:
        tm = r["test_metrics"]
        print(
            ...
            f"{r['dpaper']:>+9.4f} {r['human_silhouette']:>6.3f} "
