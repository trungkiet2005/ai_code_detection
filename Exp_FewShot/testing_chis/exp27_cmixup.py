...
logger = logging.getLogger("exp27")
...
def mixup_data(x_ids, x_mask, y, alpha=0.2):
    """Mixup: only mix within same class for contrastive purpose."""
    if random.random() > 0.5:  # 50% chance of mixup
        return x_ids, x_mask, y, 1.0, False

    # Generate lambda from beta
    ...
    mixed_ids = lam * x_ids + (1 - lam) * x_ids[index]
    mixed_mask = torch.minimum(x_mask, x_mask[index])  # Use shorter attention

    y_a, y_b = y, y[index]
    return mixed_ids, mixed_mask, y_a, y_b, lam


def mixup_criterion(criterion, pred, y_a, y_b, lam):
    """Mixup loss: weighted combination of individual losses."""
    return lam * criterion(pred, y_a) + (1 - lam) * criterion(pred, y_b)


def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    dev = torch.device(cfg.device)
    model = CMANet(cfg).to(dev)
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            ...
            tag = f"exp27_cma_{enc}_{bench}_fixed{FIXED_TOTAL}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
    ...
    with open("results/exp27_cma_results.json", "w") as f:
