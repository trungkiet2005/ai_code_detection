"""
================================================================================
Theory-Track exp -- Curriculum Few-Shot Learning (CFSL):
Curriculum-based sample ordering for gradual few-shot adaptation.

ARXIV_ID      : ICML 2019 Curriculum (1810.03642); NeurIPS 2019 Self-Paced (1910.11760)
NAME          : Curriculum Few-Shot Learning (CFSL)
ONE-LINE CLAIM: Ordering training samples by difficulty provides smoother optimization
                landscape and better generalization in the few-shot regime.
EQUATION      : P(correct) = σ(w · f(difficulty(x))) where difficulty(x) ∝ entropy(p(y|x))
                Curriculum: easy → hard, then back to mixed.
PROPERTY      : Starting with easy samples provides stable initial optimization,
                then hard samples refine decision boundaries.
WHY NOT BEFORE: Random sampling in few-shot is highly variable. Curriculum provides
                a principled ordering that reduces variance.
FALSIFIER     : If CFSL does NOT improve over random sampling on validation,
                the curriculum ordering is not helping.
================================================================================

exp24_curriculum.py — Curriculum-based few-shot AI-code attribution.
...
class Cfg:
    ...
    # Curriculum specific
    curriculum_mode: str = "easy_to_hard"  # easy_to_hard, hard_to_easy, mixed
...
def compute_difficulty_score(code, enc, tokenizer, dev):
    """Compute difficulty score based on code length and structural complexity."""
    with torch.no_grad():
        enc_out = tokenizer(code, return_tensors="pt", truncation=True, max_length=512)
        length = enc_out["input_ids"].size(1)
        return length  # Longer code = harder
...
def build_dls(cfg: Cfg):
    ...
    if cfg.FIXED_TOTAL_TRAIN > 0:
        ...
        for i in range(len(tr_d)):
            code = tr_d[i]["code"]
            score = compute_difficulty_score(code, None, tok, dev)
            difficulty_scores.append((i, score))

        # Sort by difficulty
        difficulty_scores.sort(key=lambda x: x[1])

        # Sample balanced from sorted order
        for cls in range(cfg.n_cls):
            pool = by_cls.get(cls, [])
            n = n_per_cls + (1 if cls < remaining else 0)
            n = min(n, len(pool))

            # Filter pool by difficulty order
            pool_difficulty = [(i, score) for i, score in difficulty_scores if i in pool]

            if cfg.curriculum_mode == "easy_to_hard":
                selected = pool_difficulty[:n]
            elif cfg.curriculum_mode == "hard_to_easy":
                selected = pool_difficulty[-n:]
            else:  # mixed
                rng.shuffle(pool)
                selected = [(i, 0) for i in pool[:n]]

            chosen.extend([i for i, _ in selected])

        logger.info(f"[data] {cfg.enc} | {cfg.benchmark} | FIXED_TOTAL={cfg.FIXED_TOTAL_TRAIN} | mode={cfg.curriculum_mode} | n_train={len(chosen)}")
...
def main():
    ...
    FIXED_TOTAL = 72
    modes = ["easy_to_hard", "hard_to_easy", "mixed"]

    results = []
    for enc in encoders:
        for bench, task in benchmarks:
            for mode in modes:
                cfg = Cfg(benchmark=bench, task=task, enc=enc, FIXED_TOTAL_TRAIN=FIXED_TOTAL, curriculum_mode=mode)
                cfg = _hw(cfg)
                if torch.cuda.is_available(): torch.cuda.reset_peak_memory_stats()
                tag = f"exp24_curriculum_{enc}_{bench}_{mode}_fixed{FIXED_TOTAL}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
    ...
    with open("results/exp24_curriculum_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print("\n" + "="*100)
    print(f"{'Encoder':<22} {'Benchmark':<12} {'Mode':<15} {'Macro-F1':>10} {'dPaper':>10}")
    print("-"*100)
    for r in results:
        print(f"{r['encoder']:<22} {r['benchmark']:<12} {r['curriculum_mode']:<15} {r['macro_f1']:>10.4f} {r['delta_vs_paper']:>+10.4f}")
    print("="*100)
    print("\n[OK] Curriculum experiments complete!")
