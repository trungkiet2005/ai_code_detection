# ext_detective — Faithful K-class reproduction of DeTeCtive (NeurIPS 2024)
# =============================================================================
# UPSTREAM    : "DeTeCtive: Detecting AI-generated Text via Multi-Level
#               Contrastive Learning" — NeurIPS 2024
# FAITHFULNESS: ClassificationHead 3-layer tanh MLP (in_dim → in_dim//4 →
#               in_dim//16 → out_dim), Xavier init on weights + N(0,1e-6) on
#               biases. SupCon via SimCLR_Classifier_SCL with a=d=1.0, τ=0.07.
#               In-batch only (no momentum queue, no fabric.all_gather):
#                 k = q.clone().detach()
#               Loss = a*CE(logits_label, gt_zeros) + d*CE(classifier(q), labels)
#               Encoder: RobertaModel with mean-pool over non-padding tokens.
#               Tokenisation: standard encode_plus (no manual UniXcoder prefix).
#               Optimizer: AdamW lr=2e-5, cosine warmup schedule.
# =============================================================================
...
class Cfg:
    ...
    lr_enc:      float = 2e-5;        warmup: float = 0.10; wd: float = 0.01
...
# =============================================================================
# Data loading plumbing (shared with exp84 cargo template)
# =============================================================================
...
def _load_aicd(task):
    ...
    if not pf: raise FileNotFoundError(f"[aicd] STRICT: No parquet files under {task_path}")
...
class FSDS(TD):
    """Few-shot dataset for DeTeCtive.  Uses standard tokeniser encode_plus
    (not the manual UniXcoder CLS <encoder_only> SEP protocol) to match the
    original TextEmbeddingModel which calls the tokeniser directly."""
...
# =============================================================================
# Model — faithful to SimCLR_Classifier_SCL + ClassificationHead
# =============================================================================

class ClassificationHead(nn.Module):
    """3-layer tanh MLP, faithful to detective_multi/src/simclr.py."""
    ...
    def __init__(self, in_dim, out_dim):
        ...
        for l in (self.dense1, self.dense2, self.out_proj):
...
def _compute_logits(q, q_label, k, k_label, temperature, eps=1e-6):
    """Faithful to SimCLR_Classifier_SCL._compute_logits (label-only variant,
    in-batch without fabric.all_gather).

      q_n @ k_n.T / temperature → logits
      pos_sim = mean over same-label columns
      neg_sim = logits masked to different-label columns
      return cat([pos_sim.unsqueeze(1), neg_sim], dim=1)
    """
    q_n  = F.normalize(q, dim=-1)
    k_n  = F.normalize(k, dim=-1)
    sim  = (q_n @ k_n.T) / temperature

    q_labels  = q_label.view(-1, 1)
    k_labels  = k_label.view(1, -1)
    same_label = (q_labels == k_labels)

    pos_sim = (sim * same_label).sum(1) / same_label.sum(1).float().clamp(min=eps)
    neg_sim = sim * (~same_label)
    return torch.cat([pos_sim.unsqueeze(1), neg_sim], dim=1)


class DeTeCtiveK(nn.Module):
    """K-class DeTeCtive:
      - RobertaModel encoder, mean-pool over non-padding tokens.
      - ClassificationHead (3-layer tanh MLP).
      - Loss = a * CE(logits_label, gt_zeros) + d * CE(head(q), labels)
        where logits_label uses k = q.clone().detach() (in-batch momentum).
    """

    def __init__(self, encoder, hidden, n_cls, temperature=0.07, a=1.0, d=1.0):
        super().__init__()
    ...
    def _mean_pool(self, input_ids, attention_mask):
        ...
        return (tok * mask_f).sum(1) / mask_f.sum(1).clamp(min=1.0)

    def forward(self, input_ids, attention_mask, labels):
        q = self._mean_pool(input_ids, attention_mask)
        k = q.clone().detach()

        logits_label = _compute_logits(q, labels, k, labels, self.temperature)
        gt           = torch.zeros(q.size(0), dtype=torch.long, device=q.device)
        loss_sc      = F.cross_entropy(logits_label, gt)

        logits_cls = self.head(q)
        loss_ce    = F.cross_entropy(logits_cls, labels)

        loss = self.a * loss_sc + self.d * loss_ce
        return loss, logits_cls


# =============================================================================
# Evaluation — full eval_pack with per_class, per_language, per_source,
# sibling_confusion_rate, cross_family_confusion_rate
# =============================================================================
...
def eval_pack(model, loader, cfg, sib_mask_np, dist_mat_cpu):
    ...
    for b in tqdm(loader, desc="Eval"):
        ...
        _, logits = model(ids, mask, labs)
        preds_batch = logits.argmax(dim=-1).cpu().tolist()
        preds.extend(preds_batch)
        labels.extend(labs.cpu().tolist())
        langs.extend(list(b.get("language", [""] * len(preds_batch))))
        sources.extend(list(b.get("source",   [""] * len(preds_batch))))
...
def run_exp(cfg, tag):
    ...
    # Infer actual n_cls from training labels
    ...
    model   = DeTeCtiveK(encoder, config.hidden_size, cfg.n_cls,
                         temperature=cfg.temperature, a=cfg.a, d=cfg.d).to(cfg.device)
    ...
    logger.info(f"[sched] frac={cfg.frac} ep={cfg.epochs} lr={cfg.lr_enc} "
                f"τ={cfg.temperature} a={cfg.a} d={cfg.d} "
                f"n_cls={cfg.n_cls} train={len(tr_ds)}")

    # Build dist/sib matrices now that n_cls is known
    ...
    return {
        "tag":          tag,
        "method":       "DeTeCtive-K",
        "upstream":     "NeurIPS 2024",
        "note":         (f"a*SupCon(τ={cfg.temperature})+d*CE, a={cfg.a} d={cfg.d}. "
                         "In-batch k=q.detach(). Mean-pool encoder. ClassificationHead tanh MLP."),
        "enc":          cfg.enc,
        "bench":        cfg.benchmark,
        "frac":         cfg.frac,
        "epochs":       cfg.epochs,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"ext_detective_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "ext_detective_results.json"), "w") as f:
