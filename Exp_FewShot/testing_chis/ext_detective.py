...
# =============================================================================
# UPSTREAM : "DeTeCtive: Detecting AI-generated Text via Multi-Level Contrastive Learning"
# FAITHFULNESS: ClassificationHead 3-layer tanh MLP, SupCon a=d=1.0, τ=0.07
# =============================================================================
...
logger = logging.getLogger("ext_detective"); PAPER_BASELINE = 0.6633
...
class Cfg:
    ...
    temperature: float=0.07; a: float=1.0; d: float=1.0; device: str="cuda"
...
def _load_codet():
    ...
    if "split" in ds.column_names:
        return (ds.filter(lambda x:str(x.get("split","")).lower()=="train"),
                ds.filter(lambda x:str(x.get("split","")).lower() in {"val","validation","dev"}),
                ds.filter(lambda x:str(x.get("split","")).lower()=="test"))
...
def _tokenize(code,tokenizer,max_len):
    ...
    ids=tokenizer.convert_tokens_to_ids(toks); ids+=[tokenizer.pad_token_id]*(max_len-len(ids))
    return ids[:max_len]
class FSDS(TD):
...
class DeTeCtiveK(nn.Module):
    def __init__(self,encoder,hidden,n_cls,pad_id,temperature,a=1.0,d=1.0):
        super().__init__(); self.encoder=encoder; self.pad_id=pad_id; self.temperature=temperature; self.a=a; self.d=d; self.head=ClassificationHead(hidden,n_cls)
    ...
    def forward(self,input_ids,labels):
        ...
        logits_sc=_compute_supcon(q,k,labels,labels,self.temperature)
        gt=torch.zeros(q.size(0),dtype=torch.long,device=q.device); loss_sc=F.cross_entropy(logits_sc,gt)
        out=self.head(q); loss_ce=F.cross_entropy(out,labels)
        return self.a*loss_sc+self.d*loss_ce, out
...
def run_exp(cfg,tag):
    ...
    model=DeTeCtiveK(encoder,config.hidden_size,cfg.n_cls,pad_id,cfg.temperature,cfg.a,cfg.d).to(cfg.device)
    ...
    return {"tag":tag,"method":"DeTeCtive-K","upstream":"NeurIPS 2024","note":"a*SupCon+d*CE, a=d=1.0, τ=0.07. kNN→parametric head.",
...
def main():
    ...
    for bench,task,n_cls in [("codet_m4","author",6),("aicd_t2","t2",12)]:
        for frac in [0.01,0.05,0.20]:
            cfg=Cfg(benchmark=bench,task=task,frac=frac,n_cls=n_cls); tag=f"ext_detective_{cfg.enc}_{bench}_f{frac}"
    ...
    with open(os.path.join(out_dir,"ext_detective_results.json"),"w") as f: json.dump(results,f,indent=2)
    print("\n"+"="*100)
