# exp79_mage — Manifold Augmentation via Genealogy-conditioned mixup (MAGE)
# =============================================================================
# Theory-Track exp -- MAGE (Manifold Augmentation via Genealogy mixup)
#
# ROLE           : Standard mixup (Zhang et al., 2018) samples pairs uniformly,
#                  which on a label space with tree metric structure is
#                  WASTEFUL: cross-family mixups produce semantically incoherent
#                  samples that the encoder cannot learn from.  MAGE samples
#                  pairs with probability inversely proportional to genealogy
#                  distance -- sibling pairs are over-sampled, far-family
#                  pairs are suppressed.  Sibling-heavy mixup TEACHES the
#                  encoder to handle the hard decision boundary directly.
# NAME           : MAGE  (Manifold Augmentation via Genealogy mixup)
# ARXIV_ID       : extends mixup (Zhang 2018) by introducing a label-tree-
#                  conditioned pair sampler.  Novel for AI-code attribution.
# ONE-LINE CLAIM : Tree-conditioned mixup converts the genealogy prior into
#                  a data-augmentation policy; sibling-heavy sample blending
#                  trains the encoder on the hardest decision boundary.
# EQUATION       : pair sampling: p(j | i) ~ exp(-gamma * d_tree(y_i, y_j)) (j != i)
#                  lambda ~ Beta(alpha, alpha)
#                  z_mix = lambda * z_i + (1-lambda) * z_j
#                  L_mix = lambda * CE(clf(z_mix), y_i)
#                          + (1-lambda) * CE(clf(z_mix), y_j)
#                  Total: L = CE(clf(z), y) + lambda_mix * L_mix
# WHY NOT BEFORE : mixup variants for hierarchical labels exist (e.g. Manifold
#                  Mixup, AugMix) but none condition the PAIR SAMPLER on the
#                  external label tree.  Image-domain mixup assumes label
#                  space is unstructured.  MAGE exploits that AI-code labels
#                  have a KNOWN tree metric.
# FALSIFIER      : (F1) Sibling pair fraction: count of mixup pairs where
#                       d_tree(y_i, y_j) == 1.  Should >> uniform-random
#                       baseline.  Confirms sampler is sibling-heavy.
#                  (F2) Sibling confusion rate at test.  Should drop sharply
#                       vs CE-only baseline if MAGE works.
#                  (F3) Mixup loss curve: monotone-decreasing confirms
#                       encoder genuinely learns the blended boundary.
# REPORTS        : Full eval pack + (F1) sibling-pair fraction (F2)(F3) loss curves.
# =============================================================================
...
logger = logging.getLogger("exp79_mage")
...
class MAGEModel(nn.Module):
    def __init__(self, enc_name, n_cls, z_dim=256):
        ...
        self.n_cls = n_cls; self.z_dim = z_dim
...
def sample_partners(labels, dist_mat, gamma=1.5):
    """For each anchor i, sample partner j != i with prob ~ exp(-gamma * d_tree).

    Returns: (B,) partner indices (LongTensor).
    """
    B = labels.size(0)
    if B < 2: return torch.arange(B, device=labels.device)
    dij = dist_mat[labels][:, labels]                       # (B, B)
    log_p = -gamma * dij                                     # higher prob for siblings
    eye = torch.eye(B, device=labels.device, dtype=torch.bool)
    log_p = log_p.masked_fill(eye, -1e9)                     # no self-pair
    probs = F.softmax(log_p, dim=-1)                         # (B, B)
    partners = torch.multinomial(probs, num_samples=1).squeeze(-1)  # (B,)
    return partners


def mage_mixup_step(z, labels, model_clf, dist_mat, alpha=0.4, gamma=1.5):
    """Single mixup step: sample partners by tree, mix in embedding space."""
    B = z.size(0)
    if B < 2:
        return z.sum() * 0.0, 0
    partners = sample_partners(labels, dist_mat, gamma=gamma)
    lam = torch.distributions.Beta(alpha, alpha).sample().to(z.device)
    z_mix = lam * z + (1.0 - lam) * z[partners]
    logits_mix = model_clf(z_mix)
    L_mix = lam * F.cross_entropy(logits_mix, labels) \
            + (1.0 - lam) * F.cross_entropy(logits_mix, labels[partners])
    sib_pair_count = int((dist_mat[labels, labels[partners]] == 1.0).sum().item())
    return L_mix, sib_pair_count
...
class Cfg:
    ...
    lambda_mix: float = 0.5; alpha: float = 0.4; gamma: float = 1.5
...
def train_epoch(model, loader, opt, sch, scaler, cfg, dist_mat):
    model.train(); tot, ce_s, mix_s = 0.0, 0.0, 0.0
    sib_pair_total, pair_total = 0, 0
    for b in tqdm(loader, desc="Train"):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(cfg.device == "cuda")):
            ...
            loss_mix, sib_pairs = mage_mixup_step(z, labs, model.clf, dist_mat,
                                                  alpha=cfg.alpha, gamma=cfg.gamma)
            loss = loss_ce + cfg.lambda_mix * loss_mix
        scaler.scale(loss).backward()
        scaler.unscale_(opt)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        scaler.step(opt); scaler.update(); opt.zero_grad(); sch.step()
        tot += loss.item(); ce_s += loss_ce.item(); mix_s += loss_mix.item()
        sib_pair_total += sib_pairs; pair_total += z.size(0)
    n = len(loader)
    return tot/n, ce_s/n, mix_s/n, sib_pair_total, pair_total
...
def run_exp(cfg, tag):
    ...
    logger.info(f"[sched] frac={cfg.frac} epochs={cfg.epochs} lr_enc={cfg.lr_enc} warmup={cfg.warmup} "
                f"lambda_mix={cfg.lambda_mix} alpha={cfg.alpha} gamma={cfg.gamma}")
    loader_cfg = dict(batch_size=cfg.bs, num_workers=4, pin_memory=True)
    tr_dl = DataLoader(tr_ds, shuffle=True, **loader_cfg)
    vl_dl = DataLoader(vl_ds, shuffle=False, **loader_cfg)
    ts_dl = DataLoader(ts_ds, shuffle=False, **loader_cfg)
    model = MAGEModel(cfg.enc, cfg.n_cls).to(cfg.device)
    ...
    scaler = GradScaler()
    best_val, best_state, val_hist, sib_frac_hist = 0.0, None, [], []
    for epoch in range(cfg.epochs):
        loss, ce, mix, sib_pairs, total_pairs = train_epoch(model, tr_dl, opt, sch, scaler, cfg, dist_mat)
        sib_frac = sib_pairs / max(total_pairs, 1)
        val_met = eval_pack(model, vl_dl, cfg, sib_mask_np, dist_mat_cpu)
        v = val_met["overall"]["macro_f1"]; val_hist.append(v); sib_frac_hist.append(sib_frac)
        logger.info(f"[epoch {epoch+1}] loss={loss:.4f} ce={ce:.4f} mix={mix:.4f} val={v:.4f} "
                    f"sib_pair_frac={sib_frac:.3f}")
        if v > best_val:
            best_val = v
    ...
    ts_met["falsifier"] = {
        "sib_pair_frac_history_F1": sib_frac_hist,
        "mean_sib_pair_frac_F1": float(np.mean(sib_frac_hist)) if sib_frac_hist else 0.0,
        "test_sib_conf_rate_F2": ts_met["sibling_confusion_rate"],
    }
    logger.info(f"[final] val={best_val:.4f} test={test_macro:.4f} gap={gap:+.4f} "
                f"mean_sib_pair_frac={ts_met['falsifier']['mean_sib_pair_frac_F1']:.3f} "
                f"test_sib_conf={ts_met['sibling_confusion_rate']:.4f}")
    return {"tag": tag, "method": "MAGE", "enc": cfg.enc, "bench": cfg.benchmark,
            "frac": cfg.frac, "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
            "lambda_mix": cfg.lambda_mix, "alpha": cfg.alpha, "gamma": cfg.gamma,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp79_mage_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
                    ...
                    logger.info(f"[{tag}] test={res['macro']:.4f} ({res['dpaper']:+.4f}) "
                                ...
                                f"sib_pair={fa['mean_sib_pair_frac_F1']:.3f} "
                                f"sib_conf={fa['test_sib_conf_rate_F2']:.4f} "
    ...
    with open(os.path.join(out_dir, "exp79_mage_results.json"), "w") as f:
        json.dump(results, f, indent=2)
    print("\n" + "="*130)
    print(f"{'Encoder':<22} {'Benchmark':<12} {'Frac':>6} {'Ep':>4} {'Val-F1':>8} {'Test-F1':>8} {'Gap':>8} "
          f"{'dPaper':>9} {'sibPair':>8} {'sibConf':>8} {'Wall':>8}")
    print("-"*130)
    for r in results:
        ...
        print(f"{r['enc']:<22} {r['bench']:<12} {r['frac']:>6.0%} {r['epochs']:>4d} "
              ...
              f"{fa['mean_sib_pair_frac_F1']:>8.3f} {fa['test_sib_conf_rate_F2']:>8.4f} {r['wall']:>8.0f}s")
    print("="*130)
