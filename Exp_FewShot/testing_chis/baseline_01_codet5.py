...
logger = logging.getLogger("exp00")
...
def _preflight_check():
    """Load all 4 benchmarks and report sizes. Abort if any dataset is empty."""
    logger.info("=" * 60)
...
def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    ...
    model = CodeT5Authorship(cfg).to(dev)
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            for frac in fracs:
                ...
                cfg = _hw(cfg)
                tag = f"exp00_codet5_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
    ...
    with open("results/exp00_results.json", "w") as f:
