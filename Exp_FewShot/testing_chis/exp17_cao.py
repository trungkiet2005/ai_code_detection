"""
================================================================================
Theory-Track exp -- Contrastive Attribution Objective (CAO):
Contrastive learning with genealogically-aware positive/negative sampling.

ARXIV_ID      : ICLR 2025 Unified Representation Alignment (2502.14047);
                NeurIPS 2020 SupCon (2002.05709)
NAME          : Contrastive Attribution Objective (CAO)
ONE-LINE CLAIM: Separating generator embeddings via contrastive learning improves
                few-shot attribution by pulling same-family samples together
                while pushing unrelated generators apart.
EQUATION      : L_cao = -log(exp(sim(z_i, z_j)/τ) / Σ_k exp(sim(z_i, z_k)/τ))
                where positives = same-family, negatives = different-family
PROPERTY      : InfoNCE bound implies that minimizing L_cao maximizes mutual
                information between embeddings and generator identity.
WHY NOT BEFORE: Standard CE treats all wrong classes equally. CAO uses the
                genealogical tree to define hierarchy-aware negatives.
FALSIFIER     : If same-family accuracy does NOT improve more than different-family
                accuracy, the genealogical positive sampling is wrong.
================================================================================

exp17_cao.py — CAO baseline + CE hybrid for few-shot AI-code attribution.

Protocol:
  - FIXED_TOTAL_TRAIN = 72 samples across all benchmarks
  - ModernBERT-base encoder + CAO projection head + CE classifier head
  - Temperature τ = 0.07 (standard contrastive learning)
"""
...
# === Geneaology Tree for CoDET-M4 (GPT-branch, Llama-branch, Qwen-branch) ===
GENEALOGY_TREE = {
    "human": "root",
    "gpt-3.5-turbo": "gpt-branch",
    "gpt-4": "gpt-branch",
    "gpt-4o": "gpt-branch",
    "llama-3.1": "llama-branch",
    "llama-3": "llama-branch",
    "codellama": "llama-branch",
    "nxcode": "llama-branch",  # Related to CodeLlama
    "qwen2.5": "qwen-branch",
    "qwen2": "qwen-branch",
    "qwen1.5": "qwen-branch",
}

def _get_family(label, n_cls):
    """Get genealogical family for a label."""
    if label == 0:
        return "human"
    return f"class_{label}"

def _is_positive_pair(l1, l2):
    """Same family = positive pair."""
    f1, f2 = _get_family(l1, 6), _get_family(l2, 6)
    return f1 == f2
...
class Cfg:
    ...
    temp: float = 0.07  # Contrastive temperature
    lam_cao: float = 0.5  # Weight for CAO loss
    embed_dim: int = 128  # Contrastive embedding dimension
...
class CAONet(nn.Module):
    """CE + CAO hybrid: encoder + contrastive head + classifier head."""
    ...
    def get_contr_embeddings(self, ids, mask):
        out = self.encoder(input_ids=ids, attention_mask=mask)
        pooled = out.last_hidden_state[:, 0]
        return F.normalize(self.contr_head(pooled), dim=1)


def contrastive_loss(z1, z2, labels, temp):
    """Symmetrized InfoNCE loss with genealogical positives."""
    sim = torch.mm(z1, z2.T) / temp
    n = z1.size(0)

    # Create mask: same class = positive
    targets = labels.unsqueeze(1)
    is_pos = (targets == targets.T).float()

    # Exclude diagonal
    mask_diag = torch.eye(n, device=z1.device)
    is_pos = is_pos - mask_diag

    # Numerator: positive pairs
    exp_sim = torch.exp(sim)
    exp_sim = exp_sim * (1 - mask_diag)  # zero out diagonal

    # Denominator: all pairs
    denom = exp_sim.sum(dim=1)

    # Only keep positive terms in numerator
    pos_sim = (exp_sim * is_pos).sum(dim=1) / (is_pos.sum(dim=1) + 1e-8)
...
def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    ...
    model = CAONet(cfg).to(dev)

    # Two optimizers: encoder + contrastive head, classifier head
    opt_enc = torch.optim.AdamW(
        list(model.encoder.parameters()) + list(model.contr_head.parameters()),
        lr=cfg.lr_enc, weight_decay=cfg.wd
    )
    opt_clf = torch.optim.AdamW(
        model.clf_head.parameters(), lr=cfg.lr_head, weight_decay=cfg.wd
    )
    sched_enc = torch.optim.lr_scheduler.OneCycleLR(opt_enc, max_lr=cfg.lr_enc,
        steps_per_epoch=len(tr_dl), epochs=cfg.epochs, pct_start=cfg.warmup)
    sched_clf = torch.optim.lr_scheduler.OneCycleLR(opt_clf, max_lr=cfg.lr_head,
        steps_per_epoch=len(tr_dl), epochs=cfg.epochs, pct_start=cfg.warmup)
    ...
    for ep in range(cfg.epochs):
        ...
        for b in pbar:
            ...
            with _autocast_ctx(dev):
                ...
                # CAO loss: use embeddings from model
                z = model.get_contr_embeddings(ids, mask)

                # For small batches, use class-level contrastive loss
                loss_cao = contrastive_loss(z, z, labs, cfg.temp)

                # Combined loss
                loss = loss_ce + cfg.lam_cao * loss_cao

            ep_ce.append(loss_ce.item())
            ep_cao.append(loss_cao.item())
            ep_loss.append(loss.item())

            scaler.scale(loss).backward()
            scaler.unscale_(opt_enc); scaler.unscale_(opt_clf)
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            scaler.step(opt_enc); scaler.step(opt_clf)
            scaler.update(); opt_enc.zero_grad(); opt_clf.zero_grad()
            sched_enc.step(); sched_clf.step()
            pbar.set_postfix({"ce": f"{loss_ce.item():.3f}", "cao": f"{loss_cao.item():.3f}"})

        tr_met = eval_m(model, tr_dl, dev)
        vr = eval_m(model, vl_dl, dev)
        train_history.append({
            "epoch": ep + 1, "loss": round(np.mean(ep_loss), 6),
            "ce_loss": round(np.mean(ep_ce), 6), "cao_loss": round(np.mean(ep_cao), 6),
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            ...
            tag = f"exp17_cao_{enc}_{bench}_fixed{FIXED_TOTAL}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            ...
            row = {
                ...
                "temp": cfg.temp, "lam_cao": cfg.lam_cao,
    ...
    with open("results/exp17_cao_results.json", "w") as f:
