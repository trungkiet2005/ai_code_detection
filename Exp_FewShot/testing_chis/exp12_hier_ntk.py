...
# === LOSS: HierTree + NTK ===
def hier_ntk_loss(logits, labels, z, w, cfg: Cfg):
...
def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    ...
    for ep in range(cfg.epochs):
        ...
        for b in pbar:
            ...
            loss = hier_ntk_loss(out["logits"], labs, out["z"], w, cfg)
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            for frac in fracs:
                ...
                cfg = _hw(cfg)
                tag = f"exp12_hier_ntk_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
    ...
    with open("results/exp12_results.json", "w") as f:
