...
#
# ROLE           : Isolate the contribution of every loss object that has
#                  appeared on the leaderboard.  All ablations share one
#                  backbone (encoder + AST + projection + classifier) and one
#                  schedule (RAS).  The ONLY thing that varies is which
#                  auxiliary terms are activated.  This is the table reviewers
#                  will ask for: "what does each component buy you?".
#
# NAME           : ABL  (Component Ablation)
# UNIFIED LOSS   : L = CE
#                     + lambda_ssl  * L_ssl        (sibling-weighted CE)
#                     + lambda_htka * L_htka       (genealogy Gram alignment)
#                     + lambda_gsce * L_gsce       (genealogy-smoothed CE)
#                     + lambda_scr  * L_scr        (sibling probability penalty)
# ABLATIONS RUN  :
#   "ce"             : (0, 0, 0, 0)             -- pure CE baseline
#   "ssl"            : (0.3, 0, 0, 0)
#   "htka"           : (0, 1.0, 0, 0)
#   "gsce"           : (0, 0, 0.1, 0)
#   "scr"            : (0, 0, 0, 1.0)
#   "ssl+htka"       : (0.3, 1.0, 0, 0)
#   "ssl+htka+scr"   : (0.3, 1.0, 0, 1.0)
#   "all"            : (0.3, 1.0, 0.1, 1.0)
# PROPERTY       : This is a 2^4 = 16 ablation space; the 8 settings above
#                  cover the leave-one-out lattice plus full-on / full-off.
#                  Pairwise interactions (e.g. ssl x gsce) can be added
#                  later by listing more keys in ABLATIONS.
# RUN SCOPE      : Only at frac = 0.20 (the regime where genealogy prior
#                  dominates, per Theorem 2).  Both CoDET-M4 and AICD-T2.
#                  Total: 8 ablations x 2 benchmarks = 16 runs.
# FALSIFIER      : If "ce" >= "all" - epsilon, the prior is not contributing.
#                  If only "ssl" >= "ssl+htka", HTKA is redundant given SSL.
# REPORTS        : Full eval pack per run + delta vs CE baseline within file.
# =============================================================================
...
logger = logging.getLogger("exp65_abl")
...
ABLATIONS = {
    "ce":           {"lambda_ssl": 0.0, "lambda_htka": 0.0, "lambda_gsce": 0.0, "lambda_scr": 0.0},
    "ssl":          {"lambda_ssl": 0.3, "lambda_htka": 0.0, "lambda_gsce": 0.0, "lambda_scr": 0.0},
    "htka":         {"lambda_ssl": 0.0, "lambda_htka": 1.0, "lambda_gsce": 0.0, "lambda_scr": 0.0},
    "gsce":         {"lambda_ssl": 0.0, "lambda_htka": 0.0, "lambda_gsce": 0.1, "lambda_scr": 0.0},
    "scr":          {"lambda_ssl": 0.0, "lambda_htka": 0.0, "lambda_gsce": 0.0, "lambda_scr": 1.0},
    "ssl+htka":     {"lambda_ssl": 0.3, "lambda_htka": 1.0, "lambda_gsce": 0.0, "lambda_scr": 0.0},
    "ssl+htka+scr": {"lambda_ssl": 0.3, "lambda_htka": 1.0, "lambda_gsce": 0.0, "lambda_scr": 1.0},
    "all":          {"lambda_ssl": 0.3, "lambda_htka": 1.0, "lambda_gsce": 0.1, "lambda_scr": 1.0},
}
...
def build_smoothing_targets(n_cls, adj, eps=0.1, alpha=1.0):
    D = build_distance_matrix(n_cls, adj)
    Q = torch.zeros(n_cls, n_cls)
    for y in range(n_cls):
        off_mask = torch.arange(n_cls) != y
        w = torch.exp(-alpha * D[y]) * off_mask.float()
...
# =============================================================================
# Unified model: classifier + alignment head
# =============================================================================
...
def ssl_term(logits, labels, sibling_w):
    """Sibling-weighted CE: sample_w(i) = 1 if y_pred is sibling of y, else 0."""
    ce = F.cross_entropy(logits, labels, reduction="none")
    with torch.no_grad():
        preds = logits.argmax(dim=-1)
        w = sibling_w[labels, preds]  # 1 if sibling confusion, 0 otherwise
    return (w * ce).mean()


def htka_term(z, labels, target_kernel):
...
def gsce_term(logits, labels, Q):
    log_p = F.log_softmax(logits, dim=-1)
    q = Q[labels]
    # Smooth-CE = -<q, log_p>; subtract pure CE so this term is the *additional*
    # smoothing contribution (zero when Q is one-hot).
    smooth = -(q * log_p).sum(-1).mean()
    pure_ce = F.cross_entropy(logits, labels)
    return smooth - pure_ce


def scr_term(logits, labels, sibling_mask):
    p = F.softmax(logits, dim=-1)
    return (sibling_mask[labels] * p).sum(-1).mean()
...
class Cfg:
    ...
    warmup: float = 0.1
    wd: float = 0.01
    # Activation knobs -- filled in from ABLATIONS dict
    lambda_ssl: float = 0.0
    lambda_htka: float = 0.0
    lambda_gsce: float = 0.0
    lambda_scr: float = 0.0
    eps: float = 0.10
    alpha: float = 1.0
    gamma: float = 1.0
    device: str = "cuda"
...
def train_epoch(model, loader, opt, sch, scaler, cfg,
                sib_mask, target_kernel, Q):
    model.train()
    log_acc = {"ce": 0.0, "ssl": 0.0, "htka": 0.0, "gsce": 0.0, "scr": 0.0, "tot": 0.0}
    for b in tqdm(loader, desc="Train"):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16,
            ...
            l_ce = F.cross_entropy(logits, labs)
            loss = l_ce
            l_ssl = ssl_term(logits, labs, sib_mask) if cfg.lambda_ssl > 0 else torch.tensor(0.0, device=cfg.device)
            l_htka = htka_term(z.float(), labs, target_kernel) if cfg.lambda_htka > 0 else torch.tensor(0.0, device=cfg.device)
            l_gsce = gsce_term(logits, labs, Q) if cfg.lambda_gsce > 0 else torch.tensor(0.0, device=cfg.device)
            l_scr = scr_term(logits, labs, sib_mask) if cfg.lambda_scr > 0 else torch.tensor(0.0, device=cfg.device)
            loss = l_ce + cfg.lambda_ssl * l_ssl + cfg.lambda_htka * l_htka \
                   + cfg.lambda_gsce * l_gsce + cfg.lambda_scr * l_scr

        scaler.scale(loss).backward()
        scaler.unscale_(opt)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        scaler.step(opt); scaler.update(); opt.zero_grad(); sch.step()
        log_acc["ce"] += l_ce.item()
        log_acc["ssl"] += float(l_ssl.item()) if torch.is_tensor(l_ssl) else 0.0
        log_acc["htka"] += float(l_htka.item()) if torch.is_tensor(l_htka) else 0.0
        log_acc["gsce"] += float(l_gsce.item()) if torch.is_tensor(l_gsce) else 0.0
        log_acc["scr"] += float(l_scr.item()) if torch.is_tensor(l_scr) else 0.0
        log_acc["tot"] += loss.item()
    n = len(loader)
    return {k: v / n for k, v in log_acc.items()}
...
def run_exp(cfg, tag):
    ...
    sib_mask = build_sibling_mask(cfg.n_cls, cfg.gene_adj).to(cfg.device)
    sib_mask_np = sib_mask.cpu().numpy()
    target_kernel = build_target_kernel(cfg.n_cls, cfg.gene_adj, gamma=cfg.gamma).to(cfg.device)
    Q = build_smoothing_targets(cfg.n_cls, cfg.gene_adj, eps=cfg.eps, alpha=cfg.alpha).to(cfg.device)
    ...
    logger.info(
        ...
        f"warmup={cfg.warmup} abl={cfg.ablation} "
        f"lam=(ssl={cfg.lambda_ssl}, htka={cfg.lambda_htka}, "
        f"gsce={cfg.lambda_gsce}, scr={cfg.lambda_scr}) total_steps={total_steps}"
    )
    ...
    model = UnifiedModel(cfg.enc, cfg.n_cls).to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        log_acc = train_epoch(model, tr_dl, opt, sch, scaler, cfg, sib_mask, target_kernel, Q)
        val_met = eval_pack(model, vl_dl, cfg, sib_mask_np)
        v = val_met["overall"]["macro_f1"]; val_hist.append(v)
        logger.info(
            f"[epoch {epoch+1}] ce={log_acc['ce']:.3f} ssl={log_acc['ssl']:.3f} "
            f"htka={log_acc['htka']:.3f} gsce={log_acc['gsce']:.3f} scr={log_acc['scr']:.3f} "
    ...
    logger.info(
        f"[final] abl={cfg.ablation} val={best_val:.4f} test={test_macro:.4f} "
        f"gap={val_test_gap:+.4f} sib_conf={ts_met['sibling_confusion_rate']:.4f}"
    )
    return {
        "tag": tag, "method": "ABL", "ablation": cfg.ablation,
        "enc": cfg.enc, "bench": cfg.benchmark, "frac": cfg.frac,
        "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
        "lambda_ssl": cfg.lambda_ssl, "lambda_htka": cfg.lambda_htka,
        "lambda_gsce": cfg.lambda_gsce, "lambda_scr": cfg.lambda_scr,
...
def main():
    ...
    # Ablation runs ONLY at 20% (the regime where the prior dominates)
    frac = 0.20

    results = []
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for abl_name, abl_lams in ABLATIONS.items():
                cfg = Cfg(benchmark=bench, task=task, enc=enc, frac=frac, n_cls=n_cls,
                          ablation=abl_name, **abl_lams)
                tag = f"exp65_abl_{enc}_{bench}_f{frac}_{abl_name}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "exp65_abl_results.json"), "w") as f:
        json.dump(results, f, indent=2)

    # Per-benchmark ablation tables vs CE baseline
    for bench, _, _ in benchmarks:
        rows = [r for r in results if r["bench"] == bench]
        ce_row = next((r for r in rows if r["ablation"] == "ce"), None)
        ce_macro = ce_row["macro"] if ce_row else 0.0
        print(f"\n=== {bench} ablation @ frac={frac} (CE baseline = {ce_macro:.4f}) ===")
        print(f"{'Ablation':<15} {'Val-F1':>8} {'Test-F1':>8} {'Δ vs CE':>9} "
              f"{'Gap':>8} {'SibConf':>8} {'CrossFam':>9} {'Wall':>8}")
        print("-" * 90)
        for r in sorted(rows, key=lambda x: -x["macro"]):
            d = r["macro"] - ce_macro
            tm = r["test_metrics"]
            print(
                f"{r['ablation']:<15} {r['val_macro']:>8.4f} {r['macro']:>8.4f} "
                f"{d:>+9.4f} {r['val_test_gap']:>+8.4f} "
                f"{tm['sibling_confusion_rate']:>8.4f} "
