"""
exp01_detective.py — DeTeCtive-style CE + supervised contrastive baseline

Published method: DeTeCtive (arXiv 2410.20964)
Style: CE + supervised contrastive style separation
...
def _load_droid():
    ...
    Kaggle path structure:
      /kaggle/input/datasets/chiboiz/droid-collection/DroidCollection/data/
        ...
        ├── train-00004-of-00004.parquet
        ├── test-00001-of-00002.parquet
        └── test-00002-of-00002.parquet
    
    HuggingFace fallback: project-droid/DroidCollection (train/dev/test splits).
    """
    import re
    
    # Pattern: train-*.parquet and test-*.parquet
    train_pattern = os.path.join(KAGGLE_DROID, "data", "train-*.parquet")
    test_pattern = os.path.join(KAGGLE_DROID, "data", "test-*.parquet")
...
class DeTeCtiveNet(nn.Module):
    ...
    def forward(self, ids, mask):
        ...
        return {"logits": self.clf(self.drop(emb)), "z": F.normalize(self.proj(emb), dim=-1)}

    def groups(self):
        return [{"params": self.enc.parameters(), "lr": self.cfg.lr_enc, "weight_decay": self.cfg.wd},
                {"params": list(self.clf.parameters())+list(self.proj.parameters()), "lr": self.cfg.lr_head, "weight_decay": self.cfg.wd}]

# === Supervised Contrastive Loss (DeTeCtive style) ===
...
def det_loss(logits, labels, z, w, cfg: Cfg):
    ce = F.cross_entropy(logits, labels, weight=w)
    sup = supcon_loss(z, labels, temp=0.07)
    return ce + cfg.lambda_supcon * sup
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            for frac in fracs:
                ...
                cfg = _hw(cfg)
                tag = f"exp01_detective_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
    ...
    with open("results/exp01_results.json", "w") as f:
