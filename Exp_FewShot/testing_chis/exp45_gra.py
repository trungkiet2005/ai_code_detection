...
def get_family(u: int, adj: Dict) -> int:
    ...
    for parent, children in adj.items():
        if u in children:
            return parent
    return u  # Isolated generators are their own family
...
class FamilyMeanTracker:
    ...
    def __init__(self, n_families: int, feat_dim: int):
        self.n_families = n_families
        self.feat_dim = feat_dim
        self.counts = torch.zeros(n_families)
        self.means = torch.zeros(n_families, feat_dim)
...
def compute_gra_loss(residual_emb: torch.Tensor, labels: torch.Tensor,
    ...
    neg_mask = 1 - pos_mask
    neg_mask = neg_mask - torch.eye(B, device=device)
...
class Cfg:
    ...
    def __post_init__(self):
        if self.benchmark == "codet_m4":
            ...
            self.n_families = 4  # human, gpt-family, llama, others
        else:
...
def train_epoch(model, loader, opt, sch, scaler, cfg, tracker):
    ...
    for b in tqdm(loader, desc="Train"):
        ...
        # Compute family IDs
        family_ids = torch.tensor([get_family(l.item(), cfg.gene_adj) for l in labs], device=cfg.device)
...
def run_exp(cfg: Cfg, tag: str):
    set_seed(cfg.seed)
    cfg = _hw(cfg)
    logger.info(f"[exp45] GRA: {tag} | frac={cfg.frac}")
    ...
    # Family mean tracker
    tracker = FamilyMeanTracker(cfg.n_families, 64)
    ...
    for epoch in range(cfg.epochs):
        ...
        logger.info(f"  E{epoch+1}: loss={loss:.4f} ce={loss_ce:.4f} gra={loss_gra:.4f} | val={val_met['macro']:.4f}")
...
def main():
    ...
    for bench, task, n_cls in benchmarks:
        for frac in fracs:
            ...
            tag = f"exp45_gra_{enc}_{bench}_f{frac:.2f}"
            try:
                r = run_exp(cfg, tag)
