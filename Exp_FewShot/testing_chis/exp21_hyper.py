"""
================================================================================
Theory-Track exp -- Hyperbolic Code Embeddings (HCE):
Hyperbolic geometry for hierarchical code representations.

ARXIV_ID      : ICML 2018 Hyperbolic (1806.00836); ACL 2019句子嵌入 (1901.03610)
NAME          : Hyperbolic Code Embeddings (HCE)
ONE-LINE CLAIM: Embedding code representations in hyperbolic space better captures
                the hierarchical structure of LLM generator families.
EQUATION      : d_P(u, v) = arcosh(1 + 2||u-v||² / ((1-||u||²)(1-||v||²)))
                Embeddings live on Poincaré ball model of hyperbolic space.
PROPERTY      : Hyperbolic space grows exponentially with radius, naturally encoding
                hierarchical relationships without explicit tree structure.
WHY NOT BEFORE: Euclidean embeddings distort hierarchical relationships, requiring
                large dimension for tree-like structures.
FALSIFIER     : If hyperbolic embeddings do NOT improve sibling-class accuracy more
                than flat-class accuracy, hyperbolic geometry is not appropriate.
================================================================================

exp21_hyper.py — Hyperbolic space embeddings for few-shot AI-code attribution.
...
class Cfg:
    ...
    # Hyperbolic specific
    embed_dim: int = 64  # Poincaré ball embedding dim
    curvature: float = 1.0  # Curvature of Poincaré ball
...
class ExpMapPoincare(nn.Module):
    """Exponential map for Poincaré ball model."""
...
class HyperNet(nn.Module):
    """Encoder with hyperbolic embedding for classification."""
    def __init__(self, cfg: Cfg):
        ...
        # Project to hyperbolic embedding
        self.proj = nn.Sequential(
            nn.Linear(hidden, hidden),
            nn.ReLU(),
            nn.Linear(hidden, cfg.embed_dim),
        )
        self.exp_map = ExpMapPoincare(cfg.embed_dim, cfg.curvature)

        # Classifier (on hyperbolic space)
        self.head = nn.Sequential(
            nn.Linear(cfg.embed_dim, cfg.embed_dim // 2),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(cfg.embed_dim // 2, cfg.n_cls),
        )
    ...
    def forward(self, ids, mask):
        ...
        # Project to Euclidean, then map to hyperbolic
        h_euclidean = self.proj(pooled)
        h_hyperbolic = self.exp_map(h_euclidean)

        logits = self.head(h_hyperbolic)
        return {"logits": logits, "pooled": h_hyperbolic, "euclidean": h_euclidean}


def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    dev = torch.device(cfg.device)
    model = HyperNet(cfg).to(dev)
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            ...
            tag = f"exp21_hyper_{enc}_{bench}_fixed{FIXED_TOTAL}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            ...
            row = {
                ...
                "embed_dim": cfg.embed_dim, "curvature": cfg.curvature,
    ...
    with open("results/exp21_hyper_results.json", "w") as f:
    ...
    print("="*100)
    print("\n[OK] Hyperbolic experiments complete!")
