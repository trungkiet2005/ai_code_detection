# =============================================================================
# Theory-Track exp -- SCR (Sibling Confusion Regularizer)
#
# ROLE           : DIRECT optimisation of the falsifier metric used by every
#                  genealogy-aware experiment in this repo.  SSL / GSCE / RASL
#                  / HTKA all CLAIM to reduce sibling confusion.  SCR adds a
#                  loss that ASKS for exactly that, with no other inductive
#                  bias.  Comparing SCR to the others isolates how much of
#                  the gain is attributable to confusion-rate reduction
#                  versus richer geometric structure.
# NAME           : SCR  (Sibling Confusion Regularizer)
# ONE-LINE CLAIM : Minimising soft sibling probability mass directly improves
#                  Macro-F1 -- if the falsifier metric is causal for accuracy.
# EQUATION       : Let p = softmax(logits), W_sib in {0,1}^{KxK}.
#                  scr_per_sample(i) = sum_c W_sib[y_i, c] * p_i[c]
#                  L_scr = mean_i scr_per_sample(i)
#                  Total: L = CE + lambda * L_scr
# PROPERTY       : Differentiable (gradient flows through softmax).
#                  Zero when all probability mass on the true class.
#                  Only siblings appear in W_sib -- no constraint on distant
#                  classes => CE remains responsible for the global decision.
# WHY NOT BEFORE : Mean-class-CE rebalances over CLASSES not over SIBLINGS.
#                  Pairwise hierarchical losses (HXE, hierarchical-softmax)
#                  define a TREE OVER OUTPUT, not a sibling penalty alone.
#                  SCR is the minimal object that targets only the falsifier.
# FALSIFIER      : If SCR drops sibling_confusion_rate but does NOT improve
#                  macro-F1, sibling confusion is symptomatic, not causal.
#                  If SCR fails to reduce sibling_confusion_rate at all,
#                  the loss is dominated by CE => increase lambda.
...
logger = logging.getLogger("exp62_scr")
...
def scr_loss(logits, labels, sibling_mask):
    """L_scr = mean over batch of sum_c W_sib[y_i, c] * softmax(logits)[c]."""
    p = F.softmax(logits, dim=-1)             # (B, K)
    sib_per_row = sibling_mask[labels]        # (B, K) in {0, 1}
    return (sib_per_row * p).sum(-1).mean()
...
def train_epoch(model, loader, opt, sch, scaler, cfg, sib_mask):
    model.train()
    tot, ce_sum, scr_sum = 0.0, 0.0, 0.0
    for b in tqdm(loader, desc="Train"):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16,
            ...
            loss_scr = scr_loss(logits, labs, sib_mask)
            loss = loss_ce + cfg.lambda_scr * loss_scr
        scaler.scale(loss).backward()
        scaler.unscale_(opt)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        scaler.step(opt); scaler.update(); opt.zero_grad(); sch.step()
        tot += loss.item(); ce_sum += loss_ce.item(); scr_sum += loss_scr.item()
    n = len(loader)
    return tot / n, ce_sum / n, scr_sum / n
...
def run_exp(cfg, tag):
    ...
    logger.info(
        ...
        f"warmup={cfg.warmup} lambda_scr={cfg.lambda_scr} total_steps={total_steps}"
    )
    ...
    model = SCRModel(cfg.enc, cfg.n_cls).to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        loss, ce, scr_l = train_epoch(model, tr_dl, opt, sch, scaler, cfg, sib_mask)
        val_met = eval_pack(model, vl_dl, cfg, sib_mask_np)
        v = val_met["overall"]["macro_f1"]; val_hist.append(v)
        logger.info(
            f"[epoch {epoch+1}] loss={loss:.4f} ce={ce:.4f} scr={scr_l:.4f} "
            f"val_macro={v:.4f} sib_conf={val_met['sibling_confusion_rate']:.4f}"
        )
        if v > best_val:
            best_val = v
    ...
    return {
        "tag": tag, "method": "SCR", "enc": cfg.enc, "bench": cfg.benchmark,
        "frac": cfg.frac, "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
        "lambda_scr": cfg.lambda_scr,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp62_scr_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "exp62_scr_results.json"), "w") as f:
