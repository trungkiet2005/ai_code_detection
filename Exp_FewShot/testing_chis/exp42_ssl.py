...
def ssl_loss(logits: torch.Tensor, labels: torch.Tensor,
    ...
    """Sibling Structure Loss - per-sample CE re-weighted by genealogy distance to predicted class.

    L_ssl = mean_i ( w_{y_i, ŷ_i} · CE(f(x_i), y_i) )

    Where w_{y_i, ŷ_i} is a fixed (no-grad) coefficient from weight_matrix,
    larger when the prediction is far in genealogy from the true label.
    Gradient flows through `ce` (sample-wise CE) only; weights are constants per step.
    """
    device = logits.device

    # Per-sample CE (carries gradient)
    ce = F.cross_entropy(logits, labels, reduction="none")  # (B,)

    # Per-sample weight from genealogy matrix (constant w.r.t. parameters)
    with torch.no_grad():
        preds = logits.argmax(dim=-1)
        wm = weight_matrix.to(device)
        sample_w = wm[labels, preds]                # (B,)
        sample_w = sample_w * (labels != preds).float()  # zero out correct preds

    # Differentiable multiplication: grad flows via ce only
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                cfg = _hw(cfg)
                tag = f"exp42_ssl_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "exp42_ssl_results.json"), "w") as f:
