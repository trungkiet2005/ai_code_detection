"""
exp_n17_ast_hier_tree.py — HierTree + AST features

NAME : Hierarchical Family Prior with AST (HierTree-AST)
ONE-LINE CLAIM: AST structural features capture code organization patterns;
                combining with hierarchical family prior improves attribution.
EQUATION : L = L_ce + λ_hier * (L_pull + L_push) + λ_ast * L_ast
PROPERTY : AST features encode structural patterns (nesting, control flow)
           that differentiate generator families.
WHY NOT BEFORE : Prior HierTree uses only text; AST adds orthogonal signal.
FALSIFIER : If AST features improve HierTree, structure is discriminative.

Self-contained. Runs: unixcoder-base × [codet_m4, aicd_t2] × [1%, 5%, 20%] = 6 runs.

Usage:
  python exp_n17_ast_hier_tree.py
"""
...
logger = logging.getLogger("exp_n17")
...
# =============================================================================
# AST Feature Extraction (self-contained)
# =============================================================================
...
def extract_ast_features(code: str, max_len: int = AST_FEAT_DIM) -> np.ndarray:
    ...
    features = [
        ...
        n_lines / 500.0,
        avg_indent / 10.0,
        (n_if + n_for + n_while) / max(1, n_func),
        len(code) / 10000.0,
    ]
...
class HierTreeASTNet(nn.Module):
    """HierTree with AST structural features."""
    ...
    def forward(self, ids, mask, ast_feat):
        ...
        return {"logits": self.clf(self.drop(fused)), "z": F.normalize(fused, dim=-1)}
    ...
    def groups(self):
        return [
            ...
            {"params": list(self.ast_encoder.parameters()) + list(self.fusion.parameters()) +
                       list(self.clf.parameters()), "lr": self.cfg.lr_head, "weight_decay": self.cfg.wd}
        ]

# =============================================================================
# Loss: HierTree + AST alignment
# =============================================================================

def hier_ast_loss(logits, labels, z, cfg: Cfg):
    """Hierarchical loss + AST structural alignment."""
    ...
    return ce + cfg.lambda_hier * (pull + push)
...
def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    ...
    model = HierTreeASTNet(cfg).to(dev)
    ...
    for ep in range(cfg.epochs):
        ...
        for b in pbar:
            ...
            with autocast(enabled=(dev.type == "cuda")):
                out = model(ids, mask, ast_feat)
                loss = hier_ast_loss(out["logits"], labs, out["z"], cfg)
...
def run_exp(cfg: Cfg, tag: str):
    cfg = _hw(cfg)
    logger.info(f"[exp_n17] {tag} | frac={cfg.frac}")
...
def main():
    ...
    for bench, task, n_cls in benchmarks:
        for frac in fracs:
            ...
            tag = f"exp_n17_ast_hier_tree_{enc}_{bench}_f{frac:.2f}"
            try:
                r = run_exp(cfg, tag)
                results.append(r)
