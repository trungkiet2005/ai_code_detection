"""
exp_n19_detective_lite.py — DeTeCtive-lite (Exp27 migration)

Migrated from legacy/Exp_CodeDet/run_codet_m4_exp27_detective.py (71.53 F1)

Simplified version of DeTeCtiveCode:
- Removed AST/Spectral features (kept only text encoder)
- Simplified kNN retrieval (optional)
- Added focal loss + SupCon + HierTree

Loss: L_focal + lambda_hier * L_hier + lambda_supcon * L_supcon + [optional] L_knn
...
Usage:
  python exp_n19_detective_lite.py
"""
...
logger = logging.getLogger("exp_n19")

# === CONSTANTS ===
# Tree structure: class index → family index
HIER_FAM = {0: 0, 1: 1, 2: 2, 3: 1, 4: 3, 5: 3}
...
class Cfg:
    ...
    # kNN retrieval (optional)
    use_knn: bool = True
    knn_k: int = 32
    knn_alpha: float = 0.25
    knn_bank_size: int = 10000
...
class DeTeCtiveLite(nn.Module):
    def __init__(self, cfg: Cfg):
        ...
        # Multi-head projection for SupCon
    ...
    def groups(self):
        return [
            ...
            {"params": list(self.clf.parameters()) + list(self.proj.parameters()),
             "lr": self.cfg.lr_head, "weight_decay": self.cfg.wd}
        ]

# === kNN RETRIEVAL BANK ===
class EmbeddingBank:
    def __init__(self, num_classes: int, k: int = 32, alpha: float = 0.25, max_size: int = 10000):
    ...
    def build(self, model, dataloader, device):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                out = model(ids, mask)
                emb = F.normalize(out["emb"], p=2, dim=-1).cpu()
                all_embs.append(emb)
                all_labels.append(batch["labels"])
                if sum(len(l) for l in all_labels) >= self.max_size:
                    break

        self.embeddings = torch.cat(all_embs)[:self.max_size]
        self.labels = torch.cat(all_labels)[:self.max_size]
        self.is_built = True
        logger.info(f"[knn] Bank: {len(self.embeddings)} samples")

    def retrieve_logits(self, query_embs):
        ...
        rag_logits = torch.zeros(q.shape[0], self.num_classes)
    ...
    def blend(self, clf_logits, emb):
        rag_logits = self.retrieve_logits(emb)
        rag_log = torch.log(rag_logits.clamp(min=1e-8))
        return (1.0 - self.alpha) * clf_logits + self.alpha * rag_log
...
def hier_loss(z, labels, n_cls, margin=0.3):
...
def compute_loss(logits, labels, z, w, cfg: Cfg, focal_fn):
    """Combined loss: Focal + HierTree + SupCon."""
    focal = focal_fn(logits, labels)
    h_loss = hier_loss(z, labels, cfg.n_cls, margin=0.3)
    s_loss = supcon_loss(z, labels, cfg.temperature)
    total = focal + cfg.lambda_hier * h_loss + cfg.lambda_supcon * s_loss

    return {"total": total, "focal": focal.item(), "hier": h_loss.item(), "supcon": s_loss.item()}
...
@torch.no_grad()
def eval_m(model, loader, dev, knn_bank=None):
    model.eval()
    ps, ls = [], []
    for b in loader:
        ...
        logits = out["logits"]
        if knn_bank is not None and knn_bank.is_built:
            logits = knn_bank.blend(logits, out["emb"])
...
def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    ...
    model = DeTeCtiveLite(cfg).to(dev)
    w = class_w(tr_dl, cfg.n_cls).to(dev)
    focal_fn = FocalLoss(gamma=cfg.focal_gamma, weight=w)
    ...
    # kNN bank for test-time augmentation
    knn_bank = None
    if cfg.use_knn:
        knn_bank = EmbeddingBank(cfg.n_cls, cfg.knn_k, cfg.knn_alpha, cfg.knn_bank_size)

    for ep in range(cfg.epochs):
        model.train()
        pbar = tqdm(tr_dl, desc=f"Epoch {ep + 1}/{cfg.epochs}", leave=False)
        ep_losses = {"total": 0, "focal": 0, "hier": 0, "supcon": 0}

        for b in pbar:
            ids, mask, labs = b["ids"].to(dev), b["mask"].to(dev), b["labels"].to(dev)
            with autocast(enabled=(dev.type == "cuda")):
                out = model(ids, mask)
                losses = compute_loss(out["logits"], labs, out["z"], w, cfg, focal_fn)
                loss = losses["total"]
    ...
    # Build kNN bank for test
    if knn_bank is not None:
        knn_bank.build(model, tr_dl, dev)

    return eval_m(model, ts_dl, dev, knn_bank)
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            for frac in fracs:
                ...
                cfg = _hw(cfg)
                tag = f"exp_n19_detect_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
    ...
    with open("results/exp_n19_detective_results.json", "w") as f:
