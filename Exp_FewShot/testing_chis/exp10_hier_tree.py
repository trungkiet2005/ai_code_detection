...
def _load_droid():
    ...
    logger.info(f"[droid] Found files: {len(test_files)} test, {len(dev_files)} dev, {len(train_files)} train")
    ...
    if test_files and dev_files:
        ...
        # If no train shards, combine test+dev as train and split
        if train_ds is None:
            # Merge test + dev, then split 80/10/10
            merged = load_dataset("parquet", data_files=test_files + dev_files, split="train")
            s = merged.train_test_split(test_size=0.1, seed=42)
            s2 = s["train"].train_test_split(test_size=1/9, seed=42)
            return s2["train"], s2["test"], s["test"]
        else:
            # Use provided splits: train → split for val
            s = train_ds.train_test_split(test_size=1/9, seed=42)  # 10% of train = val
            return s["train"], s["test"], test_ds
    else:
        # Kaggle version: only train shards, need to split
        if all_files:
...
def build_dls(cfg: Cfg):
    ...
    if cfg.benchmark == "codet_m4":
        ...
        logger.info(f"[DEBUG] Droid raw sizes: tr={len(tr_raw)}, vl={len(vl_raw)}, ts={len(ts_raw)}")
