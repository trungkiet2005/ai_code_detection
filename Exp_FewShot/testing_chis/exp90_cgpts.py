# exp90_cgpts — CodeGPTSensor adapter: flat SupCon + view aug, no tree weighting
# =============================================================================
# NAME           : CGPTS (CodeGPTSensor, multi-class adaptation)
# UPSTREAM       : Xu et al., "Distinguishing LLM-generated from Human-written
#                  Code by Contrastive Learning", TOSEM 2025
#                  Repo: github.com/doriscullen/CodeGPTSensor
# ROLE           : Architecturally MATCHED twin of TRACO. Same encoder
#                  (UniXcoder-base), same projector, same doubled-batch SupCon,
#                  same view augmentation, BUT no tree weighting on negatives
#                  (i.e. gamma=0 special case of TRACO). Upstream paper does
#                  binary H/M detection; we extend to K-class authorship by
#                  swapping the binary head with K-softmax.
# CLAIM TEST     : If CGPTS matches TRACO, then tree-weighting is irrelevant
#                  and we should retract that contribution. If CGPTS lags
#                  TRACO at extreme few-shot, the tree-weighting is causally
#                  responsible for the gain.
# EQUATION       : L = L_ce + lambda_aug * SupCon(z, z') with W_ij = 1[y_i=y_j]
#                  for positives, W_ij = 1 for negatives (FLAT, no tree decay).
# FALSIFIER      : Delta vs TRACO at each fraction. Positive at extreme
#                  few-shot would mean the tree term genuinely helps.
# GPU TUNING     : Single 2-view forward pass; bs auto-scaled by detected
#                  memory. On H100 (>= 40 GB) we run at bs=128, seq=512.
#                  On 10-40 GB we drop to bs=64, seq=384. On smaller GPUs
#                  we run at bs=32, seq=256.
# =============================================================================
...
logger = logging.getLogger("exp90_cgpts")
...
class CGPTSModel(nn.Module):
    """Same backbone as TRACO. Difference is only at the loss: we use FLAT
    SupCon (no tree weighting). This isolates the contribution of the tree
    weighting term in our headline method."""
...
def supcon_flat_loss(z_d, y_d, tau=0.1):
    """Standard supervised contrastive loss (Khosla 2020) — FLAT, no tree
    weighting. Every wrong-class negative receives unit weight; every
    same-class non-self receives unit weight as positive. This is the
    gamma=0 special case of TRACO's tree-weighted SupCon."""
    N = z_d.size(0)
...
def _hw(c):
    """GPU-aware autoscaling. 2-view forward pass means we halve the
    effective batch vs single-view runs at the same memory budget."""
    if torch.cuda.is_available():
        ...
        logger.info(f"[hw] mem={mem:.1f}GB bs={c.bs} seq={c.seq} (2-view contrastive)")
    return c
...
def train_epoch(model, loader, opt, sch, scaler, cfg):
    ...
    for b in tqdm(loader, desc="Train"):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(cfg.device == "cuda")):
            ...
            loss_sc = supcon_flat_loss(zd, yd, tau=cfg.tau)
...
def run_exp(cfg, tag):
    ...
    logger.info(f"[sched] frac={cfg.frac} ep={cfg.epochs} lr={cfg.lr_enc} wu={cfg.warmup} lambda={cfg.lambda_aug}")
    ...
    model = CGPTSModel(cfg.enc, cfg.n_cls, cfg.emb_dim).to(cfg.device)
    ...
    for ep in range(cfg.epochs):
        ...
        logger.info(f"[ep{ep+1}] loss={loss:.4f} ce={ce:.4f} supcon={sc:.4f} val={v:.4f}")
        if v > best_val:
    ...
    return {"tag": tag, "method": "CGPTS", "enc": cfg.enc, "bench": cfg.benchmark,
            "frac": cfg.frac, "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
            "lambda_aug": cfg.lambda_aug, "tau": cfg.tau,
...
def main():
    ...
    for bench, task, n_cls in bms:
        for frac in fracs:
            ...
            tag = f"exp90_cgpts_unixcoder-base_{bench}_f{frac}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            try:
    ...
    with open(os.path.join(out, "exp90_cgpts_results.json"), "w") as f:
