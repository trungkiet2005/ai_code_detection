"""
================================================================================
Theory-Track exp -- Bayesian Uncertainty Attribution (BUA):
Bayesian neural networks for calibrated uncertainty in few-shot attribution.

ARXIV_ID      : NeurIPS 2015 Dropout (1506.02142); ICML 2017 SWA (1704.04909)
NAME          : Bayesian Uncertainty Attribution (BUA)
ONE-LINE CLAIM: Bayesian dropout provides calibrated uncertainty estimates that
                improve few-shot attribution by identifying high-uncertainty samples.
EQUATION      : p(y|x) ≈ 1/T Σ_t p(y|x, θ_t) where θ_t ~ q(θ)
                Uncertainty: σ²(x) = Var_T[p(y|x, θ_t)]
PROPERTY      : Monte Carlo dropout provides a variance estimate that correlates
                with attribution difficulty.
WHY NOT BEFORE: Standard point estimates don't provide uncertainty. BUA enables
                selective prediction (low confidence → abstain).
FALSIFIER     : If BUA uncertainty correlates with attribution error rate,
                the uncertainty estimates are informative.
================================================================================

exp30_bayesian.py — Bayesian uncertainty for few-shot AI-code attribution.
...
logger = logging.getLogger("exp30")
...
class Cfg:
    ...
    # BUA specific
    mc_dropout_samples: int = 10  # MC dropout passes
...
class BayesianNet(nn.Module):
    """Encoder with Bayesian dropout for uncertainty estimation."""
    ...
    def forward(self, ids, mask, mc_sample=False):
        out = self.encoder(input_ids=ids, attention_mask=mask)
        pooled = out.last_hidden_state[:, 0]
        logits = self.head(pooled)
        return {"logits": logits}

    def mc_predict(self, ids, mask, n_samples=10):
        """Monte Carlo dropout: average over multiple stochastic passes."""
        self.train()  # Enable dropout
        logits_list = []
        for _ in range(n_samples):
            with torch.no_grad():
                out = self.forward(ids, mask)
                logits_list.append(F.softmax(out["logits"], dim=-1))

        # Average predictions and compute uncertainty
        mean_pred = torch.stack(logits_list).mean(dim=0)
        uncertainty = torch.stack(logits_list).std(dim=0).mean(dim=-1)
...
def train(cfg: Cfg, tr_dl, vl_dl, ts_dl):
    ...
    for ep in range(cfg.epochs):
        ...
        tr_met = eval_m(model, tr_dl, dev, cfg)
        vr = eval_m(model, vl_dl, dev, cfg)
    ...
    final_test = eval_m(model, ts_dl, dev, cfg, use_mc=True)
...
@torch.no_grad()
def eval_m(model, loader, dev, cfg, use_mc=False):
    model.eval()
    ps, ls, uncertainties = [], [], []
    ...
    for b in loader:
        ...
        if use_mc:
            pred, unc = model.mc_predict(ids, mask, n_samples=cfg.mc_dropout_samples)
            ps.extend(pred.argmax(1).cpu().tolist())
            uncertainties.extend(unc.cpu().tolist())
        else:
            with _autocast_ctx(dev): logits = model(ids, mask)["logits"]
    ...
    if use_mc and uncertainties:
        result["mean_uncertainty"] = round(np.mean(uncertainties), 6)
        result["high_uncertainty_ratio"] = round(np.mean([u > 0.3 for u in uncertainties]), 4)
...
def main():
    ...
    for enc in encoders:
        for bench, task in benchmarks:
            ...
            tag = f"exp30_bua_{enc}_{bench}_fixed{FIXED_TOTAL}"
            logger.info(f"=== {tag} ===")
            t0 = time.time()
            ...
            row = {
                ...
                "mc_dropout_samples": cfg.mc_dropout_samples, "dropout_rate": cfg.dropout_rate,
                ...
                "mean_uncertainty": res.get("mean_uncertainty", 0),
                "high_uncertainty_ratio": res.get("high_uncertainty_ratio", 0),
    ...
    with open("results/exp30_bua_results.json", "w") as f:
