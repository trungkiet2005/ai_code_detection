...
# REFERENCE  : new; combines MIRROR (token-reverse invariance) +
#              DENOISE-style style-preserving corruption + CARGO
#              structural augmentation + TRACO supcon.
# CLAIM      : Author identity is the fixed point of MULTIPLE
#              invariances simultaneously: (a) reversal-invariant
#              (token-order shuffled is same author); (b) denoise-
#              invariant (style-preserving corruption is same author);
#              (c) structural-aug-invariant (control-flow rewrites are
#              same author). Identity = intersection of all three
#              invariances ∩ author label.
# EQUATION   : For each x: produce 4 views (x, x_rev, x_dn, x_aug)
#              Encode all to z, z_rev, z_dn, z_aug ∈ R^256
#              Stacked 4B batch for TRACO supcon (all 4 views = positives)
#              L = L_CE + λ_TW·L_TRACO(4B doubled) + λ_inv·(L_rev + L_dn + L_aug + L_cross)
#              where L_inv terms are (1 - cos) pairs between each view-pair.
# WHY NEW    : Multi-view contrastive exists with 2 views (SimCLR, ContraCode).
#              No prior code-attribution paper uses FOUR view types from
#              FOUR DIFFERENT invariance families simultaneously.
# WOW HOOK   : "Identity is the fixed point of multiple invariances.
#              An author is the unique embedding that survives reversal
#              AND denoising AND structural rewriting simultaneously.
#              Four views, one identity."
# FALSIFIER  : (F1) All 6 pairwise cos > 0.85 on test (invariances achieved).
#              (F2) Per-invariance ablation: each of 4 loss terms must
#              contribute ≥ 0.003 at 1% slot. (F3) Random pair cos << 0.85
#              (not a trivial identity collapse). (F4) Composite > TRACO + 0.005.
...
logger = logging.getLogger("exp122_mirrored")
...
# =============================================================================
# MIRRORED view construction: 4 invariance views per sample
# =============================================================================

def make_views(code: str, language: str, rng: random.Random) -> Tuple[str, str, str, str]:
    """Produce 4 views of code:
        v_orig : original
        v_rev  : token-reversed (reversal-invariance)
        v_dn   : style-preserving denoise (15% token mask)
        v_aug  : CARGO structural augmentation
    """
    v_orig = code
    toks = code.split()
    v_rev = " ".join(toks[::-1]) if toks else code
    if toks:
        masked = []
        for t in toks:
        ...
        v_dn = code
    v_aug, _ = cargo_augment(code, language, rng)
    return v_orig, v_rev, v_dn, v_aug
...
def supcon_tw_loss(z_doubled, labels_doubled, dist_mat, gamma=1.0, tau=0.1):
    """Tree-weighted SupCon on a stacked batch [z_1; z_2; ...; z_V]."""
    N = z_doubled.size(0)
...
class Cfg:
    ...
    bs: int = 32; seq: int = 512; epochs: int = 3
    lr_enc: float = 2e-5; lr_head: float = 1e-4
    warmup: float = 0.1; wd: float = 0.01
    lambda_aug: float = 0.5; lambda_inv: float = 0.3
...
def _hw(cfg):
    if torch.cuda.is_available():
        ...
        # 4-view: bs ~ 1/4 of TRACO's 128
        if mem >= 80: cfg.bs, cfg.seq = 48, 512
        elif mem >= 40: cfg.bs, cfg.seq = 32, 512
        elif mem >= 10: cfg.bs, cfg.seq = 16, 384
        else: cfg.bs, cfg.seq = 8, 256
        logger.info(f"[hw] mem={mem:.1f}GB bs={cfg.bs} (QUARTERED for 4x view) seq={cfg.seq}")
    return cfg
...
class FSDS_MIRRORED(TD):
    """Returns 4 (ids, mask) pairs per sample: orig, rev, dn, aug."""
    def __init__(self, data, tok, seq_len, frac=1.0, seed=42, do_aug=True):
        ...
        if frac < 1.0:
            ...
            logger.info(f"[FSDS_MIRRORED] Sampled {len(self.data)} samples ({frac*100:.0f}%)")

    def __len__(self): return len(self.data)

    def _enc_one(self, txt):
        enc = self.tok(txt, max_length=self.seq_len, padding="max_length",
    ...
    def __getitem__(self, i):
        ...
        if self.do_aug:
            ...
            v0, v1, v2, v3 = make_views(code, lang, rng)
        else:
            v0 = v1 = v2 = v3 = code
        ids0, mask0 = self._enc_one(v0)
        ids1, mask1 = self._enc_one(v1)
        ids2, mask2 = self._enc_one(v2)
        ids3, mask3 = self._enc_one(v3)
        return {"ids0": ids0, "mask0": mask0,
                "ids1": ids1, "mask1": mask1,
                "ids2": ids2, "mask2": mask2,
                "ids3": ids3, "mask3": mask3,
                "label": r["label"],
...
def eval_pack(model, loader, cfg, sib_mask_np, dist_mat_cpu, collect_falsifier=False):
    """Eval pack. F1 falsifier = all 6 pairwise cos between (z0,z1,z2,z3) on test."""
    model.eval(); preds, labels, langs, sources = [], [], [], []
    pair_cos: Dict[str, List[float]] = {}
    random_pair_cos: List[float] = []
    pair_names = [(0,1,"orig_vs_rev"), (0,2,"orig_vs_dn"), (0,3,"orig_vs_aug"),
                  (1,2,"rev_vs_dn"),   (1,3,"rev_vs_aug"), (2,3,"dn_vs_aug")]
    for b in tqdm(loader, desc="Eval"):
        labs = b["label"]
        zs = []
        ids_list = [b[f"ids{i}"].to(cfg.device) for i in range(4)]
        msk_list = [b[f"mask{i}"].to(cfg.device) for i in range(4)]
        for i in range(4):
            z, lg = model.encode(ids_list[i], msk_list[i])
            zs.append(z)
            if i == 0:
                logits = lg
        ...
        if collect_falsifier:
            for (a, b_, nm) in pair_names:
                sim = (zs[a] * zs[b_]).sum(dim=-1).detach().cpu().float().numpy()
                pair_cos.setdefault(nm, []).extend([float(x) for x in sim])
            # F3: random pair cos (shuffled across batch, view 0 vs view 0)
            if zs[0].size(0) > 1:
                perm = torch.randperm(zs[0].size(0), device=zs[0].device)
                rand_sim = (zs[0] * zs[0][perm]).sum(dim=-1).detach().cpu().float().numpy()
                random_pair_cos.extend([float(x) for x in rand_sim])
    ...
    if collect_falsifier:
        per_pair_stats = {}
        all_cos = []
        for nm, lst in pair_cos.items():
            if not lst: continue
            per_pair_stats[nm] = {"mean_cos": float(np.mean(lst)),
                                  "std_cos": float(np.std(lst)),
                                  "n": len(lst)}
            all_cos.extend(lst)
        out["falsifier"] = {
            "mean_pairwise_cos_F1": float(np.mean(all_cos)) if all_cos else 0.0,
            "per_pair_cos_F1": per_pair_stats,
            "random_pair_cos_F3": float(np.mean(random_pair_cos)) if random_pair_cos else 0.0,
            "random_pair_std_F3": float(np.std(random_pair_cos)) if random_pair_cos else 0.0,
            "n_random_pairs": int(len(random_pair_cos)),
        }
    return out
...
def train_epoch(model, loader, opt, sch, scaler, cfg, dist_mat):
    """4-view training:
        - CE on view 0 (orig)
        - TRACO SupCon on stacked 4N batch (all 4 views, same label)
        - Pairwise (1-cos) invariance loss across 6 view-pairs.
    """
    model.train()
    tot, ce_s, sc_s, inv_s = 0.0, 0.0, 0.0, 0.0
    for b in tqdm(loader, desc="Train"):
        ids = [b[f"ids{i}"].to(cfg.device) for i in range(4)]
        msk = [b[f"mask{i}"].to(cfg.device) for i in range(4)]
        labs = b["label"].to(cfg.device)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(cfg.device == "cuda")):
            zs, logits_list = [], []
            for i in range(4):
                z, lg = model.encode(ids[i], msk[i])
                zs.append(z); logits_list.append(lg)
            loss_ce = F.cross_entropy(logits_list[0], labs)
            z_stack = torch.cat(zs, dim=0)
            y_stack = torch.cat([labs] * 4, dim=0)
            loss_sc = supcon_tw_loss(z_stack, y_stack, dist_mat, gamma=cfg.gamma, tau=cfg.tau)
            loss_inv = zs[0].sum() * 0.0
            n_pairs = 0
            for i in range(4):
                for j in range(i + 1, 4):
                    loss_inv = loss_inv + (1.0 - (zs[i] * zs[j]).sum(-1).mean())
                    n_pairs += 1
            loss_inv = loss_inv / max(n_pairs, 1)
            loss = loss_ce + cfg.lambda_aug * loss_sc + cfg.lambda_inv * loss_inv
        scaler.scale(loss).backward()
        scaler.unscale_(opt)
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        scaler.step(opt); scaler.update(); opt.zero_grad(); sch.step()
        tot += loss.item(); ce_s += loss_ce.item(); sc_s += loss_sc.item(); inv_s += loss_inv.item()
    n = max(1, len(loader))
    return tot / n, ce_s / n, sc_s / n, inv_s / n
...
def run_exp(cfg, tag):
    ...
    tr_ds = FSDS_MIRRORED(tr_data, tok, cfg.seq, frac=cfg.frac, seed=cfg.seed, do_aug=True)
    vl_ds = FSDS_MIRRORED(vl_data, tok, cfg.seq, frac=1.0, seed=cfg.seed + 1, do_aug=True)
    ts_ds = FSDS_MIRRORED(ts_data, tok, cfg.seq, frac=1.0, seed=cfg.seed + 2, do_aug=True)
    ...
    logger.info(f"[sched] frac={cfg.frac} epochs={cfg.epochs} lr_enc={cfg.lr_enc} warmup={cfg.warmup} "
                f"lambda_aug={cfg.lambda_aug} lambda_inv={cfg.lambda_inv} gamma={cfg.gamma}")
    loader_cfg = dict(batch_size=cfg.bs, num_workers=4, pin_memory=True)
    tr_dl = DataLoader(tr_ds, shuffle=True, **loader_cfg)
    vl_dl = DataLoader(vl_ds, shuffle=False, **loader_cfg)
    ts_dl = DataLoader(ts_ds, shuffle=False, **loader_cfg)
    model = MIRROREDModel(cfg.enc, cfg.n_cls, cfg.emb_dim).to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        loss, ce, sc, inv = train_epoch(model, tr_dl, opt, sch, scaler, cfg, dist_mat)
        val_met = eval_pack(model, vl_dl, cfg, sib_mask_np, dist_mat_cpu)
        v = val_met["overall"]["macro_f1"]; val_hist.append(v)
        logger.info(f"[epoch {epoch+1}] loss={loss:.4f} ce={ce:.4f} supcon={sc:.4f} inv={inv:.4f} val={v:.4f}")
        if v > best_val:
            best_val = v
    ...
    logger.info(f"[final] val={best_val:.4f} test={test_macro:.4f} gap={gap:+.4f} "
                f"mean_pair_cos={fa['mean_pairwise_cos_F1']:.3f} "
                f"random_pair_cos={fa['random_pair_cos_F3']:.3f}")
    for nm, st in sorted(fa["per_pair_cos_F1"].items()):
        logger.info(f"  [pair={nm:18}] mean_cos={st['mean_cos']:+.3f} std={st['std_cos']:.3f} n={st['n']}")
    return {"tag": tag, "method": "MIRRORED",
            "note": "4-view multi-invariance: orig+rev+dn+aug; TRACO supcon over 4N + 6-pair (1-cos) invariance",
            "enc": cfg.enc, "bench": cfg.benchmark,
            "frac": cfg.frac, "epochs": cfg.epochs, "lr_enc": cfg.lr_enc,
            "lambda_aug": cfg.lambda_aug, "lambda_inv": cfg.lambda_inv,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp122_mirrored_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
                    ...
                    logger.info(f"[{tag}] test={res['macro']:.4f} ({res['dpaper']:+.4f}) "
                                ...
                                f"pair_cos={fa['mean_pairwise_cos_F1']:.3f} "
                                f"rand_cos={fa['random_pair_cos_F3']:.3f} "
    ...
    with open(os.path.join(out_dir, "exp122_mirrored_results.json"), "w") as f:
        json.dump(results, f, indent=2)
    print("\n" + "="*150)
    print(f"{'Encoder':<22} {'Benchmark':<12} {'Frac':>6} {'Ep':>4} {'Val-F1':>8} {'Test-F1':>8} {'Gap':>8} "
          f"{'dPaper':>9} {'pair_cos':>9} {'rand_cos':>9} {'Wall':>8}")
    print("-"*150)
    for r in results:
        ...
        print(f"{r['enc']:<22} {r['bench']:<12} {r['frac']:>6.0%} {r['epochs']:>4d} "
              ...
              f"{fa['mean_pairwise_cos_F1']:>9.3f} {fa['random_pair_cos_F3']:>9.3f} {r['wall']:>8.0f}s")
    print("="*150)
