# exp76_traco — Token-level Robust Augmentation COntrastive (TRACO)
# =============================================================================
# Theory-Track exp -- TRACO (Token-level Robust Augmentation Contrastive)
#
# ROLE           : The ablation at 20% (exp65_abl) showed that CE-only matches
#                  every structure-aware loss on top of the RAS schedule -- the
#                  encoder + schedule extract all signal the labels carry.
#                  To break the ceiling we must add SIGNAL the encoder hasn't
#                  seen.  TRACO injects same-author-different-surface positives
#                  via on-the-fly token-level transformations, grounded in S7:
#                  the identity of an LLM is invariant under sampling-temperature
#                  changes (same prompt + same model + different T -> different
#                  surface code, same author).
# NAME           : TRACO  (Token-level Robust Augmentation Contrastive)
# ARXIV_ID       : novel; differs from SimCLR/MoCo by grounding view-invariance
#                  in S7 (decoder-T invariance), not in image-domain priors.
# ONE-LINE CLAIM : AI-code author identity is invariant under semantic-
#                  preserving surface transformations; training the encoder to
#                  enforce this invariance via multi-view contrastive provides
#                  signal beyond what raw supervised labels carry.
# EQUATION       : For each x in batch:
#                    x' = T(x)   T in {token_dropout, id_rename, ws_jitter,
#                                       comment_strip}  sampled per-sample
#                    z = phi(x);   z' = phi(x')
#                  L_supcon_tw = SupCon-Tree-Weighted on doubled batch [z; z']
#                                with positives = (z_i, z'_i) and same-label
#                                pairs; negatives weighted by exp(-gamma * d_tree)
#                  L_ce = CE(clf(z), y)   (classifier sees only original view)
#                  L_total = L_ce + lambda_aug * L_supcon_tw
# WHY NOT BEFORE : View-augmentation contrastive (SimCLR 2020, MoCo 2019) was
#                  developed for image domain; no prior code-attribution work
#                  USES augmentation views.  STYLO (exp68, surface counting)
#                  is the OPPOSITE of invariance; PERPSIG / DECOFP do not
#                  augment.  RACO is single-view contrastive.  TRACO is the
#                  first method to use S7-grounded augmentation contrastive
#                  for LLM-code attribution.
# FALSIFIER      : (F1) View-invariance: mean cos(z, z') on TEST after training.
#                       > 0.90 = encoder genuinely invariant.  < 0.5 = encoder
#                       collapsed under augmentation (failure mode).
#                  (F2) Per-fraction gain over CE-only baseline: TRACO macro_F1
#                       - exp65_abl CE macro_F1 at same fraction.  Positive +
#                       monotone-decreasing with n confirms "augmentation helps
#                       more at extreme few-shot".
#                  (F3) Augmentation invariance over transforms: per-transform
#                       cos(z, x_t).  Identifies if any single transform causes
#                       collapse or no signal.
# REPORTS        : Full eval pack + (F1) mean test view-cos
#                                 + (F2) per-transform sample count
#                                 + (F3) per-transform cos statistic
# =============================================================================
...
logger = logging.getLogger("exp76_traco")
...
# =============================================================================
# Token-level code augmentations (S7 invariance)
# =============================================================================

_RESERVED = {
    "if", "else", "elif", "for", "while", "do", "return", "def", "function", "func",
...
def aug_token_dropout(code: str, rng: random.Random, p: float = 0.1) -> str:
    """Drop ~p fraction of whitespace-separated tokens, replace with single space."""
...
def aug_id_rename(code: str, rng: random.Random, max_renames: int = 8) -> str:
    """Rename random non-reserved identifiers to v0, v1, ... consistently."""
    ids = set(_re.findall(r"\b[a-zA-Z_]\w{2,}\b", code))   # 3+ char identifiers only
...
def aug_ws_jitter(code: str, rng: random.Random, p: float = 0.15) -> str:
    """Insert random spaces around operators."""
    ops = ["+", "-", "*", "/", "%", "=", "<", ">", ",", ";"]
    out_chars = []
    for c in code:
        out_chars.append(c)
        if c in ops and rng.random() < p:
            out_chars.append(" ")
    return "".join(out_chars)


def aug_comment_strip(code: str, rng: random.Random) -> str:
    """Strip all comments (#, //, /* */)."""
...
def augment(code: str, rng: random.Random) -> Tuple[str, str]:
...
def supcon_tw_loss(z_doubled, labels_doubled, dist_mat, gamma=1.0, tau=0.1):
    ...
    Positives for anchor i: any j != i with y_j = y_i (includes z'_i).
    Negative pair weight: exp(-gamma * d_tree(y_i, y_j)) so sibling negs are hard.
    """
    N = z_doubled.size(0)
...
class FSDS_AUG(TD):
    """Dataset that returns (original_tokens, aug_tokens, label, aug_name).

    For TRAIN: do augmentation.  For VAL/TEST: original twice with aug_name='none'.
    """
    ...
    def __getitem__(self, i):
        ...
        if self.do_aug:
            ...
            code_aug, aug_name = augment(code, rng)
...
def eval_pack(model, loader, cfg, sib_mask_np, dist_mat_cpu, collect_falsifier=False):
    ...
    cos_per_aug = {n: [] for n, _ in _AUG_TABLE}
    cos_per_aug["none"] = []
    ...
    if collect_falsifier:
        ...
        for n, lst in cos_per_aug.items():
            if not lst: continue
            per_aug_stats[n] = {"mean_cos": float(np.mean(lst)), "std_cos": float(np.std(lst)), "n": len(lst)}
            if n != "none": all_cos.extend(lst)
        out["falsifier"] = {
...
def train_epoch(model, loader, opt, sch, scaler, cfg, dist_mat):
    ...
    aug_counts = {n: 0 for n, _ in _AUG_TABLE}
    for b in tqdm(loader, desc="Train"):
        ...
        for n in b.get("aug_name", []):
            if n in aug_counts: aug_counts[n] += 1
...
def run_exp(cfg, tag):
    ...
    for name, st in fa["per_aug_cos_F3"].items():
        logger.info(f"  [aug={name:14}] mean_cos={st['mean_cos']:.3f} std={st['std_cos']:.3f} n={st['n']}")
    return {"tag": tag, "method": "TRACO", "enc": cfg.enc, "bench": cfg.benchmark,
...
def main():
    ...
    for enc in encoders:
        for bench, task, n_cls in benchmarks:
            for frac in fracs:
                ...
                tag = f"exp76_traco_{enc}_{bench}_f{frac}"
                logger.info(f"=== {tag} ===")
                t0 = time.time()
                try:
                    res = run_exp(cfg, tag)
    ...
    with open(os.path.join(out_dir, "exp76_traco_results.json"), "w") as f:
